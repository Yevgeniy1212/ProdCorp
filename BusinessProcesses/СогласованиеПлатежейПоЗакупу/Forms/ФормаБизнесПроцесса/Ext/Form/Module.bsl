...
Процедура ОбновитьИсториюИзменений()
	...
	КонецЦикла;
	Макет = БизнесПроцессы.СогласованиеПлатежейПоЗакупу.ПолучитьМакет("Макет");
...
Процедура ПодготовитьФорму(Форма)
	...
	Если Объект.стартован 
		и не ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоЗакупу.ТочкаМаршрута.РаботаСЗаявкойАвтором") Тогда
	...
	ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоЗакупу.ТочкаМаршрута.РаботаСЗаявкойАвтором")
	...
	//ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоЗакупу.ТочкаМаршрута.СозданиеПП")	тогда
...
Процедура ВыполнитьЭтап(Команда)
	ОбработатьБП("ОК");
	ОповеститьОбИзменении(ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоЗакупу.ПустаяСсылка"));
...
&НаКлиенте
Процедура РасчетнаяТаблицаСобственныеПриИзменении(Элемент)
	ТекСтрока = Элементы.РасчетнаяТаблица.ТекущиеДанные; 
	ТекСтрока.Заемные = ТекСтрока.СуммаДоговора - ТекСтрока.Собственные;
КонецПроцедуры

&НаКлиенте
Процедура РасчетнаяТаблицаЗаемныеПриИзменении(Элемент)
	ТекСтрока = Элементы.РасчетнаяТаблица.ТекущиеДанные; 
	ТекСтрока.Собственные = ТекСтрока.СуммаДоговора - ТекСтрока.Заемные;
КонецПроцедуры
