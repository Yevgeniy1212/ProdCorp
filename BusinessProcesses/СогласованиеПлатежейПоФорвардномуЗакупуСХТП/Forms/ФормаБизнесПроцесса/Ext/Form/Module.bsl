...
Процедура ОбновитьИсториюИзменений()
	...
	КонецЦикла;
	Макет = БизнесПроцессы.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ПолучитьМакет("Макет");
...
Процедура ПодготовитьФорму(Форма)
	...
	Если Объект.стартован 
		и не ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ТочкаМаршрута.РаботаСЗаявкойАвтором") Тогда
	...
	ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ТочкаМаршрута.СогласованиеСГБ")			тогда       
		Элементы.ЗаявкаПрошлаСогласованиеГБ.Доступность 		= истина;
	...
	ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ТочкаМаршрута.РаботаСЗаявкойАвтором")
	...
	//ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ТочкаМаршрута.СозданиеПП")	тогда
...
Процедура ВыполнитьЭтап(Команда)
	ОбработатьБП("ОК");
	ОповеститьОбИзменении(ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ПустаяСсылка"));
...
Функция ОбработатьЭтап(РезультатВозврата)
	...
	ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ТочкаМаршрута.СогласованиеСГБ")			тогда      
		Объект.ЗаявкаПрошлаСогласованиеГБ = ЗначениеПоля;
	ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежейПоФорвардномуЗакупуСХТП.ТочкаМаршрута.СогласованиеСФД")			тогда     
...
Процедура ПриОткрытии(Отказ)
	...
	ДопОплатаПриИзменении(Элементы.ДопОплата);
...
Процедура СоздатьППИНаСервере()
	...
	СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Расчеты с поставщиками и подрядчиками");
	ТипОперации = Справочники.ТипыОпераций.НайтиПоНаименованию("ЗАКУП (ФОРВАРД 2020)");
	
	Для Каждого Стр из Объект.РасчетнаяТаблица Цикл
		...
		ППИ.ВалютаДокумента = Справочники.Валюты.НайтиПоКоду("398");
		ППИ.Организация = Объект.Организация;
		...
		ППИ.КодНазначенияПлатежа = "710";
		ППИ.ДатаВалютирования = ТекущаяДата();
		
		ППИ.СчетОрганизации = Стр.РасчетныйСчет;
		ППИ.СчетБанк = ПланыСчетов.Типовой.ДенежныеСредстваНаТекущихБанковскихСчетахСоб;
		Если Объект.ДопОплата = Истина Тогда 
			ППИ.СуммаДокумента = Стр.СуммаДоплаты;			
		Иначе
			ППИ.СуммаДокумента = Стр.СуммаДоговора;
		КонецЕсли;
		ППИ.Контрагент = Стр.Контрагент;
		ППИ.ТипОперации = ТипОперации;
		ППИ.СчетКонтрагента = Стр.Контрагент.ОсновнойБанковскийСчет;
		ППИ.НазначениеПлатежа = "Финансирование по договору форвардного закупа зерна согласно договора №" + Стр.НомерДоговора.НомерДоговора + " от " + Стр.НомерДоговора.ДатаДоговора +
		". Сумма " + Стр.СуммаДоговора + " тенге в т.ч. НДС(без НДС) 0-00 тенге";
		
		СтрокаППИ = ППИ.РасшифровкаПлатежа.Добавить();
		
		СтрокаППИ.ДоговорКонтрагента = Стр.НомерДоговора;
		СтрокаППИ.КратностьВзаиморасчетов = 1;
		СтрокаППИ.КурсВзаиморасчетов = 1;
		СтрокаППИ.СтатьяДвиженияДенежныхСредств = СтатьяДДС;
		
		Если Объект.ДопОплата = Истина Тогда 
			СтрокаППИ.СуммаВзаиморасчетов = Стр.СуммаДоплаты;
			СтрокаППИ.СуммаПлатежа = Стр.СуммаДоплаты;
		Иначе
			СтрокаППИ.СуммаВзаиморасчетов = Стр.СуммаДоговора;
			СтрокаППИ.СуммаПлатежа = Стр.СуммаДоговора;
		КонецЕсли;
	
		СтрокаППИ.СчетУчетаРасчетовСКонтрагентомБУ = ПланыСчетов.Типовой.КраткосрочныеАвансыВыданныеСоб;
...
&НаКлиенте
Процедура ДопОплатаПриИзменении(Элемент)
	
	Если Объект.ДопОплата = Истина Тогда 
		Элементы.РасчетнаяТаблицаСуммаПредварительнойОплаты.Видимость = Истина;	
		Элементы.РасчетнаяТаблицаСуммаДополнительнойОплаты.Видимость = Истина;	
		Элементы.РасчетнаяТаблицаСуммаПогашенияСельхозПродукцией.Видимость = Истина;	
		Элементы.РасчетнаяТаблицаСуммаДоплаты.Видимость = Истина;
		Элементы.РасчетнаяТаблицаСуммаШтрафа.Видимость = Истина;	
		Элементы.РасчетнаяТаблицаФактическиЗачтенныйВес.Видимость = Истина;	
		Элементы.РасчетнаяТаблицаЗакупочнаяЦена.Видимость = Истина;	
		//Элементы.РасчетнаяТаблицаПрогнознаяЦена.Видимость = Истина;
		Элементы.РасчетнаяТаблицаКультураДоп.Видимость = Истина;
		Элементы.РасчетнаяТаблицаГруппа3.Видимость = Ложь;
		Элементы.РасчетнаяТаблицаГарантия.Видимость = Ложь;
	Иначе 
		Элементы.РасчетнаяТаблицаСуммаПредварительнойОплаты.Видимость = Ложь;	
		Элементы.РасчетнаяТаблицаСуммаДополнительнойОплаты.Видимость = Ложь;	
		Элементы.РасчетнаяТаблицаСуммаПогашенияСельхозПродукцией.Видимость = Ложь;	
		Элементы.РасчетнаяТаблицаСуммаДоплаты.Видимость = Ложь;	
		Элементы.РасчетнаяТаблицаСуммаШтрафа.Видимость = Ложь;	
		Элементы.РасчетнаяТаблицаФактическиЗачтенныйВес.Видимость = Ложь;	
		Элементы.РасчетнаяТаблицаЗакупочнаяЦена.Видимость = Ложь;	
		//Элементы.РасчетнаяТаблицаПрогнознаяЦена.Видимость = Ложь;	
		Элементы.РасчетнаяТаблицаКультураДоп.Видимость = Ложь;		
		Элементы.РасчетнаяТаблицаГруппа3.Видимость = Истина;
		Элементы.РасчетнаяТаблицаГарантия.Видимость = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура РасчетСуммы(Команда)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СУММА(ден_ДвиженияДенежныхСредствОбороты.СуммаОборот) КАК СуммаОборот
	|ИЗ
	...
	|		ПО ден_ДвиженияДенежныхСредствОбороты.ДокументДвижения = ПлатежноеПоручениеИсходящееРасшифровкаПлатежа.Ссылка
	|ГДЕ
	|	ПлатежноеПоручениеИсходящееРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентомБУ = &СчетУчета1711
	|	И ЕСТЬNULL(ден_ДвиженияДенежныхСредствОбороты.СуммаОборот, 0) <> 0";
	...
	Если Объект.РасчетнаяТаблица.Количество() > 1 Тогда
		Для Каждого строка из Объект.РасчетнаяТаблица Цикл
			...
			Запрос.УстановитьПараметр("СчетУчета1711", ПланыСчетов.Типовой.НайтиПоКоду("1711"));
			...
			//Smart Tech. Amanov R & Sholpan - Доработали под тз заказчиком. Данная проверка будет пропушена. Работает след код.
			...
			строка.СуммаДоплаты = строка.СуммаПогашенияСельхозПродукцией - (строка.СуммаПредварительнойОплаты * строка.ФактическиЗачтенныйВес) - строка.СуммаШтрафа;
	...
	Иначе
		...
		Объект.РасчетнаяТаблица[0].СуммаДоплаты = Объект.РасчетнаяТаблица[0].СуммаПогашенияСельхозПродукцией - (Объект.РасчетнаяТаблица[0].СуммаПредварительнойОплаты * Объект.РасчетнаяТаблица[0].ФактическиЗачтенныйВес) - Объект.РасчетнаяТаблица[0].СуммаШтрафа;
...
&НаКлиенте
Процедура РасчетнаяТаблицаСФПриИзменении(Элемент)
	ТекДанные = Элементы.РасчетнаяТаблица.ТекущиеДанные;
	ТекДанные.ДатаВыписки = ТекДанные.СФ.Дата;
	ТекДанные.РегНомер = ТекДанные.СФ.РегистрационныйНомер;
КонецПроцедуры
