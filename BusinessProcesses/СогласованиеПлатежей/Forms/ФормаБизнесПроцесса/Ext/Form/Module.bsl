...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		// {{ Начало Аксиома: Попов В.С. 28.07.2024 11:03
...
Процедура ОбновитьИсториюИзменений()
	...
	КонецЦикла;
	Макет = БизнесПроцессы.СогласованиеПлатежей.ПолучитьМакет("Макет");
...
Процедура ПодготовитьФорму(Форма)
	...
	Если Объект.стартован 
		и не ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежей.ТочкаМаршрута.РаботаСЗаявкойАвтором") Тогда
	...
	ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежей.ТочкаМаршрута.РаботаСЗаявкойАвтором")
...
Функция ОбработатьЭтап(РезультатВозврата)
	...
	// {{ Начало Аксиома: Попов В.С. 01.06.2024 18:03
	...
	ИначеЕсли ТекущаяТочка = ПредопределенноеЗначение("БизнесПроцесс.СогласованиеПлатежей.ТочкаМаршрута.СогласованиеДиректораФД")			тогда
		Объект.ЗаявкаПрошлаСогласованиеДиректораФД = ЗначениеПоля;
	// }} Окончание  Аксиома: Попов В.С.
	КонецЕсли;	
...
Процедура РасчетнаяТаблицаСтатьяБюджетаПриИзмененииНаСервере()
	...
	//РассчитатьГодовойПлан(); //-=-= 20.11.2024 По требованию, чтобы не слетала колонка "Годовой план"	
	//РассчитатьОтклонение();  //-=-= 11.12.2024 По требованию, чтобы не слетали колонки "Факт предыдущего периода" и "Остаток плана с учетом факта"
...
Процедура СоздатьППИ(Команда)
	...
	//Элементы.РасчетнаяТаблица.ТекущиеДанные
КонецПроцедуры

&НаКлиенте
Процедура РасчетнаяТаблицаСчетКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Запрос.УстановитьПараметр("Владелец", Элементы.РасчетнаяТаблица.ТекущиеДанные.Контрагент);
