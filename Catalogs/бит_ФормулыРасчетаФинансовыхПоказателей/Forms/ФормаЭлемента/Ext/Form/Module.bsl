...
Процедура ВходящиеПоказателиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока И НЕ Копирование Тогда
		Элементы.ВходящиеПоказатели.ТекущиеДанные.ВидПоказателя = ПредопределенноеЗначение("Перечисление.фин_ВидыПоказателейФормулРасчетаФинансовыхПоказателей.СтатьиБюджета");
...
Функция ВыполнитьПроверку()
	...
	Для Каждого ПОказатель Из Объект.ВходящиеПоказатели Цикл
		ТекстФормулы = СтрЗаменить(ТекстФормулы, ПОказатель.ПредставлениеПоказателя, "0");
	КонецЦикла;
	Попытка 
		РезультатРасчета = Вычислить(ТекстФормулы);
		Сообщить("Формула составлена правильно.");
	Исключение
		Сообщить("Ошибка формулы");
		ТекстОшибки = ОписаниеОшибки();
		Для НСимвола = -СтрДлина(ТекстОшибки) По -1 Цикл
			Если СтрДлина(ТекстОшибки) - СтрДлина(Сред(ТекстОшибки, 1, -НСимвола)) > 3 Тогда
				Если Сред(ТекстОшибки, -НСимвола, 3) = ")}:" Тогда
					ТекстОшибки = Сред(ТекстОшибки, -НСимвола + 3, СтрДлина(ТекстОшибки));
					Прервать;
				КонецЕсли;
		...
		КонецЦикла;
		Сообщить(СтрЗаменить(ТекстОшибки, "Деление на 0", "Вероятность деления на 0!"), СтатусСообщения.ОченьВажное);
		БезОшибок = Ложь;
