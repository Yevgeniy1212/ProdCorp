...
&НаКлиенте
Процедура НачалоВыбораТНВЭД(СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ТекущийКодТНВЭД", ?(ЗначениеЗаполнено(Объект.КодТНВЭД), СокрЛП(Объект.КодТНВЭД), Неопределено));
...
Процедура КодТНВЭДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НачалоВыбораТНВЭД(СтандартнаяОбработка);
...
&НаКлиенте
Процедура ГСВСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
Процедура ГСВСПриИзменении(Элемент)
	...
	УстановитьКодТНВЭД(Объект.ГСВС);
...
Процедура УстановитьКодТНВЭД(НомеклатураГСВС)
	...
	Объект.КодТНВЭД = НомеклатураГСВС.КодГСВС;
