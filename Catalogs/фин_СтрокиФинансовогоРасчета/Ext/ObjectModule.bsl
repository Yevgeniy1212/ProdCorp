...
Процедура ИзменитьКод(Направление) Экспорт
	...
	СтрокиРасчета  = Справочники.фин_СтрокиФинансовогоРасчета;
	...
	Запрос.УстановитьПараметр("ВидСтроки",Перечисления.фин_ВидыСтрокФинансовогоРасчета.РасчетПоДругимСтрокамОтчета);
...
Процедура ПередЗаписью(Отказ)
	...
	Если ВидСтроки=Перечисления.фин_ВидыСтрокФинансовогоРасчета.ПустаяСсылка() Тогда
		ТекстСообщения="Не указан вид строки финансового расчета.";
	ИначеЕсли ВидСтроки=Перечисления.фин_ВидыСтрокФинансовогоРасчета.ОборотыПоСтатьеОборотов
		И Не ЗначениеЗаполнено(Измерение) Тогда
		ТекстСообщения="Не указан финансовый показатель.";
	ИначеЕсли ВидСтроки=Перечисления.фин_ВидыСтрокФинансовогоРасчета.ОстаткиОборотыПоПлануСчетов Тогда
		...
		КонецЕсли;
	ИначеЕсли ВидСтроки=Перечисления.фин_ВидыСтрокФинансовогоРасчета.ИсточникиДанныхДляРасчетов Тогда
		Если Не ЗначениеЗаполнено(Измерение) Тогда
			ТекстСообщения="Не указан источник данных для расчетов.";
		ИначеЕсли Не ЗначениеЗаполнено(Формула) Тогда
			ТекстСообщения="Не указан показатель источника";
		КонецЕсли;
	ИначеЕсли ВидСтроки=Перечисления.фин_ВидыСтрокФинансовогоРасчета.РасчетПоДругимСтрокамОтчета И Не ЗначениеЗаполнено(Формула)  Тогда
