...
Процедура Печать(Регистратор = "") Экспорт
	...
	|
	|ИТОГИ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Физлицо) ПО
		...
		Иначе
			...
			Для Каждого СоставнаяЧастьМероприятия Из ТаблицаСоставаМероприятия Цикл
				...
				Иначе
					...
					//ОбластьПоказатель.Параметры.ДатаМероприятия = СоставнаяЧастьМероприятия.НачалоМероприятия;
					ДокументРезультат.Присоединить(ОбластьПоказатель);
				...
				КонецЕсли;
			КонецЦикла;
			//ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы,КолонкаНачалаОбъединения,ДокументРезультат.ВысотаТаблицы,КолонкаОкончанияОбъединения).Объединить();
		КонецЕсли;
	КонецЦикла;
	ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы-2,2,ДокументРезультат.ВысотаТаблицы,ДокументРезультат.ШиринаТаблицы-1).ЦветФона = ЦветаСтиля.ФонГруппировкиВерхнегоУровня;
