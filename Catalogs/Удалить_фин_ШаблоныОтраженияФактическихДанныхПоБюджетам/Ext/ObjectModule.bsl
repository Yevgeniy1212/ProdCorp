...
Процедура ПередЗаписью(Отказ)
	...
	Для Каждого СтрокаТЧ Из НаборПравилОтражения Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ВидДанных) Тогда
			Сообщить("Элемент справочника Шаблоны отражения фактических данных по бюджетам 
			|	"+Ссылка+" не может быть записан, так как в строке №"+Строка(СтрокаТЧ.НомерСтроки)+" не указан вид данных");
			Отказ = Истина;
		ИначеЕсли СтрокаТЧ.ВидДанных <>Перечисления.фин_ВидыОтраженийПервичныхДокументовВБюджетировании.ПоШапкеДокумента И СтрокаТЧ.ИмяИсточника="" Тогда
			Сообщить("Элемент справочника Шаблоны отражения фактических данных по бюджетам 
			|	"+Ссылка+" не может быть записан, так как в строке №"+Строка(СтрокаТЧ.НомерСтроки)+" не указан источник данных");
			Отказ = Истина;
		КонецЕсли;
