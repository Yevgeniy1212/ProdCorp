...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦветСветлоСерый = Новый Цвет(220,220,220);
	ЦветКрасноФиолетовый = Новый Цвет(208,32,144);
	...
	Если ТипЗнч(Параметры.СравниваемыеВерсии) = Тип("Массив") Тогда
		...
		Если СравниваемыеВерсии.Количество() > 1 Тогда
			...
			Заголовок = куфиб_ОбщегоНазначения.ПодставитьПараметрыВСтроку(
...
Функция ПолучитьОписаниеПоВерсии(НомерВерсии)
	...
	Описание = куфиб_ОбщегоНазначения.ПодставитьПараметрыВСтроку(НСтр("ru = '№ %1 / (%2) / %3'"), 
