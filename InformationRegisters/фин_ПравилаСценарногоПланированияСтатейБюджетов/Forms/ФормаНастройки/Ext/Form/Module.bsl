...
//	ЗаполнитьЗначенияСвойств(ЭтотОбъект,Параметры,"Бюджет");
	...
	Если СценарииПланирования.Количество()=0 Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	фин_ПравилаСценарногоПланированияБюджетов.ФормируемыйСценарий,
			|	фин_ПравилаСценарногоПланированияБюджетов.ФормируемыйСценарий.Наименование
			|ПОМЕСТИТЬ ВТ
			|ИЗ
			|	РегистрСведений.фин_ПравилаСценарногоПланированияБюджетов КАК фин_ПравилаСценарногоПланированияБюджетов
			|ГДЕ
			|	фин_ПравилаСценарногоПланированияБюджетов.Бюджет = &Бюджет
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	фин_ПравилаСценарногоПланированияСтатейБюджетов.ФормируемыйСценарий,
			|	фин_ПравилаСценарногоПланированияСтатейБюджетов.ФормируемыйСценарий.Наименование
			|ИЗ
			|	РегистрСведений.фин_ПравилаСценарногоПланированияСтатейБюджетов КАК фин_ПравилаСценарногоПланированияСтатейБюджетов
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	КонецЕсли;
	ЗаполнитьДанные();
	СписокРежимы.Добавить(Перечисления.фин_СпособыЗаполненияСтатейПриСценарныхРасчетах.НеЗаполнять,"Не заполнять");
	СписокРежимы.Добавить(Перечисления.фин_СпособыЗаполненияСтатейПриСценарныхРасчетах.ПоОбщемуПравилу,"По общему правилу");
	СписокРежимы.Добавить(Перечисления.фин_СпособыЗаполненияСтатейПриСценарныхРасчетах.СПрименениемКоэффициента,"С применением коэффициента");
	СписокРежимы.Добавить(Перечисления.фин_СпособыЗаполненияСтатейПриСценарныхРасчетах.ПоФормуле,"По формуле");
КонецПроцедуры
...
Процедура ЗаполнитьДанные()
	...
	Отчет = Отчеты.фин_ПравилаСценарногоПланированияПоСтатьям.Создать();
...
Процедура ТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли РезультатОбработки.Действие = "ВыбратьРежим" Тогда
		...
		ОткрытьФорму("РегистрСведений.фин_ПравилаСценарногоПланированияСтатейБюджетов.Форма.ФормаВводаРасчетныхДанных",Новый Структура("Адрес,Бюджет",Адрес,Бюджет),ЭтотОбъект,УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно,,ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработкаВыбораРежимаНаСервере(Результат)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НаборЗаписей = РегистрыСведений.фин_ПравилаСценарногоПланированияСтатейБюджетов.СоздатьНаборЗаписей();
