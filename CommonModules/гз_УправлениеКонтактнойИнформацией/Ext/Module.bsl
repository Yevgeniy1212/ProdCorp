...
//
Процедура мПрочитатьКонтактнуюИнформацию(НаборЗаписей, Ссылка) Экспорт
	...
	Запрос.УстановитьПараметр("ВидОбъектаКонтактнойИнформации", мВидОбъектаКИ(Ссылка));
...
КонецПроцедуры // мПрочитатьКонтактнуюИнформацию()
...
//  Отказ - Булево
//
Процедура мЗаписатьКонтактнуюИнформацию(НаборЗаписей, Ссылка, Отказ) Экспорт
	...
	Исключение
		гз_ОбщегоНазначения.мСообщитьОбОшибке(ОписаниеОшибки(),, ("Элемент """ + СокрЛП(Ссылка) + """ не записан. Не записана контактная информация."));
		Отказ = Истина;
		Возврат;
	КонецПопытки;
	
	мПрочитатьКонтактнуюИнформацию(НаборЗаписей, Ссылка)
	
КонецПроцедуры // мЗаписатьКонтактнуюИнформацию()
...
//
Процедура мКонтактнаяИнформацияПриАктивизацииСтрокиТаблицы(Элемент, КнопкаУстановитьОсновным) Экспорт
...
КонецПроцедуры // мКонтактнаяИнформацияПриАктивизацииСтрокиТаблицы()
...
//
Процедура мКонтактнаяИнформацияСписокПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		Если мПолучитьмПредставлениеАдреса(ДанныеСтроки) = ДанныеСтроки.Представление Тогда
			// Формат МНС
			Если НЕ ЗначениеЗаполнено(ДанныеСтроки.Представление) Тогда
...
КонецПроцедуры // мКонтактнаяИнформацияСписокПриВыводеСтроки()
...
//
Процедура мУстановитьЗаписьОсновной(НаборЗаписей, ТабличноеПоле, Кнопка) Экспорт
...
КонецПроцедуры // мУстановитьЗаписьОсновной()
...
//  Отказ - Булево
//
Процедура мУдалитьЗаписьКонтактнойИнформации(Элемент, Отказ) Экспорт
...
КонецПроцедуры // мУдалитьЗаписьКонтактнойИнформации()
...
//
Функция мПолучитьСтруктуруЗаписиРегистра(Запись) Экспорт
...
КонецФункции // мПолучитьСтруктуруЗаписиРегистра()

// Процедура контролирует возможность редактирования текста в поле ввода представления КИ.
//
Процедура мУстановитьВозможностьРедактированияТекстаКИ(Элемент) Экспорт

	Если Элемент.ТекущиеДанные.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес
	   И ЗначениеЗаполнено(Элемент.ТекущиеДанные.Представление)
	   И (мПолучитьмПредставлениеАдреса(Элемент.ТекущиеДанные) = Элемент.ТекущиеДанные.Представление) Тогда
...
КонецПроцедуры // мУстановитьВозможностьРедактированияТекстаКИ()

// Обработчик события "ПриНачалеРедактирования" табличного поля
// набора записей регистра сведений
//
Процедура мКонтактнаяИнформацияПриНачалеРедактированияОбщая(Элемент, НоваяСтрока, ПоследнееЗначениеЭлементаТайпингаВидаКИ) Экспорт
...
// набора записей регистра сведений
//
Процедура мКонтактнаяИнформацияПередОкончаниемРедактированияОбщая(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ, мТекстТайпингаВидаКИ, мОбработкаТайпингаВидаКИ) Экспорт
...
// набора записей регистра сведений
//
Процедура мКонтактнаяИнформацияПредставлениеПриИзмененииОбщая(Элемент, ТабличноеПоле) Экспорт

	Если ТабличноеПоле.ТекущиеДанные.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
		
		СтруктураПолей = мРазложитьТелефонПоПолям(Элемент.Значение);
		...
		мСформироватьПредставлениеТелефона(ТабличноеПоле.ТекущиеДанные);
		
	КонецЕсли; 
	
КонецПроцедуры // мКонтактнаяИнформацияПредставлениеПриИзмененииОбщая()

// Обработчик события "ПередНачаломДобавления" табличного поля
// набора записей регистра сведений
//
Процедура мКонтактнаяИнформацияПередНачаломДобавленияОбщее(Элемент, Отказ, Копирование, мКнопкаРедактироватьКИВДиалоге, ТабличноеПоле, 
	...
	Иначе
		Если мКнопкаРедактироватьКИВДиалоге.Пометка Тогда
			...
			ОбработкаРедактирования = Обработки.РедактированиеКонтактнойИнформации.Создать();
			ОбработкаРедактирования.ДоступностьОбъекта = ДоступностьОбъекта;
			ОбработкаРедактирования.РедактироватьЗапись(НаборКонтактнойИнформации,, Форма, мПолучитьСтруктуруЗаписиРегистра(ТабличноеПоле.ТекущиеДанные));
		Иначе
			мУстановитьВозможностьРедактированияТекстаКИ(ТабличноеПоле);
		КонецЕсли;
	КонецЕсли; 
...
// набора записей регистра сведений
//
Процедура мКонтактнаяИнформацияПередНачаломИзмененияОбщее(Элемент, Отказ, мКнопкаРедактироватьКИВДиалоге, ДоступностьОбъекта = Истина, Форма = Неопределено) Экспорт
	...
	Иначе
		мУстановитьВозможностьРедактированияТекстаКИ(Элемент);
	КонецЕсли; 
...
// набора записей регистра сведений
//
Процедура мКонтактнаяИнформацияТипПриИзмененииОбщее(Элемент, ТабличноеПоле) Экспорт
...
//
Функция мПроверкаПустойСтроки(ВыбСтрока, ПризнакЗапятой=Истина)
...
КонецФункции // мПроверкаПустойСтроки()
...
//   Строка
//
Функция мПолучитьмПредставлениеАдреса(Запись, ПроизвольныйАдрес = Ложь) Экспорт
...
//
Процедура мСформироватьПредставлениеТелефона(НаборПолей) Экспорт

	НаборПолей.Представление = НаборПолей.Поле1;
	НаборПолей.Представление = НаборПолей.Представление + ?((Не ПустаяСтрока(НаборПолей.Поле2)),(мПроверкаПустойСтроки(НаборПолей.Представление, Ложь)+"(" + НаборПолей.Поле2 + ")"),"");
	НаборПолей.Представление = НаборПолей.Представление + ?((Не ПустаяСтрока(НаборПолей.Поле3)),(мПроверкаПустойСтроки(НаборПолей.Представление, ПустаяСтрока(НаборПолей.Поле2)) + мПривестиНомерТелефонаКШаблону(НаборПолей.Поле3)),"");
	
	Если НЕ ПустаяСтрока(НаборПолей.Представление) Тогда
		НаборПолей.Представление = НаборПолей.Представление + ?((Не ПустаяСтрока(НаборПолей.Поле4)),(мПроверкаПустойСтроки(НаборПолей.Представление) + "доб. " + мПривестиНомерТелефонаКШаблону(НаборПолей.Поле4)),"");
	Иначе
		НаборПолей.Представление = мПривестиНомерТелефонаКШаблону(НаборПолей.Поле4);
	КонецЕсли; 
...
//
Функция мСформироватьмПредставлениеАдресаЗаПределамиРК(Страна, Представление) Экспорт

	ТекущеемПредставлениеАдреса = Представление;	
	...
	Если ЗначениеЗаполнено(Страна) И Страна <> Справочники.КлассификаторСтранМира.Казахстан.Наименование Тогда
		...
		Если ПозицияСтраныВАдресе <> 1 Тогда
			ТекущеемПредставлениеАдреса = Страна + ?(ЗначениеЗаполнено(Представление), (", " + Представление), "");
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат ТекущеемПредставлениеАдреса;
...
//
Функция мПолучитьПустоеЗначениеОбъектаКонтактнойИнформации(ВидОбъектаКонтактнойИнформации) Экспорт
...
КонецФункции // мПолучитьПустоеЗначениеОбъектаКонтактнойИнформации()
...
Функция мВидОбъектаКИ(ОбъектСсылка) Экспорт
...
//
Функция мНачалоВыбораОбъектаКИ(ФормаЗаписи, Элемент, ТекущийПользователь, ТолькоВнешниеОбъекты = Ложь) Экспорт
...
//
Функция мОтделитьСокращение(Знач СтрокаЭлемента) экспорт
	...
	Если ПозицияРазделителя > 0 Тогда
		...
		Если мЯвляетсяАдреснымСокращением(ПервоеСлово) Тогда
	...
	Если мЯвляетсяАдреснымСокращением(ПоследнееСлово) Тогда
...
КонецФункции // мОтделитьСокращение()
...
//
Функция мЯвляетсяАдреснымСокращением(знач СтрокаЭлемента)
...
КонецФункции // мЯвляетсяАдреснымСокращением()
...
// обратном случае
Функция мОбрезатьСокращение(Знач СтрокаЭлемента) Экспорт
...
КонецФункции // мОбрезатьСокращение()
...
Функция мПолучитьСписокСокращений(Знач Название, Уровень) Экспорт
	...
	Если мОбрезатьСокращение(Название) <> Название Тогда // было что обрезать
...
КонецФункции // мПолучитьСписокСокращений()
...
//
Функция мПолучитьНазвание(АдресныйЭлемент) Экспорт
...
КонецФункции // мПолучитьНазвание()
...
//
Функция мПолучитьПолноеНазвание(Знач КодЭлемента) Экспорт

	КодОбласти = Цел(КодЭлемента / мМаскаОбласти());
	КодЭлемента = КодЭлемента % мМаскаОбласти();

	КодРайона = Цел(КодЭлемента / мМаскаРайона());
	КодЭлемента = КодЭлемента % мМаскаРайона();
	...
	Если КодОбласти > 0 Тогда
		НовыйКод = КодОбласти * мМаскаОбласти();
		Название = Название + ", " + мПолучитьНазвание(мПолучитьСтруктуруАдресногоЭлемента(НовыйКод));
	КонецЕсли;

	Если КодРайона > 0 Тогда
		НовыйКод = НовыйКод + КодРайона * мМаскаРайона();
		Название = Название + ", " + мПолучитьНазвание(мПолучитьСтруктуруАдресногоЭлемента(НовыйКод));
	КонецЕсли;
...
КонецФункции // мПолучитьПолноеНазвание()
...
// адресного элемента
//
Функция мПолучитьМаскуПоТипу(ТипАдресногоЭлемента) Экспорт

	Если ТипАдресногоЭлемента = 1 Тогда
		Возврат мМаскаОбласти();

	ИначеЕсли ТипАдресногоЭлемента = 2 Тогда
		Возврат мМаскаРайона();

	ИначеЕсли ТипАдресногоЭлемента = 3 Тогда
		Возврат мМаскаГорода();
	Иначе
		Возврат мМаскаСела();
	КонецЕсли;

КонецФункции // мПолучитьМаскуПоТипу()
...
Функция мПолучитьТипАдресногоЭлемента(КодОбласти, КодРайона = 0, КодОкруга =0 , КодНасПункта = 0, ТипЭлемента) Экспорт
...
КонецФункции // мПолучитьТипАдресногоЭлемента()

// Функция возвращает строку адресного элемента
//
Функция мПолучитьСтруктуруАдресногоЭлемента(КодЭлемента) Экспорт
	...
	Иначе 
		Возврат мПолучитьПустуюСтруктуруАдреса();
		
	КонецЕсли;
	
КонецФункции // мПолучитьСтруктуруАдресногоЭлемента()

// Возвращает код области из адрессного классификатора по его Названию
//
Функция мКодОбластиПоНазванию(Название) Экспорт
...
КонецФункции // мКодОбластиПоНазванию()
...
// адресного элемента
//
Функция мПолучитьМаску(АдресныйЭлемент) Экспорт

	Возврат мПолучитьМаскуПоТипу(АдресныйЭлемент.ТипАдресногоЭлемента);

КонецФункции // мПолучитьМаску()
...
//  АдресныйКлассификатор с пустым набором значений
//
Функция мПолучитьПустуюСтруктуруАдреса() Экспорт
...
КонецФункции // мПолучитьПустуюСтруктуруАдреса()
...
//
Функция мПолучитьАдресныйЭлемент(НазваниеЭлемента, ТипЭлемента, КодЭлементаРодителя = 0) Экспорт
	...
	Если КодЭлементаРодителя > 0 Тогда // проверка на соответствие подчинению родителю
		СтруктураАдресаРодителя = мПолучитьСтруктуруАдресногоЭлемента(КодЭлементаРодителя);
		МаскаРодителя = мПолучитьМаскуПоТипу(СтруктураАдресаРодителя.ТипАдресногоЭлемента);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Возврат мПолучитьСтруктуруАдресногоЭлемента(Выборка.Код);
		
	КонецЦикла;
	
	Возврат мПолучитьПустуюСтруктуруАдреса();
	
КонецФункции // мПолучитьАдресныйЭлемент()
...
//  Ложь   - если не входит
//
Функция мВходитВИнтервал(Знач Номер, Интервал) Экспорт

	Если ПустаяСтрока("" + Номер) Тогда
		Возврат Ложь;
	КонецЕсли;

	Если НЕ гз_ОбщегоНазначения.мТолькоЦифрыВСтроке(Номер, Ложь) Тогда
		Возврат СтрЗаменить("" + Номер, " ", "") = СтрЗаменить("" + Интервал, " ", "");
	КонецЕсли;

	Номер = Число(Номер);

	ТолькоЧислоИнтервал = гз_ОбщегоНазначения.мТолькоЦифрыВСтроке(Интервал, Ложь);
	...
	Иначе
		Если гз_ОбщегоНазначения.мТолькоЦифрыВСтроке(Интервал, Ложь) Тогда
			Если Номер = Число(Интервал) Тогда
				Попали = 1;
			КонецЕсли;
		КонецЕсли;
...
//  Ложь   - если не входит
//
Функция мНомерДомаВИнтервале(Номер, Знач Интервал) Экспорт
	
	Пока НЕ ПустаяСтрока(Интервал) Цикл

		Позиция = Найти(Интервал, ",");
		Если Позиция = 0 Тогда
			Возврат мВходитВИнтервал(Номер, Интервал);
		Иначе
			Если мВходитВИнтервал(Номер, СокрЛП(Лев(Интервал, Позиция - 1))) Тогда
				Возврат Истина;
			Иначе
...
КонецФункции // мНомерДомаВИнтервале()
...
//
Функция мПолучитьИндексПоУлицеДомуКорпусу(Улица, НомерДома, НомерКорпуса) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ПустаяСтрока(Выборка.Индекс) Тогда
			...
			Если (мНомерДомаВИнтервале(СокрЛП(НомерДома) + ?(НЕ ПустаяСтрока(НомерКорпуса), "К", "") + СокрЛП(НомерКорпуса), Интервал)) Тогда
				Возврат Выборка.Индекс;
				
			ИначеЕсли (мНомерДомаВИнтервале(СокрЛП(НомерДома) + ?(НЕ ПустаяСтрока(НомерКорпуса), "/", "") + СокрЛП(НомерКорпуса), Интервал)) Тогда
				Возврат Выборка.Индекс;
				
			ИначеЕсли (мНомерДомаВИнтервале(СокрЛП(НомерДома) + ?(НЕ ПустаяСтрока(НомерКорпуса), "стр", "") + СокрЛП(НомерКорпуса), Интервал)) Тогда
				Возврат Выборка.Индекс;
...
КонецФункции // мПолучитьИндексПоУлицеДомуКорпусу()
...
//  Строка - шестизначный почтовый индекс
//
Функция мПолучитьИндекс(Знач НазваниеОбласти, Знач НазваниеРайона, Знач НазваниеГорода, Знач НазваниеНаселенногоПункта, Знач НазваниеУлицы, Знач НомерДома, Знач НомерКорпуса) Экспорт

	РодительИндекса = мПолучитьПустуюСтруктуруАдреса();
	Индекс = "";
	
	Область = мПолучитьАдресныйЭлемент(мОбрезатьСокращение(НазваниеОбласти), 1);
	...
	Район = мПолучитьАдресныйЭлемент(мОбрезатьСокращение(НазваниеРайона), 2, РодительИндекса.Код);
	...
	Город = мПолучитьАдресныйЭлемент(мОбрезатьСокращение(НазваниеГорода), 3, РодительИндекса.Код);
	...
	НаселенныйПункт = мПолучитьАдресныйЭлемент(мОбрезатьСокращение(НазваниеНаселенногоПункта), 4, РодительИндекса.Код);
	...
	Улица = мПолучитьАдресныйЭлемент(мОбрезатьСокращение(НазваниеУлицы), 5, РодительИндекса.Код);
	Если Улица.Код > 0 Тогда
		ИндексУлицы = мПолучитьИндексПоУлицеДомуКорпусу(Улица, НомерДома, НомерКорпуса);
...
//
Функция мВернутьСтрокуАдресногоКлассификатораПоАдреснымЭлементам(Знач НазваниеОбласти, Знач НазваниеРайона, Знач НазваниеНаселенногоПункта) Экспорт
	...
	Район = мПолучитьАдресныйЭлемент(мОбрезатьСокращение(НазваниеРайона), 2, КодРодителяРодителя);
	Если Район.Код > 0 Тогда
		РодительскийЭлемент = Район;
		КодРодителяРодителя = Район.Код;
	КонецЕсли;
	
	НаселенныйПункт = мПолучитьАдресныйЭлемент(мОбрезатьСокращение(НазваниеНаселенногоПункта), 3, КодРодителяРодителя);
	Если НаселенныйПункт.Код > 0 Тогда
...
//  Число - маска с единица в значимых разрядах и нулями в остальных
//
Функция мМаскаНасПункта() Экспорт
	Возврат 1;
КонецФункции
...
//
Функция мПолучитьАдресИзКонтактнойИнформации(Объект, ТипАдреса = "") Экспорт
...
КонецФункции // мПолучитьАдресИзКонтактнойИнформации()
...
//
Функция мПолучитьТелефонИзКонтактнойИнформации(Объект) Экспорт
...
КонецФункции // мПолучитьТелефонИзКонтактнойИнформации()
...
//   Список значений
//
Функция мРазложитьТелефонПоПолям(НомерТелефона) Экспорт
	...
	СтруктураПолейТелефона = Новый Структура("КодСтраны,КодГорода,СамТелефон", КодСтраны, КодГорода, мПривестиНомерТелефонаКШаблону(СамТелефон));
	
	Возврат СтруктураПолейТелефона;

КонецФункции // мРазложитьТелефонПоПолям()
...
//
Функция мПривестиНомерТелефонаКШаблону(НомерТЛФ) Экспорт
...
КонецФункции // мПривестиНомерТелефонаКШаблону()
...
//
Процедура мПередНачаломДобавленияКонтактногоЛица(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, 
...
КонецПроцедуры // мПередНачаломДобавленияКонтактногоЛица()
...
Функция мРазложитьАдрес(Знач СтрокаАдрес) Экспорт
	...
	МассивАдрес	=	гз_ОбщегоНазначения.мРазложитьСтрокуВМассивПодстрок(СтрокаАдрес);
...
Функция мСобратьАдрес(Знач СтруктураАдрес) Экспорт
...
//
Функция мПредставлениеАдреса(Знач АдреснаяСтрока, Способ = 0, ПредставлениеПустого = "<<Адрес не задан>>") Экспорт

	Если НЕ ЗначениеЗаполнено(СтрЗаменить(АдреснаяСтрока,",","")) Тогда
		Возврат ПредставлениеПустого;
	КонецЕсли;

	СтруктураАдрес = мРазложитьАдрес(АдреснаяСтрока);
	Адрес = "";
	...
	Возврат Адрес;
КонецФункции	// глмПредставлениеАдреса
...
Процедура мОткрытьФормуВыбораВидаКИ(РежимВыбора, ВладелецФормы, ЗначениеОтбораПоТипу = Неопределено,
...
КонецПроцедуры // мОткрытьФормуВыбораВидаКИ()
