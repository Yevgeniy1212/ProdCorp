...
#Если ВебКлиент Тогда
	...
	Если общ_ОбщегоНазначенияКлиент.ПредлагатьУстановкуРасширенияРаботыСФайлами() = Неопределено Тогда
		
		ПервоеОбращениеЗаСеанс = Истина;
		общ_ОбщегоНазначенияКлиент.УстановитьПредлагатьУстановкуРасширенияРаботыСФайлами(ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить(
			"НастройкиПрограммы/ПредлагатьУстановкуРасширенияРаботыСФайлами", ИдентификаторКлиента));
		Если общ_ОбщегоНазначенияКлиент.ПредлагатьУстановкуРасширенияРаботыСФайлами() = Неопределено Тогда
			общ_ОбщегоНазначенияКлиент.УстановитьПредлагатьУстановкуРасширенияРаботыСФайлами(Истина);
	...
	Если общ_ОбщегоНазначенияКлиент.ПредлагатьУстановкуРасширенияРаботыСФайлами() = Ложь Тогда
	...
	Если ПервоеОбращениеЗаСеанс Тогда
		...
		общ_ОбщегоНазначенияКлиент.УстановитьПредлагатьУстановкуРасширенияРаботыСФайлами(КодВозврата);
		ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить(
			"НастройкиПрограммы/ПредлагатьУстановкуРасширенияРаботыСФайлами", ИдентификаторКлиента,
			общ_ОбщегоНазначенияКлиент.ПредлагатьУстановкуРасширенияРаботыСФайлами());
	КонецЕсли;
...
Процедура ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытия, ПредставлениеУровня = "Информация", 
	...
	УстановитьСообщенияДляЖурналаРегистрации(СообщенияДляЖурналаРегистрации);
КонецПроцедуры

Процедура УстановитьСообщенияДляЖурналаРегистрации(Значение)
	Если глХранилищеЗначений = Неопределено ИЛИ ТипЗнч(глХранилищеЗначений)<>Тип("Структура") Тогда
		глХранилищеЗначений = Новый Структура;
	КонецЕсли;
	глХранилищеЗначений.Вставить("СообщенияДляЖурналаРегистрации",Значение);
КонецПроцедуры

Функция СообщенияДляЖурналаРегистрации()
	Если глХранилищеЗначений = Неопределено ИЛИ ТипЗнч(глХранилищеЗначений)<>Тип("Структура") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если НЕ глХранилищеЗначений.Свойство("СообщенияДляЖурналаРегистрации") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат глХранилищеЗначений.СообщенияДляЖурналаРегистрации;
КонецФункции
