...
Функция НовыйОтборФормыВыбораЭСФ(Знач СсылкаСчетФактураПолученный) Экспорт
	...
	// Адаптировано для обычных форм
	Если ЗначениеЗаполнено(СсылкаСчетФактураПолученный.Контрагент) Тогда
		Отбор.Вставить("Контрагент", ЭСФСерверПереопределяемый.ЗначениеРеквизитаОбъекта(СсылкаСчетФактураПолученный, ЭСФКлиентСерверПереопределяемый.ИмяРеквизитаПоставщикаСчетаФактуры()));
	КонецЕсли;
...
Функция ПараметрыПолученияТикета(Знач ИмяСервиса) Экспорт
	...
	Возврат ЭСФСервер.ДанныеАутентификацииПользователяИнтернетПоддержки();
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(СообщениеОбОшибке, Данные = Неопределено) Экспорт
	...
	ЭСФСервер.ЗаписатьОшибкуВЖурналРегистрации(СообщениеОбОшибке, Данные);
