...
Процедура ПриСозданииНаСервере(Форма) Экспорт
	...
	Если РольДоступна("ЧтениеВерсийОбъектов") И ПолучитьФункциональнуюОпцию("ИспользоватьВерсионированиеОбъектов") Тогда
		ИмяФормыМассив = куфиб_ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(Форма.ИмяФормы, ".");
...
//
Процедура СоздатьВерсиюОбъекта(Объект, АвторВерсии = Неопределено, Знач ТипВерсииОбъекта = Неопределено, Комментарий = "", УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если Ссылка.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(УникальныйИдентификаторСтрокой) Тогда
			Ссылка = фин_ОбщегоНазначенияСервер.МенеджерОбъектаПоСсылке(Ссылка).ПолучитьСсылку(Новый УникальныйИдентификатор(УникальныйИдентификаторСтрокой))
		КонецЕсли;
	КонецЕсли;
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Для Каждого ЭлементСвойств Из Объект.ДополнительныеСвойства Цикл
			ДополнительныеСвойства.Вставить(ЭлементСвойств.Ключ,ЭлементСвойств.Значение);
		КонецЦикла;
		общ_ПривилегированныеПроцедуры.ЗаписатьВерсию(Ссылка,ХранилищеДанных,Число(НомерПоследнейВерсии) + 1,АвторВерсии,ТипВерсииОбъекта,Комментарий,ДополнительныйКомментарий,ДополнительныеСвойства)		
	#Иначе
		...
		Иначе
			...
			МенеджерЗаписиВерсииОбъектов.АвторВерсии	  = ?(ЗначениеЗаполнено(АвторВерсии), АвторВерсии, фин_ОбщегоНазначенияВызовСервераПовтИсп.ЗначениеПеременной("ТекущийПользователь"));
...
Функция ОбъектВерсионируется(знач Источник, НомерПоследнейВерсии) Экспорт
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Возврат общ_ПривилегированныеПроцедуры.ОбъектВерсионируется(Новый Структура("Ссылка,Проведен",Источник.Ссылка,?(Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(Источник.Ссылка)),Ложь,Источник.Проведен)), НомерПоследнейВерсии,Источник.Метаданные().ПолноеИмя());
	...
	ВариантВерсионирования = общ_ПривилегированныеПроцедуры.ВариантВерсионированияОбъекта(Источник.Метаданные().ПолноеИмя());
			...
			ИначеЕсли УровеньЧтения = 2 Тогда // указатель на втором уровне - это реквизит или имя табличной части
				...
				Если Не ЗначениеЗаполнено(НовоеЗР.Тип) Тогда
					...
					Если ОписаниеРеквизита = Неопределено Тогда
						...
						Если куфиб_ОбщегоНазначения.ЭтоСтандартныйРеквизит(МетаданныеОбъекта.СтандартныеРеквизиты, НаименованиеРеквизита) Тогда
...
Процедура СформироватьОтчетПоПереданнойВерсии(ТЧОтчета, СериализованныйXML, СсылкаНаОбъект)
	...
	Если СсылкаНаОбъект.Метаданные().Макеты.Найти("МакетОбъекта") <> Неопределено Тогда
		Макет = фин_ОбщегоНазначенияСервер.МенеджерОбъектаПоСсылке(СсылкаНаОбъект).ПолучитьМакет("МакетОбъекта");
	Иначе
		Макет = Неопределено;
	КонецЕсли;
