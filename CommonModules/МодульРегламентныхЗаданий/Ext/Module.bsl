...
//Процедура обновляет индексы полнотекстового поиска, при условии что у базы
//разрешен режим полнотекствого поиска
//
...
//Процедура обновляет индексы полнотекстового поиска с их слиянием , при условии что у базы
//разрешен режим полнотекствого поиска
//    
...
//Процедура устанавливает дату актуальности учета, перепроводя документы на установленную дату
//
...
//Функция получает набор записей регистра 
//
...
Процедура ОбновлениеКонфигурации() Экспорт
	...
	Иначе
		...
		ком = Новый COMОбъект(ОбщегоНазначения.ИмяCOMСоединителя());
	...
	//Установим нужную версию платформы
	КонечныйТекст = СтрЗаменить(КонечныйТекст, "v81", ОбщегоНазначения.ВерсияПлатформыИнформационнойБазы());
