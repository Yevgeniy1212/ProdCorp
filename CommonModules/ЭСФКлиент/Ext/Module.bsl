	...
	Если ЭСФВызовСервера.ИспользоватьОткрытиеСессииСПодписью() Тогда
		...
		Если ТипЗнч(ДанныеПрофилейИСЭСФ) = Тип("Массив") Тогда
			...
			Для каждого ДанныеПрофиля Из ДанныеПрофилейИСЭСФ Цикл
				...
				ИдентификаторСессии = ОткрытьСессиюСПодписьюПользователя(ДанныеПрофиля, ПараметрыОткрытияСессии);
		...
		ИначеЕсли ТипЗнч(ДанныеПрофилейИСЭСФ) = Тип("Соответствие") Тогда
			Для каждого ДанныеПрофиля Из ДанныеПрофилейИСЭСФ Цикл
				ИдентификаторСессии = ОткрытьСессиюСПодписьюПользователя(ДанныеПрофиля.Значение.ПрофильИСЭСФ, ПараметрыОткрытияСессии);
				Если ИдентификаторСессии = Неопределено Тогда
					ТекстСообщения = нСтр("ru='При открытии сессии возникла ошибка.'", "ru") ;
					ЭСФКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
					Возврат Ложь;
...
Функция ОткрытьСессиюСПодписьюПользователя(ПрофильИСЭСФ, ПараметрыОткрытияСессии = Неопределено) Экспорт
...
//Функция открытия сессии с подписью авторизационного тикета.
//Выполнена только на клиенте так как подпись через NCA Layer с которым обмен может идти только на клиенте
Функция ОткрытьСессиюСПодписью(ПрофильИСЭСФ, ПараметрыОткрытияСессии = Неопределено) Экспорт
	...
	Если ЭСФВызовСервера.ИспользоватьОткрытиеСессииСПодписью() Тогда
		...
		ПараметрыПодписи = ЭлектронноеВзаимодействиеССервисамиВызовСервера.ПолучитьПараметрыКомпонентыЭСФ(БИН, Ложь, "", Истина, ДанныеПрофиляИСЭСФ.СтруктурнаяЕдиница.Ссылка);
		...
		Если ПодписьВФорматеJSON <> "" Тогда
			ОтветСокетаJson = ЭСФВызовСервера.ЭСФ_ПрочитатьЗначениеJSON(ПодписьВФорматеJSON);
			
			
			Если ОтветСокетаJson.Свойство("Code") и ОтветСокетаJson.code = "200" Тогда
				РезультатПодписиXML = ОтветСокетаJson.responseObject;
				Если ПараметрыОткрытияСессии.ИмяМодуля = "ESF" Тогда
			...
			Иначе
				...
				ЭСФКлиентСервер.СообщитьПользователю(ОбработатьСообщениеОбОшибке(ОтветСокетаJson));
			КонецЕсли;
		Иначе
			ЭСФКлиентСервер.СообщитьПользователю(ОбработатьСообщениеОбОшибке(Неопределено));
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьЭЦПXML(Знач ДанныеДляПодписиJSON, БИН, Сервис, ДополнительныеПараметры = Неопределено, ПредставлениеОрганизации = "", СлужебныеДанные = "") Экспорт
	...
	ПродолжитьВыборКлюча = ОткрытьФормуБлокировкиИнтерфейса("Выберите ключ для аутентификации (AUTH)", ПредставлениеОрганизации);
	Если Не ПродолжитьВыборКлюча Тогда
		Возврат ""
	КонецЕсли;
	...
	Попытка
		...
		Если ПустаяСтрока(ОтветСокета) Тогда
			ЭСФКлиентСервер.СообщитьПользователю("При формировании xml подписи возникла ошибка: " + Криптопровайдер.ТекстОшибки);
		КонецЕсли;
...
Функция СоздатьЭЦПXMLs(Знач ДанныеДляПодписиJSON, БИН, Сервис, ДополнительныеПараметры = Неопределено, ПредставлениеОрганизации = "", СлужебныеДанные = "") Экспорт
	...
	Попытка
		...
		СлужебныеДанные = КриптоПровайдер.ServiceDataTicket;
		Если ПустаяСтрока(ОтветСокета) Тогда
			ЭСФКлиентСервер.СообщитьПользователю("При формировании xml подписи пакета возникла ошибка: " + Криптопровайдер.ТекстОшибки);	
		КонецЕсли;
...
Функция СоздатьЭЦПСырая(Знач ДанныеДляПодписиJSON, БИН, Сервис, ДополнительныеПараметры = Неопределено, ПредставлениеОрганизации = "", СлужебныеДанные = "") Экспорт
	...
	ПродолжитьВыборКлюча = ОткрытьФормуБлокировкиИнтерфейса("Выберите ключ для подписи (RSA или GOSTKNCA)", ПредставлениеОрганизации);
	Если Не ПродолжитьВыборКлюча Тогда
		Возврат ""
	КонецЕсли;
...
Функция ОткрытьФормуБлокировкиИнтерфейса(ТекстСообщения = "", ПредставлениеОрганизации = "")
	...
	//переопределено для Толстого клиента
	РезультатВызова = ОткрытьФормуМодально("Обработка.ОбменЭСФ.Форма.ФормаВыполненияОпераций", ПараметрыФормы);
	Если РезультатВызова <> КодВозвратаДиалога.Отмена И РезультатВызова <> Неопределено Тогда
...
Функция ПолучитьДанныеСертификатаЧерезНЦА(БИН, Сервис, ДополнительныеПараметры = "") Экспорт
	...
	ПродолжитьВыборКлюча = ОткрытьФормуБлокировкиИнтерфейса("Выберите ключ");
	Если Не ПродолжитьВыборКлюча Тогда
...
Функция НоваяКоллекцияПодписейЭСФ2(Знач КоллекцияSignedContent, Знач ДанныеКлючаЭЦП, ДанныеПрофиляИСЭСФ) Экспорт
	...
	Для Каждого Элемент Из КоллекцияSignedContent Цикл
		...
		ИначеЕсли ТипЗнч(КоллекцияSignedContent) = Тип("Массив") Тогда
			Base64ДокументДляПодписи = Base64Строка(ПолучитьДвоичныеДанныеИзСтроки(Элемент, КодировкаТекста.UTF8, Ложь));
		КонецЕсли;
	...
	ИмяСервиса = "1CService";
	БИН = ДанныеПрофиляИСЭСФ.СтруктурнаяЕдиница.ИдентификационныйНомер;
	БИНКлюча = ДанныеПрофиляИСЭСФ.ПользовательИСЭСФ.ИмяАутентификации;
	ПредставлениеОрганизации = ДанныеПрофиляИСЭСФ.СтруктурнаяЕдиница.Представление;
	ПараметрыПодписи = ЭлектронноеВзаимодействиеССервисамиВызовСервера.ПолучитьПараметрыКомпонентыЭСФ(БИН, Ложь, , ,ДанныеПрофиляИСЭСФ.СтруктурнаяЕдиница.Ссылка);
	...
	Если ПодписьВФорматеJSON <> "" Тогда
		...
		Если ОтветСокетаJson.Свойство("Code") и ОтветСокетаJson.code = "200" Тогда
			...
			Если КоллекцияSignedContent.Количество() > 1 Тогда 
				НомерЭлемента = 0;
				МассивПодписей = ЭСФВызовСервера.ЭСФ_ПрочитатьЗначениеJSON(ОтветСокетаJson.responseObject.signResult);
				Для Каждого Элемент Из КоллекцияSignedContent Цикл
					
					Если ТипЗнч(КоллекцияSignedContent) = Тип("Соответствие") Тогда
						КоллекцияПодписейЭСФ.Вставить(Элемент.Ключ, МассивПодписей[НомерЭлемента]);
					ИначеЕсли ТипЗнч(КоллекцияSignedContent) = Тип("Массив") Тогда
						КоллекцияПодписейЭСФ.Вставить(Элемент, МассивПодписей[НомерЭлемента]);
					КонецЕсли;
					
					СертификатПодписавшего = ОтветСокетаJson.responseObject.signerCert;
			...
			ИначеЕсли КоллекцияSignedContent.Количество() = 1 Тогда
				Для Каждого Элемент Из КоллекцияSignedContent Цикл
					...
					Если ТипЗнч(КоллекцияSignedContent) = Тип("Соответствие") Тогда
						КоллекцияПодписейЭСФ.Вставить(Элемент.Ключ, ОтветСокетаJson.responseObject.signResult);
					ИначеЕсли ТипЗнч(КоллекцияSignedContent) = Тип("Массив") Тогда
						КоллекцияПодписейЭСФ.Вставить(Элемент, ОтветСокетаJson.responseObject.signResult);
					КонецЕсли;
		...
		Иначе
			// Возврат ошибки от ВК
			ЭСФКлиентСервер.СообщитьПользователю(ЭСФКлиент.ОбработатьСообщениеОбОшибке(ОтветСокетаJson));
		КонецЕсли;
		
	Иначе
		ЭСФКлиентСервер.СообщитьПользователю(ЭСФКлиент.ОбработатьСообщениеОбОшибке(Неопределено));
	КонецЕсли;
...
Функция РазобратьПоУзламKeyInfo(KeyInfo, Узел = Неопределено)
	...
	МассивKeyInfo = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(KeyInfo, ","); // Так как разделитель между тэгами запятая, так понимаем что начался новый тэг
	...
	Для каждого Узел Из МассивKeyInfo Цикл
		...
		Если Поз <> 0 Тогда
			...
			Если Ключ = "pem" Тогда
				...
				Значение = Сред(Значение,30,СтрДлина(Узел)-64);
			КонецЕсли; 
...
Функция РазобратьСтрокуСертификата(СоставнаяСтрокаДанных) Экспорт
	...
	//Данные разделены запятой всегда
	МассивПолей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СоставнаяСтрокаДанных, ","); 
