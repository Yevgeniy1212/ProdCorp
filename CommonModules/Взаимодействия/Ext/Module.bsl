...
Процедура ДобавитьПараметрыРаботыКлиента(Параметры) Экспорт
	...
	//куфиб - Параметры.Вставить("ИспользоватьПочтовыйКлиент", ПолучитьФункциональнуюОпцию("ИспользоватьПочтовыйКлиент"));
...
Функция СформироватьТекстЗапросаДляПоискаПоEmail(ПоискПоСписку,ИтогиПоEmail = Ложь) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ПОДСТРОКА(КонтактнаяИнформация.Представление, 0, 250) КАК Адрес,
				   |	ПОДСТРОКА(КонтактнаяИнформация.Представление, 0, 250) КАК Представление,
	               ...
	               |ГДЕ
	               |	ПОДСТРОКА(КонтактнаяИнформация.Представление, 0, 250) = &Адрес
...
Функция ПолучитьАдресаЭлектроннойПочтыДляМассиваКонтактов(МассивКонтактов,Группа = "") Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
	               |ГДЕ
	               |	КонтактнаяИнформация.Объект В(&МассивКонтактов)
				   |	И (КонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))";
	...
	Для каждого СтрокаТаблицы Из ТаблицаРезультат Цикл
		СтрокаТаблицы.СписокАдресов = СтрокаТаблицы.СписокАдресов + ?(ПустаяСтрока(СтрокаТаблицы.СписокАдресов), "", ";") + СтрокаТаблицы.Адрес;
	КонецЦикла;
...
Процедура ИнициализироватьФормуСпискаВзаимодействий(Форма) Экспорт
	...
	Форма.ТолькоПочта = Истина; //НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПрочиеВзаимодействия");
...
Процедура ОбработатьНеобходимостьОтображенияГруппПользователей(Форма) Экспорт
	...
	Форма.ИспользоватьГруппыПользователей = Ложь;// ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей");
