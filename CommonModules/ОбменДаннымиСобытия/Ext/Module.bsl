...
Процедура ОбменДаннымиПередЗаписьюУзлаПланаОбменаПередЗаписью(Источник, Отказ) Экспорт
	...
	Если ОбъектМодифицирован Тогда
		...
		УзелИнформационнойБазы = Источник.Ссылка;
		...
		Если ЭтотУзелОбмена = УзелИнформационнойБазы Тогда
			ОбщегоНазначения.СообщитьОбОшибке("В качестве узла обмена выбран узел, соответствующий текущей информационной базе.");
			Возврат;
		КонецЕсли;
...
Процедура ОпределитьМассивПолучателейПоНаборуЗаписей(МассивУзловРезультат, НаборЗаписей, ПРО, ОбъектМетаданных, ИмяПланаОбмена, ЭтоВерсияОбъектаДоИзменения)
	...
	Если Выгрузка = Истина Тогда
		...
		Пока ОбратныйИндекс >= 0 Цикл
			...
			ПРО_СтрокиНабора = ПравилоРегистрацииСтруктурой(ПРО, ПРО.Владелец().Колонки);
			...
			Иначе // ПРОП
				...
				// определяем массив узлов для регистрации объекта
				МассивУзлов = ОпределитьМассивУзловПоЗначениямСвойств(ЗначенияСвойствОбъекта, ПРО_СтрокиНабора, ИмяПланаОбмена, СтрокаНабора);
