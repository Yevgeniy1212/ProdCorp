...
Функция ПроверитьПараметрыТикета(Знач Тикет) Экспорт
	...
	Если ЗначениеЗаполнено(Тикет) Тогда
		Попытка
			Начало = Найти(Тикет,"<timeMark>");
			Конец = Найти(Тикет,"</timeMark>");
...
Процедура ОбновитьПредставлениеЭСФ(Форма, Знач СсылкаСчетФактура = Неопределено) Экспорт
	...
	Иначе // ЭтоСчетФактураПолученный
		...
		Если НЕ ЗначениеЗаполнено(Форма.РеквизитыЭСФ.СсылкаЭСФ) Тогда
			ДокументВыписанНаБумажномНосителе = ЭСФСерверПереопределяемый.ДокументВыписанНаБумажномНосителе(Форма, ИмяРеквизитаСпособОбмена);
			Если ДокументВыписанНаБумажномНосителе = Истина Тогда
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(СообщениеОбОшибке, Данные = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Интернет-поддержка пользователей.Ошибка'"),
...
Функция СохранитьДанныеАутентификации(ДанныеАутентификации) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ХранилищеДанныхАутентификацииИТС = Новый ХранилищеЗначения(ДанныеАутентификации, Новый СжатиеДанных(6));
	Константы.ДанныеАутентификацииИТС.Установить(ХранилищеДанныхАутентификацииИТС);
...
Функция ДанныеАутентификацииПользователяИнтернетПоддержки() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ХранилищеДанныхАутентификацииИТС = Константы.ДанныеАутентификацииИТС.Получить();
	
	ДанныеАутентификацииИТС = ХранилищеДанныхАутентификацииИТС.Получить();
...
Функция ТикетАутентификацииНаПорталеПоддержки(ВладелецТикета) Экспорт
	...
	НастройкиСоединения = ЭСФКлиентСервер.НастройкиСоединенияССерверами();
	...
	Иначе
		...
		РезультатПолученияТикета = ЭСФКлиентСервер.СлужебнаяТикетАутентификации(
