...
Процедура ВерсииФайловПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Не ТекущаяВерсия.Ссылка.Пустая() Тогда
		...
		РеквизитыФайла = фин_ОбщегоНазначенияСервер.ЗначенияРеквизитовОбъекта(ФайлСсылка, 
...
Функция ПолучитьПараметрыПриЗавершенииРаботы()
	...
	Если ТипЗнч(АвторизованныйПользователь) = Тип("СправочникСсылка.Пользователи") Тогда
		...
		ПараметрыПриЗавершении.Вставить("КоличествоЗанятыхФайлов", РаботаСФайламиСлужебныйВызовСервера.ПолучитьКоличествоЗанятыхФайлов(, ТекущийПользователь));
	Иначе
