...
Функция УстановитьЦветОбласти(ОбластьУстановки, ИндексТекущейГруппировки, КоличествоГруппировок)
	//ОбластьУстановки.Область().ЦветФона = Новый Цвет(255-ИндексТекущейГруппировки*20, 255-ИндексТекущейГруппировки*10, 255-ИндексТекущейГруппировки*5);
	Попытка
		Если ИндексТекущейГруппировки = КоличествоГруппировок Тогда
			ОбластьУстановки.Область().ЦветФона = Новый Цвет(255,255,255);
		Иначе
			ОбластьУстановки.Область().ЦветФона = ЦветаСтиля["СтильГруппировки" + СокрЛП(ИндексТекущейГруппировки+1)]
		КонецЕсли;
	Исключение
	КонецПопытки;
КонецФункции
		
// Процеедура устанавливает параметры шрифта
//
Функция УстановитьПараметрыШрифта(ОбластьУстановки, флЖирный=Неопределено, флНаклонный=Неопределено, флПодчеркнутый=Неопределено, флЗачеркнутый=Неопределено) Экспорт
	Если флЖирный <> Неопределено Тогда
		ОбластьУстановки.Область().Шрифт = Новый Шрифт(ОбластьУстановки.ТекущаяОбласть.Шрифт,,,флЖирный);
	КонецЕсли;
	Если флНаклонный <> Неопределено Тогда
		ОбластьУстановки.Область().Шрифт = Новый Шрифт(ОбластьУстановки.ТекущаяОбласть.Шрифт,,,,флНаклонный);
	КонецЕсли;
	Если флПодчеркнутый <> Неопределено Тогда
		ОбластьУстановки.Область().Шрифт = Новый Шрифт(ОбластьУстановки.ТекущаяОбласть.Шрифт,,,,,флПодчеркнутый);
	КонецЕсли;
	Если флЗачеркнутый <> Неопределено Тогда
		ОбластьУстановки.Область().Шрифт = Новый Шрифт(ОбластьУстановки.ТекущаяОбласть.Шрифт,,,,,,флЗачеркнутый);
	КонецЕсли;
	Возврат ОбластьУстановки;
КонецФункции

// Функция проверяет выбрана ли в отчете хотя бы одна структурная единица, 
// и если не одной не выбрано, то для формирования отчета передается список
// всех структурных единиц, доступных пользователю
//
Функция ПроверкаЗаполенияСпискаСтруктурныхЕдиниц(СписокСтруктурныхЕдиниц, ПредставлениеСпискаСтруктурныхЕдиниц="") Экспорт
	...
	Если СписокСтруктурныхЕдиниц.Количество() = 0 Тогда
		...
		Если РезультатЗапроса.Пустой() Тогда
			Предупреждение("Нет организаций, доступных для формирования отчета.");
			РезультатПроверки = Ложь;
		КонецЕсли;
		
		МассивОрганизаций = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Организация");
		СписокСтруктурныхЕдиниц.ЗагрузитьЗначения(МассивОрганизаций);
		ПредставлениеСпискаСтруктурныхЕдиниц = РаботаСДиалогами.ВыгрузитьСписокВСтроку(СписокСтруктурныхЕдиниц);
...
Функция СформироватьСтрокуВыводаПараметровПоДатам(Знач ДатаНач, Знач ДатаКон) Экспорт
	...
	Иначе
		...
		ИначеЕсли ДатаНач > ДатаКон Тогда
			ОписаниеПериода = "Период: " + Формат(ДатаНач, "ДФ = ""дд.ММ.гггг") 
							+ " - "      + Формат(ДатаКон, "ДФ = ""дд.ММ.гггг");
		Иначе							
...
Процедура ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграничения, Знач ИмяОграниченияПоРеквизиту, Знач ОграничениеПоРеквизиту, ИмяРеквизита = "") Экспорт
...
//	ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграниченийПоРеквизитам, "Организация", СтруктураПараметров.СтруктураОграничений.Организация);
	ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграниченийПоРеквизитам, "СписокОрганизаций", СтруктураПараметров.СтруктураОграничений.СписокОрганизаций, "Организация");
...
Функция ВернутьЧастьЗапросаПоВыборкеПолейОборотносальдоваяВедомость(МассивПоказателей, Знач ПоляДляВыборки = Истина, 
		Знач ИспользоватьАлиасыПолей = Ложь, Знач ИспользоватьВКачествеПрефиксаАлиасаИмяПоказателя = Ложь) Экспорт
	...
	Если ПоляДляВыборки Тогда
		...
		Если ИспользоватьАлиасыПолей Тогда
			...
			Для Каждого ИмяПоказателя Из МассивПоказателей Цикл
				...
				|	" + ИмяПоказателя + "НачальныйОстатокДт КАК " + СтрокаПрефикса + "НачальныйОстатокДт,
				|	" + ИмяПоказателя + "НачальныйОстатокКт КАК " + СтрокаПрефикса + "НачальныйОстатокКт,
				|	" + ИмяПоказателя + "ОборотДт КАК "           + СтрокаПрефикса + "ОборотДт,
				|	" + ИмяПоказателя + "ОборотКт КАК "           + СтрокаПрефикса + "ОборотКт,
				|	" + ИмяПоказателя + "КонечныйОстатокДт КАК "  + СтрокаПрефикса + "КонечныйОстатокДт,
				|	" + ИмяПоказателя + "КонечныйОстатокКт КАК "  + СтрокаПрефикса + "КонечныйОстатокКт";
...
Функция СформироватьТаблицуДанныхРазворотаСчетов(СтруктураПараметров, Знач ПравилаВыводаИтогов) Экспорт
	...
	Запрос.УстановитьПараметр("СписокОрганизаций", СтруктураПараметров.СтруктураОграничений.СписокОрганизаций);
	...
	Для Каждого ПравилаСчета из ПравилаВыводаИтогов Цикл
		...
		Если СтруктураПараметров.ПоОрганизациям Тогда
			Если СтруктураПараметров.ПорядокГруппировок[0].ИмяРеквизита = "ПоОрганизациям" Тогда
				Если ПравилаСчета.Субсчета Тогда
					
					ТекстИтогиПо = ТекстИтогиПо + ", Организация ИЕРАРХИЯ, Счет ИЕРАРХИЯ";
								
				Иначе
						
					ТекстИтогиПо = ТекстИтогиПо + ", Организация, Счет ";
									
				КонецЕсли;
			ИначеЕсли СтруктураПараметров.ПорядокГруппировок[0].ИмяРеквизита = "ДанныеПоПодразделам" Тогда
				Если ПравилаСчета.Субсчета Тогда
					
					ТекстИтогиПо = ТекстИтогиПо + ", Счет ИЕРАРХИЯ, Организация ИЕРАРХИЯ";
								
				Иначе
						
					ТекстИтогиПо = ТекстИтогиПо + ", Счет, Организация ";
...
Функция СформироватьСтруктуруОграниченийДляОборотноСальдовойВедомости(ОбъектОтчета,
	...
	Попытка
		Структура.Вставить("СписокОрганизаций", ОбъектОтчета.мСписокСтруктурныхЕдиниц);
	Исключение
		Структура.Вставить("СписокОрганизаций", Неопределено);
	КонецПопытки;
...
Процедура ДополнитьСтруктуруХраненияИтоговОборотка(Знач ИмяПоказателя, СтруктураОбщийИтог,
	...
	Знач ОборотДт, Знач ОборотКт, СтруктураИтогУровня) Экспорт
	...
	Если СтруктураИтогУровня <> Неопределено Тогда
	
		СтруктураИтогУровня[ИмяПоказателя + "НачальныйОстатокДт"] = СтруктураИтогУровня[ИмяПоказателя + "НачальныйОстатокДт"] + НачальныйОстатокДт;
		СтруктураИтогУровня[ИмяПоказателя + "НачальныйОстатокКт"] = СтруктураИтогУровня[ИмяПоказателя + "НачальныйОстатокКт"] + НачальныйОстатокКт;
		СтруктураИтогУровня[ИмяПоказателя + "КонечныйОстатокДт"] =  СтруктураИтогУровня[ИмяПоказателя + "КонечныйОстатокДт" ] + КонечныйОстатокДт;
		СтруктураИтогУровня[ИмяПоказателя + "КонечныйОстатокКт"] =  СтруктураИтогУровня[ИмяПоказателя + "КонечныйОстатокКт" ] + КонечныйОстатокКт;
		СтруктураИтогУровня[ИмяПоказателя + "ОборотДт"] =           СтруктураИтогУровня[ИмяПоказателя + "ОборотДт"]           + ОборотДт;
		СтруктураИтогУровня[ИмяПоказателя + "ОборотКт"] =           СтруктураИтогУровня[ИмяПоказателя + "ОборотКт"]           + ОборотКт;
...
//
Процедура ВывестиПоказателиОборотноСальдовойВедомости(ДокументРезультат, Выборка, Счет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, НулевойУровень=0, ЗаполнятьДанными=Истина, СтруктураИтогУровня=Неопределено, Знач ЖирныйШрифт = Неопределено) Экспорт

	ВыведеноПоказателей = 0;	
	
	ИмяГруппировки = Выборка.Группировка();
	
	Для Каждого ИмяПоказателя Из 
		СтруктураПараметров.МассивПоказателей Цикл
		...
		НужноДополнятьИтоги = (ИмяГруппировки = "Счет") И (Выборка.Уровень() = НулевойУровень);
		...
		Если НужноДополнятьИтоги Тогда
			...
			ДополнитьСтруктуруХраненияИтоговОборотка(ИмяПоказателя, СтруктураОбщийИтог,
				...
				ОборотДт, ОборотКт, СтруктураИтогУровня);
				
		КонецЕсли;

		// заполняет данными область вывода
		Если ЗаполнятьДанными Тогда
		...
		Если ВыведеноПоказателей > 1 Тогда
			...
			УстановитьПараметрыШрифта(ОбластьСтрокаПоказателяСчет, ЖирныйШрифт);
		...
		УстановитьПараметрыШрифта(Область, ЖирныйШрифт);
...
Процедура ВывестиГруппировкуОборотноСальдовойВедомости(Выборка, Знач ИндексТекущейГруппировки, МассивГруппировок, Уровень, СдвигУровня, 
	...
	ОтборСубконто, ОрганизацияОтбора=Неопределено, НулевойУровень, СтруктураИтогУровня) Экспорт

	Пока Выборка.Следующий() Цикл
		
		//Если ЗначениеЗаполнено(ОрганизацияОтбора) ТОгда
		//	Если Выборка.Организация <> ОрганизацияОтбора Тогда
		//		Продолжить;
		//	КонецЕсли;
		//КонецЕсли;
		...
		Если ИмяГруппировки = "Счет" Тогда
			...
			Если ПоследнийВыведенныйСчет = Выборка.Счет 
				...
				ВывестиВложеннуюГруппировкуОборотноСальдовойВедомости(Выборка, ИндексТекущейГруппировки, МассивГруппировок, Уровень, СдвигУровня, 
					СброситьСдвигУровня, ПоследнийВыведенныйСчет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, ОтборСубконто, ОрганизацияОтбора, НулевойУровень, СтруктураИтогУровня);
		...
		ВывестиДанныеПоВалютамОборотноСальдовойВедомости(СтруктураПараметров, ОбластьСтрокаПоказателяСчет, Выборка, ДокументРезультат, 
			СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, Отступ, НулевойУровень, СтруктураИтогУровня);
		
		ВывестиВложеннуюГруппировкуОборотноСальдовойВедомости(Выборка, ИндексТекущейГруппировки, МассивГруппировок, Уровень, СдвигУровня, 
			СброситьСдвигУровня, ПоследнийВыведенныйСчет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, ОтборСубконто,, НулевойУровень, СтруктураИтогУровня);
...
Процедура ВывестиВложеннуюГруппировкуОборотноСальдовойВедомости(Выборка, Знач ИндексТекущейГруппировки, МассивГруппировок, Уровень, СдвигУровня, 
	СброситьСдвигУровня, ПоследнийВыведенныйСчет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, ОтборСубконто, ОрганизацияОтбора=Неопределено, НулевойУровень, СтруктураИтогУровня) Экспорт
	...
	ВывестиГруппировкуОборотноСальдовойВедомости(Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, МассивГруппировок[ИндексТекущейГруппировки + 1]), 
						...
						СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, ОтборСубконто, ОрганизацияОтбора, НулевойУровень, СтруктураИтогУровня);
...
Процедура ВывестиДанныеПоВалютамОборотноСальдовойВедомости(СтруктураПараметров, ОбластьСтрокаПоказателяСчет, Выборка, 
	ДокументРезультат, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, Знач Отступ, НулевойУровень, СтруктураИтогУровня) Экспорт
	...
	Если СтруктураПараметров.СтруктураОграничений.ПоВалютам Тогда
		...
		Пока ВыборкаПоВалютам.Следующий() Цикл
			...
			// Заполенние параметров расшифровки
				Если ВыборкаПоВалютам.Валюта <> Null Тогда		
					СтруктураПараметров.СтруктураОграничений.Вставить("Валюта",  ВыборкаПоВалютам.Валюта);
				Иначе
					СтруктураПараметров.СтруктураОграничений.Вставить("Валюта",  неопределено);
				КонецЕсли;      
			...
			ЗаполнитьПараметрыРасшифровкиОборотноСальдовойВедомости(СтруктураПараметров.СтруктураОграничений.ДатаНач, 
			СтруктураПараметров.СтруктураОграничений.ДатаКон, СтруктураПараметров.СтруктураОграничений.ИмяРегистраБухгалтерии,
			ОбластьСтрокаПоказателяСчет, Выборка, , , СтруктураПараметров.СтруктураОграничений.ВВалютеСценария, СтруктураПараметров.СтруктураОграничений.Валюта);
			...
			ВывестиПоказателиОборотноСальдовойВедомости(ДокументРезультат, ВыборкаПоВалютам, ВыборкаПоВалютам.Счет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, НулевойУровень, ,СтруктураИтогУровня);
...
Процедура ВывестиОборотноСальдовуюВедомость(ВыборкаИсточник, ИмяГруппировки, ОбъектОтчета, ТаблицаРазвернутогоСальдо, ДанныеПоПодразделам, МассивПоказателей,
											СтруктураПараметров, СтруктураОбщийИтог, СоотвПодчСчета, ОбластьСтрокаСчет, ВВалютеСценария, ДокументРезультат,
											ТаблицаРазворотаСчетов, ОбластьСтрокаПоказателяСчет, Макет, НулевойУровень, СтруктураИтогУровня=Неопределено)
	
	Выборка = ВыборкаИсточник.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, ИмяГруппировки);
	
	Пока Выборка.Следующий() Цикл
		
		Если ИмяГруппировки = "Организация" Тогда
		    СтруктураИтогУровня = Новый Структура;
			ИнициализироватьСтруктуруХраненияИтоговОборотка(МассивПоказателей, СтруктураИтогУровня);
		КонецЕсли;
		...
		Если (НЕ ОбъектОтчета.ПоСубсчетамИСубконто И Уровень > НулевойУровень) 
			Или  (Не ДанныеПоПодразделам И ТипЗаписи = ТипЗаписиЗапроса.ИтогПоИерархии) 
			И ИмяГруппировки = "Счет" Тогда
			
			//в случае когда не выводятся итоги по иерархии,
			//нам необходимо только посичтать итоговые данные
			Если Не ДанныеПоПодразделам И  Уровень = НулевойУровень Тогда
		...
		Если ИмяГруппировки = "Организация" Тогда
			Если Уровень = 0 ТОгда
				ОбластьСтрокаСчет = Макет.ПолучитьОбласть("Строка|Организация");
				ОбластьСтрокаСчет.Параметры.Заполнить(Выборка);
			КонецЕсли;
		КонецЕсли;
		...
		Если Уровень = 0 Тогда
			Если СтруктураПараметров.ПоОрганизациям Тогда
				ЖирныйШрифт = Истина
			КонецЕсли;
			Если СтруктураПараметров.ДанныеПоПодразделам И (НЕ СтруктураПараметров.ПоОрганизациям) И СтруктураПараметров.ПоСубсчетамИСубконто Тогда
				ЖирныйШрифт = Истина
			КонецЕсли;
		ИначеЕсли Уровень = 1 Тогда
			Если СтруктураПараметров.ДанныеПоПодразделам И СтруктураПараметров.ПоОрганизациям И СтруктураПараметров.ПоСубсчетамИСубконто Тогда
				ЖирныйШрифт = Истина
			КонецЕсли;
		КонецЕсли;
		УстановитьПараметрыШрифта(ОбластьСтрокаСчет, ЖирныйШрифт);
		
		ДокументРезультат.Вывести(ОбластьСтрокаСчет, Уровень);
		
		Если Уровень = 0 Тогда
			Если СтруктураПараметров.ПоОрганизациям И (НЕ СтруктураПараметров.ДанныеПоПодразделам) Тогда
		...
		ВывестиПоказателиОборотноСальдовойВедомости(ДокументРезультат, Выборка, Выборка.Счет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, НулевойУровень, ЗаполнятьДанные, СтруктураИтогУровня, ЖирныйШрифт);
		...
		СтруктураОбщийИтог, -1, НулевойУровень, СтруктураИтогУровня);
		...
		Если СтрокаРазворотаСчета <> Неопределено Тогда
			...
			Если СтруктураПараметров.ПоОрганизациям Тогда
				Если СтруктураПараметров.ПорядокГруппировок[0].ИмяРеквизита = "ПоОрганизациям" Тогда
					ВыборкаПоОрганизации = СтрокаРазворотаСчета.Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Организация");
					Пока ВыборкаПоОрганизации.Следующий() Цикл
						Если ВыборкаПоОрганизации.Организация = Выборка.Организация Тогда
							ВыборкаПоСчету = ВыборкаПоОрганизации.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, СтрокаРазворотаСчета.МассивГруппировок[0]);
							Прервать;
						КонецЕсли;
					КонецЦикла;
			...
			СтруктураОбщийИтог, СтруктураПараметров, Новый Соответствие, Выборка.Организация, НулевойУровень, СтруктураИтогУровня);			
		КонецЕсли;
		
		Если СтруктураПараметров.ПоОрганизациям И (Уровень = 0) Тогда
			Если ИмяГруппировки = "Счет1" Тогда
				ВывестиОборотноСальдовуюВедомость(Выборка, "Организация", ОбъектОтчета, ТаблицаРазвернутогоСальдо, ДанныеПоПодразделам, МассивПоказателей,
											СтруктураПараметров, СтруктураОбщийИтог, СоотвПодчСчета, ОбластьСтрокаСчет, ВВалютеСценария, ДокументРезультат,
											ТаблицаРазворотаСчетов, ОбластьСтрокаПоказателяСчет, Макет, 1);
				//Продолжить;
			ИначеЕсли ИмяГруппировки = "Организация" Тогда
				ВывестиОборотноСальдовуюВедомость(Выборка, "Счет", ОбъектОтчета, ТаблицаРазвернутогоСальдо, ДанныеПоПодразделам, МассивПоказателей,
											СтруктураПараметров, СтруктураОбщийИтог, СоотвПодчСчета, ОбластьСтрокаСчет, ВВалютеСценария, ДокументРезультат,
											ТаблицаРазворотаСчетов, ОбластьСтрокаПоказателяСчет, Макет, 1, СтруктураИтогУровня);
				//Продолжить;
			КонецЕсли;
			
			Если НЕ СтруктураПараметров.ДанныеПоПодразделам Тогда
		...
		Если ИмяГруппировки = "Организация" Тогда	
			...
			ОбластьИтогиСчет = Макет.ПолучитьОбласть("ИтогиПоОрганизации|Счет");
			ДокументРезультат.Вывести(ОбластьИтогиСчет, 0);
			
			ОбластьИтогиСтрокаЧисла = Макет.ПолучитьОбласть("ИтогиСтрока|Числа");
			ОбластьИтогиЧисла       = Макет.ПолучитьОбласть("ИтогиПоОрганизации|Числа");
			...
			Для Каждого ИмяПоказателя Из МассивПоказателей Цикл
				...
				ЗаполнитьДаннымОблатьВыводаОборотка(Область, ФорматПоказателя, 
				СтруктураИтогУровня[ИмяПоказателя + "НачальныйОстатокДт"], СтруктураИтогУровня[ИмяПоказателя + "НачальныйОстатокКт"], 
				СтруктураИтогУровня[ИмяПоказателя + "КонечныйОстатокДт"], СтруктураИтогУровня[ИмяПоказателя + "КонечныйОстатокКт"], 
				СтруктураИтогУровня[ИмяПоказателя + "ОборотДт"], СтруктураИтогУровня[ИмяПоказателя + "ОборотКт"]);
				
				Если НомерТекущейСтрокиИтога > 0 Тогда
					ДокументРезультат.Вывести(ОбластьИтогиСтрокаСчет, 0);
				КонецЕсли;
				
				УстановитьПараметрыШрифта(Область, Истина);
				ДокументРезультат.Присоединить(Область);
...
Процедура СформироватьОтчетОборотноСальдовойВедомости(ОбъектОтчета, ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0,
	...
	Знач ВидУчета = Неопределено, Знач Валюта = Неопределено, Знач ОтражениеВНУ = Неопределено, 
	Знач Сценарий = Неопределено, Знач ВВалютеСценария = Неопределено, Знач ДанныеПоПодразделам = Истина, Знач ПоОрганизациям = Ложь, Знач ПорядокГруппировок = Неопределено, Знач ПоСубсчетамИСубконто = Истина) Экспорт
	...
	Если ДанныеПоПодразделам Тогда // Или ПоОрганизациям Тогда
	...
	СтруктураПараметров.Вставить("ДанныеПоПодразделам", ДанныеПоПодразделам);
	СтруктураПараметров.Вставить("ПоОрганизациям", ПоОрганизациям);
	СтруктураПараметров.Вставить("ПорядокГруппировок", ПорядокГруппировок);
	...
	Если ПоОрганизациям Тогда 
		Если ПорядокГруппировок[0].ИмяРеквизита = "ПоОрганизациям" Тогда
			ИмяГруппировки = "Организация";
		ИначеЕсли ПорядокГруппировок[0].ИмяРеквизита = "ДанныеПоПодразделам" Тогда
			ИмяГруппировки = "Счет1";
		КонецЕсли;
	Иначе
		ИмяГруппировки = "Счет";
	КонецЕсли;
	
	ВывестиОборотноСальдовуюВедомость(РезультатЗапроса, ИмяГруппировки, ОбъектОтчета, ТаблицаРазвернутогоСальдо, ДанныеПоПодразделам, МассивПоказателей,
										СтруктураПараметров, СтруктураОбщийИтог, СоотвПодчСчета, ОбластьСтрокаСчет, ВВалютеСценария, ДокументРезультат,
										ТаблицаРазворотаСчетов, ОбластьСтрокаПоказателяСчет, Макет, 0);
	...
	//и не печатем последние 5 колонок, т.к. это впомогательная область для печати данных по организации
	ДокументРезультат.ОбластьПечати = ДокументРезультат.Область(1, 2, ДокументРезультат.ВысотаТаблицы, ДокументРезультат.ШиринаТаблицы - 5);
...
Процедура ЗаполнитьПараметрыРасшифровкиОборотноСальдовойВедомости(Знач ДатаНач, Знач ДатаКон, Знач ИмяРегистраБухгалтерии,
	Область, Выборка, ОтборСубконто = Неопределено, ВыведенныйСчет = Неопределено, Знач ВВалютеСценария = Неопределено, Валюта = неопределено) Экспорт 
	
	// Заполение значений для расшифровки
	СчетДляРасшифровки = ?(Выборка.Счет <> NULL, Выборка.Счет, ВыведенныйСчет);
	
	Если НЕ ИмяРегистраБухгалтерии = "фин_Бюджетирование" Тогда
		ОрганизацияДляРасшифровки = ?(Выборка.Организация <> NULL, Выборка.Организация, Неопределено);
	КонецЕсли;
	...
	ПараметрыОСВПоСчету.Вставить("Организация", ОрганизацияДляРасшифровки);
	...
	ПараметрыКарточкиСчета.Вставить("Организация", ОрганизацияДляРасшифровки);
	Если Валюта <> Неопределено Тогда
		ПараметрыКарточкиСчета.Вставить("Валюта", Валюта);
	КонецЕсли;
	...
	ПараметрыАнализСчета.Вставить("Организация", ОрганизацияДляРасшифровки);
	Если ВВалютеСценария <> Неопределено Тогда
	...
	ПараметрыОборотыСчетаМесяц.Вставить("Организация", ОрганизацияДляРасшифровки);
    ...
    ПараметрыОборотыСчетаДень.Вставить("Организация", ОрганизацияДляРасшифровки);	
...
Процедура ДокументРезультатОбработкаРасшифровкиОборотноСальдовойВедомости(Элемент, Расшифровка, 
		...
		//если эта первая ячейка - то не нужна расшифровка
		Если Элемент.ТекущаяОбласть.Имя  = Элемент.Область(1,1).Имя  Или Элемент.ТекущаяОбласть.Лево = 1 Тогда 
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;     	
	...
	Если ТипЗнч(ВыбраннаяРасшифровка) = Тип("Структура") 
		...
		Отчет = Отчеты[?(Найти(ВыбраннаяРасшифровка["ИмяОбъекта"],"Бюджетирование")=0,"Удалить","") + ВыбраннаяРасшифровка["ИмяОбъекта"]].Создать();
...
Процедура ВывестиДанныеОстатковКарточкиСчетаВМакет(ЗапросПоОстаткам, Знач ОбластьМакет, Знач Счет, Знач ЗапросНаНачало, 
	ТекущееСальдо = 0, ТекущееСальдоКолво = 0, ЗначениеВалюты = Неопределено) Экспорт
	...
	ВалютнаяСуммаОстатокДт = ПривестиКЧислу(Остатки.ВалютнаяСуммаОстатокДт);
	ВалютнаяСуммаОстатокКт = ПривестиКЧислу(Остатки.ВалютнаяСуммаОстатокКт);

	
	ТекущееСальдо = СуммаОстатокДт - СуммаОстатокКт;
	ТекущееСальдоКолво = КоличествоОстатокДт - КоличествоОстатокКт;
	ЕстьВалютаВОтборе = Ложь;
	
	Если ЗначениеВалюты <> Неопределено Тогда
		ЕстьВалютаВОтборе = Истина;	
	КонецЕсли; 		
	...
	Если Счет.Вид = ВидСчета.АктивноПассивный Тогда
		Если СуммаОстатокДт > СуммаОстатокКт Тогда
			...
			КонецЕсли;			
			Если ЕстьВалютаВОтборе Тогда
				ОбластьМакет.Параметры.ВалютнаяСуммаСальдоДт   = ВалютнаяСуммаОстатокДт - ВалютнаяСуммаОстатокКт;
				ОбластьМакет.Параметры.ВалютнаяСуммаСальдоКт   = 0;
				ОбластьМакет.Параметры.Валюта   = "Валютная сумма (" + ЗначениеВалюты + ")" ;
			КонецЕсли;  			
		...
		Иначе
			...
			Если ЕстьВалютаВОтборе Тогда
				...
				ОбластьМакет.Параметры.ВалютнаяСуммаСальдоКт   = ВалютнаяСуммаОстатокКт - ВалютнаяСуммаОстатокДт;
				ОбластьМакет.Параметры.Валюта   = "Валютная сумма (" + ЗначениеВалюты + ")" ;
			КонецЕсли;
...
// процедура выводит данные остатков по оборотам в макет
Процедура ВывестиДанныеОстатковКарточкиСчетаНУВМакет(ЗапросПоОстаткам, Знач ОбластьМакет, Знач Счет, Знач ЗапросНаНачало, 
...
Процедура ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, Знач ИмяОбластиВывода, Знач Выборка, 
	Знач ДокументРезультат, Знач ПоСубсчетам, Знач ИмяПоляНаличияОборота, СписокСчетов, Знач ДополнениеКУровнюВывода = 0, ДанныеПоКорПодразделам = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ДанныеПоКорПодразделам и ТипЗаписи = ТипЗаписиЗапроса.ИтогПоИерархии Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПолучитьДанныеДляОборотов(ДанныеДляВыбораСумм, Знач ПолучениеСуммНаНачало, 
	...
	ПостфиксИменПолей = ?(ПолучениеСуммНаНачало, "Нач", "Оборот");
...
Процедура ВывестиПолныйЗаголовокОтчетаОборотовСчета(Выборка = Неопределено, ШиринаТаблицы, ОбъектОтчет, Макет, ДокументРезультат,
	...
	Знач ОбщийИтог = Неопределено, ДанныеПоКорПодразделам = Истина) Экспорт
	...
	Если ОбъектОтчет.ОборотДтКорСчета Тогда
		...
		ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, ИмяСтроки+"|ОборотДтКорсчет", Выборка, 
			ДокументРезультат, ОбъектОтчет.ПоСубсчетамКорСчетов, "ЕстьСуммаОборотДт", СоотвДт, ДополнениеКУровнюВывода, ДанныеПоКорПодразделам);
	...
	Если ОбъектОтчет.ОборотКтКорСчета Тогда
		...
		ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, ИмяСтроки+"|ОборотКтКорсчет", Выборка, 
			ДокументРезультат, ОбъектОтчет.ПоСубсчетамКорСчетов, "ЕстьСуммаОборотКт", СоотвКт, ДополнениеКУровнюВывода,ДанныеПоКорПодразделам);
...
Процедура ВывестиОборотыЗаПериод(СчетОграниченийОтчета, Знач ЕстьИтог, ОбщийИтог, Макет, ДокументРезультат,
	...
	Если ЕстьИтог Тогда
		...
		ПолучитьДанныеДляОборотов(ОбщийИтог, Истина,
			Ложь, Ложь, СуммаНачДт, СуммаНачКт);
			
		ПолучитьДанныеДляОборотов(ОбщийИтог, Ложь, 
			Ложь, Ложь, СуммаОборотДт, СуммаОборотКт);	
						
						
		РассчитатьКонечныеДанныеПоВидуСчетаИНачальнымДанным(СчетОграниченийОтчета.Вид, СуммаКонДт, СуммаКонКт, СуммаНачДт, СуммаНачКт,
...
Процедура ВывестиИтогиОборотноСальдовойВедомостиПоСчету(Знач Выборка, Знач ИмяРегистраБухгалтерии, Знач МассивПоказателей, Знач ФорматПоказателей,
	Знач Счет, Знач ОбластьИтогиСтрокаЧисла, Знач ОбластьИтогиЧислаМакет, Знач ОбластьИтогиСтрокаСчет, Знач ДокументРезультат) Экспорт
...
Процедура ВывестиПоказателиОборотноСальдовойВедомостиПоСчету(Выборка, СтруктураПараметров, Знач Счет, 
	Знач СтруктураЗамещающихДанных = Неопределено, ЖирныйШрифт=Ложь, ИндексТекущейГруппировки) Экспорт

	КоличествоГруппировок = СтруктураПараметров.МассивГруппировок.Количество() - 1;
	...
	Для Каждого ИмяПоказателя Из СтруктураПараметров.МассивПоказателей Цикл
		...
		УстановитьПараметрыШрифта(Область, ЖирныйШрифт);		
		УстановитьЦветОбласти(Область, ИндексТекущейГруппировки, КоличествоГруппировок);
		...
		Если ВыведеноПоказателей > 0 Тогда
			...
			УстановитьПараметрыШрифта(ВыводимаяОбласть, ЖирныйШрифт);
			
			УстановитьЦветОбласти(ВыводимаяОбласть, ИндексТекущейГруппировки, КоличествоГруппировок);			
...
Процедура ВывестиГруппировкуОборотноСальдовойВедомостиПоСчету(ОбъектОтчета, Выборка, Знач ИндексТекущейГруппировки, СтруктураПараметров) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ИмяГруппировки = "Организация" Тогда
			...
			ВыводимаяОбласть.Параметры.СчетПредставление = Выборка.Организация.НаименованиеПолное;
		...
		ИначеЕсли Лев(ИмяГруппировки, 8) = "Субконто" Тогда
			...
			Если (Выборка.Уровень() = 0) И (СтруктураПараметров.МассивГруппировок.Количество() > 1) Тогда
				ЖирныйШрифт = Истина;
			КонецЕсли;
		...
		// последний уровень не будем расскрашивать
		УстановитьЦветОбласти(ВыводимаяОбласть, ИндексТекущейГруппировки, КоличествоГруппировок);
		...
		Если Выборка.ТипЗаписи() = ТипЗаписиЗапроса.ИтогПоИерархии И 
			...
			ИмяГруппировки <> "Счет" И ИмяГруппировки <> "Организация" Тогда
			
			ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы, 2).Шрифт = СтруктураПараметров.ШрифтГрупп;
			
		КонецЕсли;
		
		ВывестиПоказателиОборотноСальдовойВедомостиПоСчету(Выборка, СтруктураПараметров, ОбъектОтчета.Счет,, ЖирныйШрифт, ИндексТекущейГруппировки);
		...
		Если СтруктураПараметров.СтруктураОграничений.ПоВалютам Тогда
			...
			Пока ВыборкаПоВалютам.Следующий() Цикл
				...
				Если ВыборкаПоВалютам.Валюта <> Null Тогда		
					Валюта = ВыборкаПоВалютам.Валюта;
				Иначе
					Валюта = Неопределено;
				КонецЕсли;  
				
				// Заполенние параметров расшифровки
				ЗаполнитьПараметрыРасшифровкиОбороткиПоСчету(ОбъектОтчета, ВыводимаяОбласть, Выборка, СтруктураПараметров, Валюта);
				
				ДокументРезультат.Вывести(ВыводимаяОбласть, Выборка.Уровень());
				
				ВывестиПоказателиОборотноСальдовойВедомостиПоСчету(ВыборкаПоВалютам, СтруктураПараметров, ОбъектОтчета.Счет,,, ИндексТекущейГруппировки);
	...
	Попытка
		...
		Если МетаданныеОбъекта<>Неопределено Тогда
			...
			Если Метаданные.Справочники.Найти(МетаданныеОбъекта.Имя) <> Неопределено 
				ИЛИ Метаданные.ПланыВидовХарактеристик.Найти(МетаданныеОбъекта.Имя) <> Неопределено  Тогда
...
Функция СоздатьСтруктуруДопОграниченийДляОборотноСальдовойВедомостиПоСчету(ОтчетОбъект, Выборка, МассивГруппировок, 
	...
	Для Сч = 0 По НомерГруппировкиВМассиве Цикл 
		...
		Если ТекущееИмяГруппировки = "ОРГАНИЗАЦИЯ"
			ИЛИ ТекущееИмяГруппировки = "СЧЕТ"
...
Процедура ЗаполнитьРасшифровкуДляСчета(ОтчетОбъект, Область, Выборка, СтруктураПараметров, Знач ИмяРегистраБухгалтерии,
	Знач РасшифровываемыйСчет, Знач РасшифровываемыйСчетПредставление, Валюта = Неопределено, Знач РасшифровываемаяОрганизация, Знач РасшифровываемаяОрганизацияПредставление) Экспорт
	...
	Если РасшифровываемаяОрганизация <> Неопределено Тогда
		ПараметрыКарточкиСчета.Вставить("Организация", РасшифровываемаяОрганизация);
	КонецЕсли;
	...
	ИначеЕсли ИмяГруппировки = "Организация" Тогда
		...
		СписокРасшифровки = Новый СписокЗначений;
        СписокРасшифровки.Добавить(ПараметрыКарточкиСчета, "Карточка счета " + РасшифровываемаяОрганизацияПредставление);
...
//	Нет.
//
Процедура ЗаполнитьПараметрыРасшифровкиОбороткиПоСчету(ОтчетОбъект, Область, Выборка, СтруктураПараметров, Валюта = Неопределено) Экспорт
	...
	Если НЕ ОтчетОбъект.ИмяРегистраБухгалтерии = "фин_Бюджетирование" Тогда
		Если Выборка.Организация <> NULL Тогда
			РасшифровываемаяОрганизация = Выборка.Организация;
			РасшифровываемаяОрганизацияПредставление = Выборка.ОрганизацияПредставление;
		Иначе
			РасшифровываемаяОрганизация = Неопределено;
			РасшифровываемаяОрганизацияПредставление = "";
		КонецЕсли;
	КонецЕсли;
	
	ЗаполнитьРасшифровкуДляСчета(ОтчетОбъект, Область, Выборка, СтруктураПараметров, ОтчетОбъект.ИмяРегистраБухгалтерии,
		РасшифровываемыйСчет, РасшифровываемыйСчетПредставление, Валюта, РасшифровываемаяОрганизация, РасшифровываемаяОрганизацияПредставление);
...
// Функция получает вид сравнения субконто
Функция ПолучитьВидСравненияСубконто(ТипыСравнения, КлючСравнения)
	Резт = Неопределено;
	
	Если ТипыСравнения <> Неопределено Тогда
		
		НомерСубконто = Прав(КлючСравнения, 1);
		Резт = ТипыСравнения["Субконто"+НомерСубконто];
...
Процедура КонтрольДоступностиЗаголовкаИПодписей(ДатаНачала, ДатаКонца, КнопкаЗаголовок, КнопкаПодпись, КнопкаПодменюЗаголовок, КнопкаПодменюПодпись) Экспорт	
	...
	КнопкаПодменюЗаголовок.Доступность = ДоступностьКнопок;
	КнопкаПодменюПодпись.Доступность   = ДоступностьКнопок;
