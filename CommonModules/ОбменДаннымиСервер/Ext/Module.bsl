...
Процедура ОбновитьКонстантуНастройкаПодчиненногоУзлаРИБЗавершена() Экспорт
	...
	Если  ЭтоПодчиненныйУзелРИБ
		...
		Выполнить("Константы.НастройкаПодчиненногоУзлаРИБЗавершена.Установить(Истина);");
...
Процедура ВыполнитьПроверкуНаличияПравилОбменаЗагруженныхИзФайла()
	...
	Если Не Результат.Пустой() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление информационной базы'"), УровеньЖурналаРегистрации.Ошибка,,, СтрокаСообщения);
...
Процедура ДобавитьПараметрыРаботыКлиентскойЛогикиСтандартныхПодсистем(Параметры) Экспорт
	...
	Параметры.Вставить("НастройкаПодчиненногоУзлаРИБЗавершена", Вычислить("Константы.НастройкаПодчиненногоУзлаРИБЗавершена.Получить()"));
