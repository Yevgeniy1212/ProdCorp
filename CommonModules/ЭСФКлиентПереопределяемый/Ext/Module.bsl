...
Процедура ОткрытьФормуПечати(ТабличныйДокумент, МассивДокументов) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабличныйДокумент, , , НСтр("ru = 'Электронные счета-фактуры'"));
...
Процедура ОткрытьФормуВыбораКлассификатора(СтруктураПараметров, Элемент, УдалятьПрефиксМакета = Истина) Экспорт
	...
	ФормаВыбораКода = ПолучитьОбщуюФорму("ФормаВыбораИзКлассификатора", Элемент);
	ФормаВыбораКода.РежимВыбора = Истина;
	ФормаВыбораКода.ТекущийКодСтроки = ?(НЕ ЗначениеЗаполнено(СтруктураПараметров.ТекущийКодСтроки), Неопределено, СокрЛП(СтруктураПараметров.ТекущийКодСтроки));
	ФормаВыбораКода.ИмяМакета = СтруктураПараметров.ИмяМакета;
	ФормаВыбораКода.ИмяСекции = СтруктураПараметров.ИмяСекции;
	
	ФормаВыбораКода.ПолучатьПолныеДанные = СтруктураПараметров.ПолучатьПолныеДанные;
