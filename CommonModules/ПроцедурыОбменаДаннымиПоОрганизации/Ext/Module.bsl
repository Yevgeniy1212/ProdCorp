...
Функция НеИспользоватьФильтрПоДокументу(ДокументДанных)
	...
	ИначеЕсли Метаданные.Подсистемы.УниверсальныеМеханизмы.Подсистемы.РИБ.Подсистемы.НастройкиБюджетированияОбщиеДляОрганизаций.Состав.Содержит(ДокументДанных.Метаданные()) И НЕ ЗначениеЗаполнено(ДокументДанных.Организация) Тогда
		Результат = Истина;
	// - КУФИБ - конец
	КонецЕсли;
...
Функция ОпределениеУзловДляВыгрузкиНабораЗаписейРегистраСведений(Элемент, МетаданныеРегистра, 
	...
	ИначеЕсли МетаданныеРегистра.Имя = "ОбъектыДоступаДокументов"
		Или МетаданныеРегистра.Имя = "ПодтверждениеРеализацииВЕАЭС" 
		Или МетаданныеРегистра.Имя = "АктуальныеЭСФ" 
		Или МетаданныеРегистра.Имя = "ИсторияЭСФ" Тогда
		
		Если МетаданныеРегистра.Имя = "ОбъектыДоступаДокументов" Тогда
			ПолеОтбора = "ДокументСсылка";
		ИначеЕсли МетаданныеРегистра.Имя = "АктуальныеЭСФ" Тогда
			ПолеОтбора = "СчетФактура";
		ИначеЕсли МетаданныеРегистра.Имя = "ИсторияЭСФ" Тогда
			ПолеОтбора = "ЭСФ";
		ИначеЕсли МетаданныеРегистра.Имя = "ПодтверждениеРеализацииВЕАЭС" Тогда
			ДокументОтбора = Элемент.Отбор.ДокументРеализации.Значение;
		КонецЕсли;
		
		ОпределитьПоДокументуИзмерениюМассивУзловДляПередачи(Элемент, МассивУзловДляПередачи, СоотвествиеУзловИОрганизаций, МетаданныеРегистра, ВыгружатьДляВсехУзлов,ПолеОтбора);	
...
Процедура ДополнитьМассивУзловДляВыгрузкиДокумента(Элемент, МассивУзловДляПередачи, 
	...
	МассивОрганизаций.Добавить(Элемент.Организация);
	
	Если ТипЭлемента = Тип("ДокументСсылка.КадровоеПеремещениеОрганизаций") 
		Или ТипЭлемента = Тип("ДокументОбъект.КадровоеПеремещениеОрганизаций") Тогда 
		Если ЗначениеЗаполнено(Элемент.ОбособленноеПодразделениеКуда) Тогда 
			МассивОрганизаций.Добавить(Элемент.ОбособленноеПодразделениеКуда);
		КонецЕсли;
		Если ЗначениеЗаполнено(Элемент.ОбособленноеПодразделениеОткуда) Тогда 
			МассивОрганизаций.Добавить(Элемент.ОбособленноеПодразделениеОткуда);
		КонецЕсли;
	КонецЕсли;
...
Функция ОпределениеУзловДляВыгрузкиСправочника(Элемент, ТипЭлемента = Неопределено, 
	...
	ИначеЕсли Элемент.Метаданные().Имя = "Файлы" И ФайлОрганизации(Элемент) Тогда
	...
	Иначе
		...
		Если ТипЭлемента = Тип("СправочникОбъект.ЭСФПрисоединенныеФайлы") ИЛИ ТипЭлемента = Тип("СправочникСсылка.ЭСФПрисоединенныеФайлы") Тогда
			
			ВладелецФайла = ПолучитьЗначениеПараметраОбъектаИлиСсылки(Элемент, "Справочники", ТипЭлемента, "ВладелецФайла");
			ОрганизацияВладельца = ПолучитьЗначениеПараметраОбъектаИлиСсылки(ВладелецФайла, "Документы", ТипЭлемента, "Организация");
			
			ДополнитьМассивУзламиПоОрганизации(МассивУзловДляПередачи, ОрганизацияВладельца, СоотвествиеУзловИОрганизаций);
...
Процедура ПередЗаписьюСправочникаДляОбменаПоОрганизацииПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Элемент.Отбор.Найти(ПолеСправочник) <> Неопределено Тогда
		...
		Если СохраненныйМассивУзлов = Неопределено Тогда
			...
			ИначеЕсли РегистраторНабораЗаписей.Метаданные().Реквизиты.Найти("Организация") <> Неопределено Тогда
				ОрганизацияРегистратора = РегистраторНабораЗаписей.Организация;
			ИначеЕсли РегистраторНабораЗаписей.Метаданные().Владельцы.Количество()>0 И ТипЗнч(РегистраторНабораЗаписей.Владелец)=Тип("СправочникСсылка.Организации") Тогда
