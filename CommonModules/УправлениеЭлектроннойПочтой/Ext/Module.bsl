...
//
Процедура ОтправитьПисьма(Письма, СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, Знач ПоказыватьОкноВыполнения = Истина, ТекстОшибок = "",Массивом=Ложь) Экспорт
	...
	Если ИзмененныйСоставПисем.Количество() > 0 Тогда
		ПолучениеОтправкаПисем(СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, , ?(Массивом,МассивПисем,ИзмененныйСоставПисем), 
	...
	#КонецЕсли
							...
							Пока Выборка.Следующий() Цикл
								...
								Если ТипЗнч(ДвоичныеДанные) = Тип("ДвоичныеДанные") Тогда
									ПочтовоеСообщение.Вложения.Добавить(Выборка.ХранилищеЗначения.Получить(), УстранитьДублированиеТочек(ИмяФайла));
								КонецЕсли; 
							КонецЦикла; 
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЭлектронноеПисьмо") И НЕ Копирование Тогда
		...
		Если Письмо.ВидТекстаПисьма = Перечисления.ВидыТекстовЭлектронныхПисем.HTML ИЛИ Письмо.ВидТекстаПисьма = Перечисления.ВидыТекстовЭлектронныхПисем.HTMLСКартинками Тогда
			...
			Если (Основание.СтатусПисьма = Перечисления.СтатусыПисем.Полученное ИЛИ Основание.СтатусПисьма = Перечисления.СтатусыПисем.Отправленное) И ЗначениеЗаполнено(Основание.Дата) Тогда
				...
				ДатаИсходногоПисьма = Основание.ДатаОтправления;
...
Процедура ОбработкаСобытияOnClickПоляHTML(ТекущийПользователь, Элемент, pEvtObj, УчетнаяЗапись = Неопределено, ГруппаУчетнойЗаписи = Неопределено, ФормаВладелец = Неопределено, ЭлектронноеПисьмо = Неопределено) Экспорт
	...
	Если НЕ ПустаяСтрока(href) Тогда
		...
		Если ((ТипЗнч(ЭлектронноеПисьмо) = Тип("ДокументСсылка.ЭлектронноеПисьмо") И ЗначениеЗаполнено(ЭлектронноеПисьмо))
			   ИЛИ (ТипЗнч(ЭлектронноеПисьмо) = Тип("ДокументОбъект.ЭлектронноеПисьмо") И ЭлектронноеПисьмо <> Неопределено))
		       И СтрЧислоВхождений(href, "ИмяФайлаРедактированияХТМЛТекста") > 0 Тогда
...
КонецПроцедуры // ОтправитьДокументПоЭлектроннойПочте()
...
// Функция находит все многоточия в строке и превращает их в одну точку
// Например: "Мой..Файл.....txt" -> "Мой.файл.txt"
// Эту функцию можно использовать для изменения имени вложения, т.к. "ИнтернетПочта.Послать()" преобразует 
// имена вложений, что может привести, например, к следующему: "Отчет........pdf" -> "Отчет._._._._pdf"
//
// Параметры:
//   Строка - Строка - Строка, в которой нужно убрать дублирующиеся точки
//
// Возвращаемое значение:
//   Строка, Строка, в которой нет дублирующихся точек
//
Функция УстранитьДублированиеТочек(Строка)
