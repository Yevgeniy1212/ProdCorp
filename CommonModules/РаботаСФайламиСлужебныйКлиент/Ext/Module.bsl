...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	КоличествоЗанятыхФайлов = РаботаСФайламиСлужебныйВызовСервера.ПолучитьКоличествоЗанятыхФайлов(, ТекущийПользователь);
...
Функция ПроинициализироватьКомпоненту() Экспорт
	...
	Если глХранилищеЗначений=Неопределено ИЛИ (НЕ глХранилищеЗначений.Свойство("КомпонентаTwain")) ИЛИ глХранилищеЗначений.КомпонентаTwain = Неопределено Тогда
		...
		Если глХранилищеЗначений=Неопределено Тогда
			глХранилищеЗначений = Новый Структура;
		КонецЕсли;
		глХранилищеЗначений.Вставить("КомпонентаTwain",Новый("AddIn.twain.AddInNativeExtension"));	
	КонецЕсли;
...
Функция ВерсияКомпонентыСканирования() Экспорт
	...
	Возврат глХранилищеЗначений.КомпонентаTwain.Версия();
...
Функция ПолучитьУстройства() Экспорт
	...
	СтрокаУстройств = глХранилищеЗначений.КомпонентаTwain.ПолучитьУстройства();
...
Функция ДоступнаКомандаСканировать() Экспорт
	...
	Если глХранилищеЗначений.КомпонентаTwain.ЕстьУстройства() Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ПолучитьНастройку(ИмяУстройства, ИмяНастройки) Экспорт
	...
	Попытка
		Возврат глХранилищеЗначений.КомпонентаTwain.ПолучитьНастройку(ИмяУстройства, ИмяНастройки);
	Исключение
		Возврат -1;
	КонецПопытки;
