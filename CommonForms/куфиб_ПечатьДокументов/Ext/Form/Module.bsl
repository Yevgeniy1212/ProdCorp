...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда 
		Проверить(ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Или ОбщегоНазначения.ЗначениеСсылочногоТипа(Параметры.ПараметрКоманды),
	...
	КонецЕсли;
    фин_ОбщегоНазначенияСервер.НастроитьКнопкиОтправки(ЭтаФорма,Элементы.ГруппаКоманднаяПанель);
	
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьНаПочту(Кнопка)
	общ_ОбщегоНазначенияКлиент.ОтправкаПечатнойФормыПоЭлПочте(ЭтаФорма, ПечатнаяФормаОбразец);
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.БледноЛиловый);
...
Процедура СоздатьРеквизитыИЭлементыФормыДляПечатныхФорм(КоллекцияПечатныхФорм)
	...
	Для Каждого РеквизитФормы Из НовыеРеквизитыФормы Цикл
		...
		НоваяНастройкаПечатнойФормы.ИмяФайлаПечатнойФормы = фин_ОбщегоНазначенияСервер.ЗначениеВСтрокуXML(ОписаниеПечатнойФормы.ИмяФайлаПечатнойФормы);
		...
		Для Каждого СоответствиеОбъектаПечатнойФорме Из ПечатныеФормыПоОбъектам Цикл
			...
			Для Каждого ТипФайла Из ВыбранныеФорматыСохранения Цикл
				...
				Если СоответствиеОбъектаПечатнойФорме.Ключ <> "ОбъектыПечатиНеЗаданы" Тогда
					ИмяФайла = ИмяФайлаПечатнойФормыОбъекта(СоответствиеОбъектаПечатнойФорме.Ключ, фин_ОбщегоНазначенияСервер.ЗначениеИзСтрокиXML(НастройкаПечатнойФормы.ИмяФайлаПечатнойФормы));
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		ПараметрыДляВставки = фин_ОбщегоНазначенияСервер.ЗначенияРеквизитовОбъекта(ОбъектПечати, "Дата,Номер");
