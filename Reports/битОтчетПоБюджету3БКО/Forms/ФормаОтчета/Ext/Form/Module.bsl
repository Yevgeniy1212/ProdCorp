...
Процедура СформироватьНаСервере()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(Справочник.битИсточникиФинансирования.Собственные) КАК СтатьяБюджета,
		|	ВЫБОР
		...
		|	ЕСТЬNULL(фин_ОборотыБюджетовОбороты.СуммаУпрОборот, 0) КАК СуммаИтого,
		...
		|	ЗНАЧЕНИЕ(Справочник.битИсточникиФинансирования.Собственные) КАК ИсточникФинансирования,
		...
		|	СУММА(ВЫБОР
		|			КОГДА Итог.СуммаФактПредыдущий <> 0
		|				ТОГДА Итог.СуммаИтого / Итог.СуммаФактПредыдущий * 100
	...
	Если ЭтотОбъект.Отчет.ПоКварталам = Истина Тогда
		Макет = Отчеты.битОтчетПоБюджету3БКО.ПолучитьМакет("ПоКварталам");
	Иначе
		Макет = Отчеты.битОтчетПоБюджету3БКО.ПолучитьМакет("Макет");
	КонецЕсли;
	...
	Пока ВыборкаОбщийИтог.Следующий() Цикл		
		...
		Пока ВыборкаСтатьяБюджета.Следующий() Цикл
			...
			Если ВыборкаСтатьяБюджета.СтатьяБюджета.Наименование = "Всего инвестиций" Тогда
				...
				Если ОбластьГруппаИтог.Параметры.СуммаФактТриГодаНазад  > 0 Тогда
					ОбластьГруппаИтог.Параметры.СуммаПредществующий =  ОбластьГруппаИтог.Параметры.СуммаИтого / ОбластьГруппаИтог.Параметры.СуммаФактТриГодаНазад *100;
					иначе
					ОбластьГруппаИтог.Параметры.СуммаПредществующий = 0;
				КонецЕсли;

				
				Если ОбластьГруппаИтог.Параметры.СуммаПредществующийГод > 0 Тогда
					ОбластьГруппаИтог.Параметры.СуммаПланКФактуПредш =  ОбластьГруппаИтог.Параметры.СуммаИтого / ОбластьГруппаИтог.Параметры.СуммаПредществующийГод *100;
					иначе
					ОбластьГруппаИтог.Параметры.СуммаПланКФактуПредш = 0;
				КонецЕсли;
				Если Отчет.ПриоритетФакта = Истина Тогда
				    Если ОбластьГруппаИтог.Параметры.СуммаФактПредыдущий > 0 Тогда
					ОбластьГруппаИтог.Параметры.СуммаИтогоФакт =  ОбластьГруппаИтог.Параметры.СуммаИтого / ОбластьГруппаИтог.Параметры.СуммаФактПредыдущий *100;
				иначе
					ОбластьГруппаИтог.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
			иначе
				Если ОбластьГруппаИтог.Параметры.СуммаПланПредыдущийГод > 0 Тогда
					ОбластьГруппаИтог.Параметры.СуммаИтогоФакт =  ОбластьГруппаИтог.Параметры.СуммаИтого / ОбластьГруппаИтог.Параметры.СуммаПланПредыдущийГод *100;
				иначе
					ОбластьГруппаИтог.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
				    КонецЕсли;
			...
			Иначе
				...
				ИначеЕсли ВыборкаСтатьяБюджета.СтатьяБюджета.Категория2Уровня = Истина Тогда
					...
					Если ОбластьГруппаОС.Параметры.СуммаФактТриГодаНазад  > 0 Тогда
					ОбластьГруппаОС.Параметры.СуммаПредществующий =  ОбластьГруппаОС.Параметры.СуммаИтого / ОбластьГруппаОС.Параметры.СуммаФактТриГодаНазад *100;
					иначе
					ОбластьГруппаОС.Параметры.СуммаПредществующий = 0;
				КонецЕсли;
					
					Если ОбластьГруппаОС.Параметры.СуммаПредществующийГод > 0 Тогда
					ОбластьГруппаОС.Параметры.СуммаПланКФактуПредш =  ОбластьГруппаОС.Параметры.СуммаИтого / ОбластьГруппаОС.Параметры.СуммаПредществующийГод *100;
					иначе	ОбластьГруппаОС.Параметры.СуммаПланКФактуПредш = 0;
					КонецЕсли;
				
				Если Отчет.ПриоритетФакта = Истина Тогда
				    Если ОбластьГруппаОС.Параметры.СуммаФактПредыдущий > 0 Тогда
					ОбластьГруппаОС.Параметры.СуммаИтогоФакт =  ОбластьГруппаОС.Параметры.СуммаИтого / ОбластьГруппаОС.Параметры.СуммаФактПредыдущий *100;
				иначе
					ОбластьГруппаОС.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
			иначе
				Если ОбластьГруппаОС.Параметры.СуммаПланПредыдущийГод > 0 Тогда
					ОбластьГруппаОС.Параметры.СуммаИтогоФакт =  ОбластьГруппаОС.Параметры.СуммаИтого / ОбластьГруппаОС.Параметры.СуммаПланПредыдущийГод *100;
				иначе
					ОбластьГруппаОС.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
				    КонецЕсли;
				...
				Иначе
					...
					Если ОбластьГруппа.Параметры.СуммаФактТриГодаНазад  > 0 Тогда
					ОбластьГруппа.Параметры.СуммаПредществующий =  ОбластьГруппа.Параметры.СуммаИтого / ОбластьГруппа.Параметры.СуммаФактТриГодаНазад *100;
					иначе
					ОбластьГруппа.Параметры.СуммаПредществующий = 0;
				КонецЕсли;

					Если ОбластьГруппа.Параметры.СуммаПредществующийГод > 0 Тогда
					ОбластьГруппа.Параметры.СуммаПланКФактуПредш =  ОбластьГруппа.Параметры.СуммаИтого / ОбластьГруппа.Параметры.СуммаПредществующийГод *100;
					иначе ОбластьГруппа.Параметры.СуммаПланКФактуПредш =0;	
					КонецЕсли;
					Если Отчет.ПриоритетФакта = Истина Тогда
				    Если ОбластьГруппа.Параметры.СуммаФактПредыдущий > 0 Тогда
					ОбластьГруппа.Параметры.СуммаИтогоФакт =  ОбластьГруппа.Параметры.СуммаИтого / ОбластьГруппа.Параметры.СуммаФактПредыдущий *100;
				иначе
					ОбластьГруппа.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
			иначе
				Если ОбластьГруппа.Параметры.СуммаПланПредыдущийГод > 0 Тогда
					ОбластьГруппа.Параметры.СуммаИтогоФакт =  ОбластьГруппа.Параметры.СуммаИтого / ОбластьГруппа.Параметры.СуммаПланПредыдущийГод *100;
				иначе
					ОбластьГруппа.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
				    КонецЕсли;
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Если ВыборкаДетальныеЗаписи.СтатьяБюджета.Подкласс = Ложь Тогда
					...
					Если Область.Параметры.СуммаПредществующийГод > 0 Тогда
					Область.Параметры.СуммаПланКФактуПредш =  Область.Параметры.СуммаИтого / Область.Параметры.СуммаПредществующийГод *100;
					иначе	Область.Параметры.СуммаПланКФактуПредш = 0;
					КонецЕсли;
				
				Если Область.Параметры.СуммаФактТриГодаНазад  > 0 Тогда
					Область.Параметры.СуммаПредществующий =  Область.Параметры.СуммаИтого / Область.Параметры.СуммаФактТриГодаНазад *100;
					иначе
					Область.Параметры.СуммаПредществующий = 0;
				КонецЕсли;
				
				    Если Отчет.ПриоритетФакта = Истина Тогда
				    Если Область.Параметры.СуммаФактПредыдущий > 0 Тогда
					Область.Параметры.СуммаИтогоФакт =  Область.Параметры.СуммаИтого / Область.Параметры.СуммаФактПредыдущий *100;
				иначе
					Область.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
			иначе
				Если Область.Параметры.СуммаПланПредыдущийГод > 0 Тогда
					Область.Параметры.СуммаИтогоФакт =  Область.Параметры.СуммаИтого / Область.Параметры.СуммаПланПредыдущийГод *100;
				иначе
					Область.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
				    КонецЕсли;
				...
				Иначе 
					...
					Если ОбластьСтрокаОС.Параметры.СуммаПредществующийГод > 0 Тогда
					ОбластьСтрокаОС.Параметры.СуммаПланКФактуПредш =  ОбластьСтрокаОС.Параметры.СуммаИтого / ОбластьСтрокаОС.Параметры.СуммаПредществующийГод *100;
					иначе	 ОбластьСтрокаОС.Параметры.СуммаПланКФактуПредш = 0;
					КонецЕсли;
				
				Если ОбластьСтрокаОС.Параметры.СуммаФактТриГодаНазад  > 0 Тогда
					ОбластьСтрокаОС.Параметры.СуммаПредществующий =  ОбластьСтрокаОС.Параметры.СуммаИтого / ОбластьСтрокаОС.Параметры.СуммаФактТриГодаНазад *100;
					иначе
					ОбластьСтрокаОС.Параметры.СуммаПредществующий = 0;
				КонецЕсли;

				Если Отчет.ПриоритетФакта = Истина Тогда
				    Если ОбластьСтрокаОС.Параметры.СуммаФактПредыдущий > 0 Тогда
					ОбластьСтрокаОС.Параметры.СуммаИтогоФакт =  ОбластьСтрокаОС.Параметры.СуммаИтого / ОбластьСтрокаОС.Параметры.СуммаФактПредыдущий *100;
				иначе
					ОбластьСтрокаОС.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
			иначе
				Если ОбластьСтрокаОС.Параметры.СуммаПланПредыдущийГод > 0 Тогда
					ОбластьСтрокаОС.Параметры.СуммаИтогоФакт =  ОбластьСтрокаОС.Параметры.СуммаИтого / ОбластьСтрокаОС.Параметры.СуммаПланПредыдущийГод *100;
				иначе
					ОбластьСтрокаОС.Параметры.СуммаИтогоФакт = 0;
					КонецЕсли;
				    КонецЕсли;
