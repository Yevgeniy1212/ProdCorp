...
Функция ПолучитьТаблицуДанных(Отбор,мГруппировки)
	...
	Для Каждого Измерение Из мГруппировки Цикл
		...
		ИначеЕсли Имя = "Валюта" ИЛИ Имя="Организация" Тогда
			ТекстИзмерения = ТекстИзмерения+",
			|	Обороты."+Имя+" КАК "+Имя;
		ИначеЕсли ТипЗнч(Измерение.Измерение)=Тип("ПеречислениеСсылка.фин_ФактическиеПоказателиБюджетирования") Тогда
			...
			Запрос.УстановитьПараметр("ПустоеЗначение"+Имя,фин_ОбщегоНазначенияВызовСервераПовтИсп.ТипЗначенияГруппировки(Измерение.Измерение).ПривестиЗначение(Неопределено));
			ТекстИзмерения = ТекстИзмерения+",
			|	"+ПолеИзмерение(Имя);
	...
	Для Каждого СтрокаДанных Из ДанныеДляАнализа Цикл
		...
		Запрос.УстановитьПараметр("ОкончаниеПланирования"+МеткаУровня,		КонецДня(СтрокаДанных.ОкончаниеПериода));
		...
		|ВЫБРАТЬ
	               ...
	               |				&НачалоПланированияМеткаУровня,
	               |				&ОкончаниеПланированияМеткаУровня,
	               |				Регистратор,
	               ...
	               |			ИЛИ Обороты.Регистратор.Дата < &ГраницаАктуальности"+?(ТекстОтбораОстатки="",""," И "+ТекстОтбораОстатки)+")"+?(ТекстОтбораИтоги="",""," И "+ТекстОтбораИтоги),"МеткаУровня",МеткаУровня);
	КонецЦикла;
...
Процедура Скомпоновать(ДокументРезультат,Отбор) Экспорт
	...
	Для Каждого СтрокаОтбор Из Отбор Цикл
		СтрокаОтбор.Путь = СтрЗаменить(фин_ОбщегоНазначенияСервер.ПолучитьИмяЭлементаПеречисленияПоЗначению(СтрокаОтбор.ПолеОтбора),"ФинансовыйПоказатель","Счет");
	КонецЦикла;
	...
	Заголовок=фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина("СравнительныйАнализОборотов",ЯзыкОтчета,,КЭШ)+": "+фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина(СтатьяБаланса,ЯзыкОтчета,,КЭШ)+Символы.ПС+фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина(ЗаголовокОтчета,ЯзыкОтчета,,КЭШ);
...
ДополнительныеПредставления.Добавить(Перечисления.фин_ФактическиеПоказателиБюджетирования.ФинансовыйПоказатель,"Статья баланса");
