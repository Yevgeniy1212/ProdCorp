...
Процедура СформироватьНаСервере()
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ГруппированныеДанные.СтатьяБюджета КАК СтатьяБюджета,
		|	СУММА(ВЫБОР
		...
		|		КОНЕЦ) КАК СуммаИтого,
		|	ГруппированныеДанные.СтатьяБюджета.КодБО КАК КодСтроки,
		...
		|ГДЕ
		|	ГруппированныеДанные.СтатьяБюджета В ИЕРАРХИИ(&КапитальныеВложения)
		|
		|СГРУППИРОВАТЬ ПО
		|	ГруппированныеДанные.СтатьяБюджета,
		|	ГруппированныеДанные.СтатьяБюджета.КодБО,
		|	ГруппированныеДанные.ИсточникФинансирования
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|ИЗ
		|	Справочник.фин_СтатьиБюджета КАК бит_СтатьиОборотов,
		|	ГруппированныеДанные КАК ГруппированныеДанные
		|ГДЕ
		|	бит_СтатьиОборотов.Ссылка В ИЕРАРХИИ(&КапитальныеВложения)
		|	И ГруппированныеДанные.СтатьяБюджета В ИЕРАРХИИ(&КапитальныеВложения)
		|
		|СГРУППИРОВАТЬ ПО
		|	бит_СтатьиОборотов.Ссылка,
		|	бит_СтатьиОборотов.КодБО
	...
	Если ЭтотОбъект.Отчет.ПоКварталам = Истина Тогда
		Макет = Отчеты.битОтчетПоБюджету8БО.ПолучитьМакет("ПоКварталам");
	Иначе
		Макет = Отчеты.битОтчетПоБюджету8БО.ПолучитьМакет("Макет");
	КонецЕсли;
	...
	Пока ВыборкаОбщийИтог.Следующий() Цикл		
		...
		Пока ВыборкаСтатьяБюджета.Следующий() Цикл
			...
			Если ВыборкаСтатьяБюджета.СтатьяБюджета.Код = "000000001" ИЛИ ВыборкаСтатьяБюджета.СтатьяБюджета.Код = "УД0000003" ИЛИ МассивСтатей.Найти(ВыборкаСтатьяБюджета.СтатьяБюджета) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			МассивСтатей.Добавить(ВыборкаСтатьяБюджета.СтатьяБюджета);
			
			Если ВыборкаСтатьяБюджета.СтатьяБюджета.Наименование = "Инвестиции" Тогда
			...
			Иначе
				...
				ОбластьГруппа.Параметры.КодСтроки = ВыборкаСтатьяБюджета.СтатьяБюджета.КодБО;    
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Если ВыборкаДетальныеЗаписи.СтатьяБюджета.Наименование = "Инвестиции" Тогда
				...
				Если ВыборкаДетальныеЗаписи.СтатьяБюджета.Родитель.Наименование = "Инвестиции" Тогда
...
Процедура РасчетИтога(СтатьяБюджета, Макет, Результат)
	...
	Запрос.Текст = 
		...
		|	СУММА(Итог.СуммаЗаДевятьМесяцев) КАК СуммаЗаДевятьМесяцев,
		|	СУММА(Итог.СуммаИтого) КАК СуммаИтого,
		|	СУММА(Итог.СуммаИтогоФакт) КАК СуммаИтогоФакт,
		|	СУММА(Итог.СуммаПредществующий) КАК СуммаПредществующий,
		|	Итог.КодСтроки КАК КодСтроки,
		|	СУММА(Итог.СуммаПервПериод) КАК СуммаПервПериод,
		|	СУММА(Итог.СуммаФактТриГодаНазад) КАК СуммаФактТриГодаНазад,
		|	СУММА(Итог.СуммаПредществующийГод) КАК СуммаПредществующийГод,
