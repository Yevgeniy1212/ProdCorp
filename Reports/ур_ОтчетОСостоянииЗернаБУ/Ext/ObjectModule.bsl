...
Функция ПолучитьВыборку()
	...
	|	ВЫБОР КОГДА ур_ЗерноНаСкладахОстатки.Класс.Класс = Значение(Перечисление.ур_Классность.ПервыйКласс)
	|	  Тогда
	...
	|	ВЫБОР КОГДА ур_ЗерноНаСкладахОстатки.Класс.Класс = Значение(Перечисление.ур_Классность.ВторойКласс)
	|	  Тогда
	|   	ВЫБОР КОГДА &ЕдиницаИзмерения = Значение(Перечисление.ур_ВариантыВыводаОтчетовПоУчетуЗерна.килограммы)
	|		  Тогда ЕстьNull(ур_ЗерноНаСкладахОстатки.ЗачтенныйВесОстаток,0)*1000
	|		  Иначе ЕстьNull(ур_ЗерноНаСкладахОстатки.ЗачтенныйВесОстаток,0)
	...
	|	ВЫБОР КОГДА ур_ЗерноНаСкладахОстатки.Класс.Класс = Значение(Перечисление.ур_Классность.ТретийКласс)
	|	  Тогда
	|   	ВЫБОР КОГДА &ЕдиницаИзмерения = Значение(Перечисление.ур_ВариантыВыводаОтчетовПоУчетуЗерна.килограммы)
	|		  Тогда ЕстьNull(ур_ЗерноНаСкладахОстатки.ЗачтенныйВесОстаток,0)*1000
	|		  Иначе ЕстьNull(ур_ЗерноНаСкладахОстатки.ЗачтенныйВесОстаток,0)
	...
	|	ВЫБОР КОГДА ур_ЗерноНаСкладахОстатки.Класс.Класс = Значение(Перечисление.ур_Классность.ЧетвертыйКласс)
	|	  Тогда
	|   	ВЫБОР КОГДА &ЕдиницаИзмерения = Значение(Перечисление.ур_ВариантыВыводаОтчетовПоУчетуЗерна.килограммы)
	|		  Тогда ЕстьNull(ур_ЗерноНаСкладахОстатки.ЗачтенныйВесОстаток,0)*1000
	|		  Иначе ЕстьNull(ур_ЗерноНаСкладахОстатки.ЗачтенныйВесОстаток,0)
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Отчет о состоянии зерна " + " на " + Формат(Период, "ДФ=dd.MM.yyyy")+" ("+ЕдиницаИзмеренияОтчета+")";
