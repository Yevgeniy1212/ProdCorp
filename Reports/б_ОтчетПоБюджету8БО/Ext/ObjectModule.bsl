...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	Если Поле <> Неопределено Тогда
		Поле.Заголовок = "План "+Год( НачалоДня(Дата(Год(НачалоПериода),1,1)))+" года к факту "  +Год(НачалоДня(Дата(Год(НачалоПериода)-1,1,1)))+" года,%";
	КонецЕсли;
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Инвестиционный бюджет"  + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
