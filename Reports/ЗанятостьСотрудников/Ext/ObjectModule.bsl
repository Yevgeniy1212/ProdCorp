...
Процедура ДоработатьКомпоновщикПередВыводом() Экспорт
	...
	ПараметрПериодЗначение = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Период"));
	
	Если ПараметрПериодЗначение = Неопределено Тогда
		Возврат;
	Иначе
		ПараметрПериод = ?(ПараметрПериодЗначение.Значение <> Неопределено, Дата(ПараметрПериодЗначение.Значение), '00010101');
		Если ПараметрПериод = '00010101' Тогда
			ПараметрПериод = НачалоМесяца(ТекущаяДата());
		КонецЕсли;
		ПараметрПериодЗначение.Использование = Истина;
		ПараметрПериодЗначение.Значение 	 = ПараметрПериод;
	КонецЕсли;
