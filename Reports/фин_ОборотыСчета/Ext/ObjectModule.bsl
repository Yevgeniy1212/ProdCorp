...
Процедура Скомпоновать(ДокументРезультат,Отбор) Экспорт
	...
	КонецЦикла;
		...
		ВыбранныеПоляДляТекущей.Использование=Истина;
	Если НЕ НачальноеСальдоДт Тогда
	...
	КонецЕсли;
	Если НЕ НачальноеСальдоКт Тогда
		Отбор = ПолеКорсчет.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		Отбор.Использование = Истина;
		Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Порядок");
	...
	КонецЕсли;
	Если НЕ ОборотыДт Тогда
		ЗначениеОтбора = Новый СписокЗначений;
		ЗначениеОтбора.Добавить(3);
		ЗначениеОтбора.Добавить(4);
		Отбор = ПолеКорсчет.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	...
	КонецЕсли;
	Если НЕ ОборотыКт Тогда
		ЗначениеОтбора = Новый СписокЗначений;
		ЗначениеОтбора.Добавить(5);
		ЗначениеОтбора.Добавить(6);
		Отбор = ПолеКорсчет.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке;
		Отбор.Использование = Истина;
		Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Порядок");
		Отбор.ПравоеЗначение = ЗначениеОтбора;
	КонецЕсли;
	Если НЕ КонечноеСальдоДт Тогда
		Отбор = ПолеКорсчет.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		Отбор.Использование = Истина;
		Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Порядок");
		Отбор.ПравоеЗначение = 7;
	КонецЕсли;
	Если НЕ КонечноеСальдоКт Тогда
		Отбор = ПолеКорсчет.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
		Отбор.Использование = Истина;
		Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Порядок");
		Отбор.ПравоеЗначение = 8;
	КонецЕсли;
	...
	Если ПоКорСубконто Тогда
		Для Инд = 1 По фин_ОбщегоНазначенияВызовСервераПовтИсп.ЗначениеПеременной("МаксимальноеКоличествоСубконто") Цикл
			...
			Отбор.Использование = Истина;
			Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КорСубконто"+Строка(Инд));
			...
			Если ЗначениеЗаполнено(ПериодичностьОтчета) Тогда
                ...
                Если Инд<>5 Тогда
					...
					Отбор.Использование = Истина;
					Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КорСубконто"+Строка(Инд+1));
				КонецЕсли;
...
ЗаполнениеПараметров.Вставить("НачальноеСальдоДт",Истина);
ЗаполнениеПараметров.Вставить("НачальноеСальдоКт",Истина);
ЗаполнениеПараметров.Вставить("ОборотыДт",Истина);
ЗаполнениеПараметров.Вставить("ОборотыКт",Истина);
ЗаполнениеПараметров.Вставить("КонечноеСальдоДт",Истина);
ЗаполнениеПараметров.Вставить("КонечноеСальдоКт",Истина);
ЗаполнениеПараметров.Вставить("ЯзыкОтчета",фин_ОбщегоНазначенияСервер.ОсновнойЯзыкОтчетов());

ДополнительныеПараметры = Новый Массив;
ДополнительныеПараметры.Добавить("НачальноеСальдоДт");
ДополнительныеПараметры.Добавить("НачальноеСальдоКт");
ДополнительныеПараметры.Добавить("ОборотыДт");
ДополнительныеПараметры.Добавить("ОборотыКт");
ДополнительныеПараметры.Добавить("КонечноеСальдоДт");
ДополнительныеПараметры.Добавить("КонечноеСальдоКт");
