...
//	Если ЯзыкЗаполнения = "Русский" Тогда
		Макет = Отчеты.фин_ПрогнозныйОтчетОДоходахИРасходах.ПолучитьМакет("Макет2016");
	//Иначе
...
Процедура ЗаполнитьАвто(ДатаНачала, ДатаОкончания, Ячейка)
	...
	ТаблицаСтатей = фин_УправлениеБюджетнойМоделью.ПолучитьТаблицуПостатейногоСоставаБюджетов(,Перечисления.фин_КлассификаторБюджетов.БюджетДоходовИРасходов);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			И (фин_НастройкаСоответствийСтатейБюджетовПозициямОтчетовПоФинансовымБюджетам.КлассификаторБюджетов = ЗНАЧЕНИЕ(Перечисление.фин_КлассификаторБюджетов.БюджетДоходовИРасходов))
	...
	Если ВидДанных=Перечисления.фин_ВидыДанныхОтчета.ФактическиеСУчетомПрогнозных Тогда
       ...
       |	ВТ_СтатьиСЗаполненнымиПозициями.ПозицияКлассификатора КАК ДвижениеДенежныхСредств
       |ИЗ
       |	РегистрНакопления.фин_ПрогнозИсполненияБюджетов.Обороты(&ГраницаФактическихИПрогнозныхДанных
       |			,
       |			&ГраницаАктуальности,
       |			,
       |			Сценарий = &Сценарий
       |				И Организация = &Организация
       |				И ПериодПланирования >= &НачалоПериода  И ПериодПланирования >=&ГраницаФактическихИПрогнозныхДанных
       |				И ПериодПланирования <= &КонецПериода
       |				И ФинансовыйПоказатель В
       |					(ВЫБРАТЬ
       |						ВТ_СтатьиСЗаполненнымиПозициями.ФинансовыйПоказатель
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		КодСтрокиОтчета = Отчеты.фин_ПрогнозныйОтчетОДоходахИРасходах.КодСтрокиОтчета(ВыборкаДанных.ДвижениеДенежныхСредств);
	...
	МассивКВыводу = Новый Массив;
	МассивКВыводу.Добавить("010");
	МассивКВыводу.Добавить("011");
	МассивКВыводу.Добавить("013");
	МассивКВыводу.Добавить("014");
	МассивКВыводу.Добавить("015");
	МассивКВыводу.Добавить("016");
	МассивКВыводу.Добавить("021");
	МассивКВыводу.Добавить("022");
	МассивКВыводу.Добавить("023");
	МассивКВыводу.Добавить("024");
	МассивКВыводу.Добавить("025");
	МассивКВыводу.Добавить("021");
	МассивКВыводу.Добавить("101");
	МассивКВыводу.Добавить("201");
	МассивКВыводу.Добавить("410");
	МассивКВыводу.Добавить("411");
	МассивКВыводу.Добавить("412");
	МассивКВыводу.Добавить("413");
	МассивКВыводу.Добавить("414");
	МассивКВыводу.Добавить("415");
	МассивКВыводу.Добавить("416");
	МассивКВыводу.Добавить("417");
	МассивКВыводу.Добавить("418");
	МассивКВыводу.Добавить("419");
	МассивКВыводу.Добавить("420");
	
	Для Каждого Показатель Из МассивКВыводу Цикл
		УстановитьЗначениеПоказателя("Б"+Показатель+"_" + Ячейка);
	КонецЦикла;
	...
	Отчеты.фин_ПрогнозныйОтчетОДоходахИРасходах.Расчет(ПолеРезультата);  
...
ЗаполнениеПараметров.Вставить("ТочностьЕдиницыИзмерения",2);
	...
	Запрос.Текст = 
		...
		|	И фин_Бюджеты.ПозицияКлассификатора = ЗНАЧЕНИЕ(Перечисление.фин_КлассификаторБюджетов.БюджетДоходовИРасходов)";
