...
Функция СформироватьТекстЗапроса()
	...
	|	ЕСТЬNULL(РасчетПереоценкиВалютныхСредств.СуммаВТенге, 0) КАК СуммаВТенге,
	...
	|	И РасчетПереоценкиВалютныхСредств.СтруктурноеПодразделение = &СтруктурноеПодразделение
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период,
	|	СчетУчета
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь, НачалоПодписи, ПоказыватьПодписи) Экспорт
		...
		Пока ВыборкаПериод.Следующий() Цикл
			...
			СведенияОбОрганизации 	 	  = ОбщегоНазначения.СведенияОЮрФизЛице(СтруктурнаяЕдиницаОрганизация, , , );
			...
			Пока ВыборкаСчетУчета.Следующий() Цикл
				...
				Пока ВыборкаВалюта.Следующий() Цикл
					...
					Пока Выборка.Следующий() Цикл
						...
						ОбластьСтрока.Параметры.СуммаВТенге = Выборка.СуммаВТенге;
						...
						ОбластьСтрока.Параметры.СуммаВРубляхПослеПереоценки = Выборка.СуммаВТенге + Переоценка;
						...
						ИтогСуммаДо         = ИтогСуммаДо + Выборка.СуммаВТенге;
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	        ...
	        |ИЗ
	        |	РегистрБухгалтерии.Типовой.Обороты(, , Месяц, Счет В ИЕРАРХИИ (&Счет )И Счет.Валютный, , {Валюта.*}, , ) КАК ТиповойОбороты
	        |ИТОГИ
	        |	СУММА(СуммаОборотКт)
	        |ПО
	        |	ОБЩИЕ
...
Массив = Новый Массив;
Массив.Добавить(ПланыСчетов.Типовой.ДенежныеСредства);
