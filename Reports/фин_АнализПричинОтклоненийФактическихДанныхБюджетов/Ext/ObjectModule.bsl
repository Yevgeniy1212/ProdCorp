...
Процедура ЗаполнитьСпискиПоказателейИГруппировок() Экспорт
	...
	ДополнительныеГруппировки.Добавить(Перечисления.фин_ДополнительныеРазрезыОтчетов.ПричинаОтклоненияФактическихДанных,"Причина отклонения");
	ДополнительныеГруппировки.Добавить(Перечисления.фин_ДополнительныеРазрезыОтчетов.ДокументРассмотрения,"Документ рассмотрения");
	ДополнительныеГруппировки.Добавить(Перечисления.фин_ДополнительныеРазрезыОтчетов.ВидВлиянияНаФинансовыйРезультат,"Вид влияния на финансовый результат");
	ДополнительныеГруппировки.Добавить(Перечисления.фин_ДополнительныеРазрезыОтчетов.ВидОтклонения,"Вид отклонения");
	ДополнительныеГруппировки.Добавить(Перечисления.фин_ДополнительныеРазрезыОтчетов.СтруктурноеОтклонение,"Структурное отклонение");
	ДополнительныеГруппировки.Добавить(Перечисления.фин_ДополнительныеРазрезыОтчетов.ВидСтруктурногоОтклонения,"Вид структурного отклонения");
	ДополнительныеГруппировки.Добавить(Перечисления.фин_ДополнительныеРазрезыОтчетов.ОтветственноеЛицо,"Ответственное лицо");
...
Функция ПолучитьТаблицуДанных(мПериодичность,Отбор,мГруппировки)
	...
	ОбработатьФрагментЗапроса(ТекстИзмерения,Истина);
	...
	ОбработатьФрагментЗапроса(ТекстОтбораОбороты);
	ОбработатьФрагментЗапроса(ТекстФильтраОбороты);
	...
	ОбработатьФрагментЗапроса(ТекстОтбораИтоговыеОбороты);
	...
	ТекстПоказателиБазовыеСПсевдонимом = СтрЗаменить(ТекстПоказателиСПсевдонимом,"Обороты.","ОборотыПлан.");
	ТекстПоказателиБазовыеСПсевдонимом = СтрЗаменить(ТекстПоказателиБазовыеСПсевдонимом,"Оборот,","ОборотПлан,");
	ТекстПоказателиБазовыеСПсевдонимом = ТекстПоказателиБазовыеСПсевдонимом + ?(ТекстПоказателиБазовыеСПсевдонимом="","","План");
	ТекстФормированияВременныхТаблиц ="	
	|ВЫБРАТЬ
	|	Обороты.ФинансовыйПоказатель КАК Финансовыйпоказатель"+ТекстИзмерения+ТекстПоказателиСПсевдонимом+?(РассчитыватьЗначимостьОтклонений,ТекстПоказателиБазовыеСПсевдонимом,"")+"
	|ПОМЕСТИТЬ ВТ_ПредварительныеОбороты
	|ИЗ РегистрНакопления.фин_АнализОтклоненийФактическихДанных.Обороты(,&ГраницаАктуальности, ,ВЫБОР КОГДА &Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) ТОГДА ИСТИНА ИНАЧЕ Организация = &Организация КОНЕЦ И 
	...
	Если РассчитыватьЗначимостьОтклонений Тогда
		ТекстФильтраОборотыПлан = "";
		ТекстОтбораОборотыПлан = "";
		фин_УправлениеОтчетамиБюджетирование.ЗаполнитьТекстУсловийОтбора(Отбор,ТекстОтбораОборотыПлан,,ТекстФильтраОборотыПлан,Новый Структура("СтатьяБюджета,ДокументРассмотрения,ОтветственноеЛицо,ВидВлиянияНаФинансовыйРезультат,ВидСтруктурногоОтклонения,СтруктурноеОтклонение,ВидОтклонения,ПричинаОтклоненияФактическихДанных"));
		ТекстСоединенияПоИзмерениям = фин_УправлениеОтчетамиБюджетирование.ПолучитьФрагментСоединенияЗапросаПоГруппировкам("Обороты","ОборотыПлан",Новый Структура("СтатьяБюджета,ДокументРассмотрения,ОтветственноеЛицо,ВидВлиянияНаФинансовыйРезультат,ВидСтруктурногоОтклонения,СтруктурноеОтклонение,ВидОтклонения,ПричинаОтклоненияФактическихДанных"),мГруппировки,Показатели,ОсновнаяГруппировка,мПериодичность);
		ТекстФормированияВременныхТаблиц =ТекстФормированияВременныхТаблиц + "	
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.фин_ОборотыБюджетов.Обороты(,&ГраницаАктуальности, ,ВЫБОР КОГДА &Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) ТОГДА ИСТИНА ИНАЧЕ Организация = &Организация КОНЕЦ И 
		|																							Сценарий = &Сценарий И ПериодПланирования>=&НачалоПланирования И ПериодПланирования<=&ОкончаниеПланирования
		|																							И ФинансовыйПоказатель В (&Показатели)"+?(ТекстФильтраОборотыПлан="",""," И "+ТекстФильтраОборотыПлан)+") КАК ОборотыПлан
		|			ПО Обороты.ФинансовыйПоказатель = ОборотыПлан.Финансовыйпоказатель"+?(ТекстСоединенияПоИзмерениям="",""," И ")+ТекстСоединенияПоИзмерениям;
		ТекстПоказателиПланДляВТ = фин_УправлениеОтчетамиБюджетирование.ПолучитьФрагментЗапросаПоказатели(Показатели,ТекстРазрядность,ТекстРазрядностьКоличество,"ОборотПлан","ОборотПлан");
		ТекстПоказателиПлан = фин_УправлениеОтчетамиБюджетирование.ПолучитьФрагментЗапросаПоказатели(Показатели,ТекстРазрядность,ТекстРазрядностьКоличество,"План","ОборотПлан");
	КонецЕсли;
	ТекстФормированияВременныхТаблиц =ТекстФормированияВременныхТаблиц + "	
	|;
	|ВЫБРАТЬ
	|	Обороты.ФинансовыйПоказатель КАК Финансовыйпоказатель"+СтрЗаменить(ТекстИзмерения,"ДополнительныеРазрезы.","")+ТекстПоказателиСПсевдонимом+?(РассчитыватьЗначимостьОтклонений,ТекстПоказателиПланДляВТ,"")+"
	...
	|	СтатьиБюджета.КодСтрокиБюджета КАК КодСтрокиБюджета"+ТекстПоказатели+?(РассчитыватьЗначимостьОтклонений,ТекстПоказателиПлан,"")+СтрЗаменить(ТекстИзмерения,"ДополнительныеРазрезы.","")+"
	|ИЗ
	|	Справочник.фин_СтатьиБюджета КАК СтатьиБюджета
...
Процедура ОбработатьФрагментЗапроса(Фрагмент,ТолькоПоляТаблицы = Ложь)
	Фрагмент = СтрЗаменить(Фрагмент,?(ТолькоПоляТаблицы,"Обороты.","")+"ОтветственноеЛицо",?(ТолькоПоляТаблицы,"Обороты.","")+"ДокументРассмотрения.ОтветственныйЗаОтклонение");	
	Фрагмент = СтрЗаменить(Фрагмент,?(ТолькоПоляТаблицы,"Обороты.","")+"ВидВлиянияНаФинансовыйРезультат",?(ТолькоПоляТаблицы,"Обороты.","")+"ПричинаОтклоненияФактическихДанных.ВидВлиянияНаФинансовыйРезультат");	
	Фрагмент = СтрЗаменить(Фрагмент,?(ТолькоПоляТаблицы,"Обороты.","")+"ВидСтруктурногоОтклонения",?(ТолькоПоляТаблицы,"Обороты.","")+"ПричинаОтклоненияФактическихДанных.ВидСтруктурногоОтклонения");	
	Фрагмент = СтрЗаменить(Фрагмент,?(ТолькоПоляТаблицы,"Обороты.","")+"СтруктурноеОтклонение",?(ТолькоПоляТаблицы,"Обороты.","")+"ПричинаОтклоненияФактическихДанных.СтруктурноеОтклонение");	
	Фрагмент = СтрЗаменить(Фрагмент,?(ТолькоПоляТаблицы,"Обороты.","")+"ВидОтклонения",?(ТолькоПоляТаблицы,"Обороты.","")+"ПричинаОтклоненияФактическихДанных.ВидОтклонения");	
КонецПроцедуры

Функция ПолучитьСКД(мГруппировки)
	//получаем схему компоновки данных
	СхемаКомпоновкиДанных=фин_УправлениеОтчетамиБюджетирование.ПолучитьСКД(ЭтотОбъект,мГруппировки,СтруктураПеревода,ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных"),КЭШ,"фин_АнализПричинОтклоненийФактическихДанныхБюджетов");
	Возврат СхемаКомпоновкиДанных;
КонецФункции

Функция ПолучитьНастройкиСКД()
	Настройки = фин_УправлениеОтчетамиБюджетирование.ПолучитьНастройкиСКД(ЭтотОбъект,КомпоновщикНастроек,СтруктураПеревода,КЭШ,фин_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьПериодичностьСценария(Сценарий),"фин_АнализПричинОтклоненийФактическихДанныхБюджетов",,Истина);
	Возврат Настройки;
КонецФункции
...
Процедура Скомпоновать(ДокументРезультат,Отбор) Экспорт
	...
	Заголовок=фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина("АнализПричинОтклоненийФактическихДанныхБюджетов",ЯзыкОтчета,,КЭШ)+": "+фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина(Бюджет,ЯзыкОтчета,,КЭШ)+Символы.ПС+фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина("Сценарий",ЯзыкОтчета,,КЭШ)+": "+фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина(Сценарий,ЯзыкОтчета,,КЭШ)+Символы.ПС+
