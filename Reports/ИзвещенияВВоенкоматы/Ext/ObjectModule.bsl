...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|	ВЫРАЗИТЬ(Призывники.Организация.НаименованиеПолное КАК СТРОКА(1000)) КАК НаименованиеОрганизации,
	...
	|			КОНЕЦ КАК Событие,
	|			ВЫБОР
	|				КОГДА ВоинскийУчетСрезПоследних.ОтношениеКВоинскойОбязанности = &Призывник
	|					ТОГДА ""гражданин, подлежащий призыву на военную службу""
	|				ИНАЧЕ ВоинскийУчетСрезПоследних.Звание.Наименование + "" запаса; ВУС № "" + ВоинскийУчетСрезПоследних.ВУС
	|			КОНЕЦ КАК ТекстЗвание,
	|			ВЫБОР
	|				КОГДА РаботникиОрганизации.ПричинаИзмененияСостояния = &Прием
	|					ТОГДА ""на должность "" + РаботникиОрганизации.Должность.Наименование
	|				ИНАЧЕ """"
	|			КОНЕЦ КАК ДолжностьРаботника
	|		ИЗ
	...
	|			И ВоинскийУчетСрезПоследних.ОтношениеКВоинскойОбязанности В(&Военнообязанные)
	...
	|			РаботникиОрганизации.Сотрудник.Физлицо.*}) КАК Призывники
	...
	|		ПО Призывники.Организация = АдресОрганизации.Объект
	|			И (АдресОрганизации.Тип = &Адрес)
	|			И (АдресОрганизации.Вид = &АдресОрганизации)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК ТелефонОрганизации
	|		ПО Призывники.Организация = ТелефонОрганизации.Объект
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	ПостроительОтчета.Параметры.Вставить("СреднееОбщееОбразование", Справочники.ВидыОбразованияФизЛиц.СреднееОбщееОбразование);
	...
	ПостроительОтчета.Параметры.Вставить("ДомашнийТелефон", Справочники.ВидыКонтактнойИнформации.ТелефонФизЛица);
