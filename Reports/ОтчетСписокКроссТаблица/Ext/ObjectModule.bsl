...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка, СтандартнаяОбработка, ЭтотОтчет) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Иначе
            ...
            Если Отчет.ПолучитьПостроительОтчета().ИзмеренияСтроки.Найти(Выбор.Значение) = Неопределено Тогда
		...
		КонецЕсли;
		Форма.Открыть();
		Форма.ДатаНач = ЭтотОбъект.ДатаНач;
		Форма.ДатаКон = ЭтотОбъект.ДатаКон;
	КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь, УказыватьРасшифровки = Истина) Экспорт
	...
	СтолбецНумерации = ?(ВыводитьНумерацию = Истина, 1, 2);	
	ДопСтолбецПотребностьВОбучении = ?(ЭтотОбъект.ИмяРегистра = "ПотребностиВОбучении",1,0);
	// Печатаемая область
	ДокументРезультат.ОбластьПечати = ДокументРезультат.Область(1,СтолбецНумерации,ДокументРезультат.ВысотаТаблицы,ШиринаТаблицы+1+ДопСтолбецПотребностьВОбучении);
	...
	//Вывод столбца нумерации
	Если ВыводитьНумерацию = Истина Тогда
		
		РезультатЗапроса = ПостроительОтчета.Результат.Выбрать();
		Счетчик = 1;
		МассивНумерации = Новый Массив;
		
		Пока РезультатЗапроса.Следующий() Цикл
			Если РезультатЗапроса.ТипЗаписи() <> ТипЗаписиЗапроса.ОбщийИтог Тогда
				Если РезультатЗапроса.ТипЗаписи() = ТипЗаписиЗапроса.ДетальнаяЗапись Тогда
					
					МассивНумерации.Добавить(Строка(Счетчик));
					Счетчик = Счетчик + 1;
					
				Иначе
					
					МассивНумерации.Добавить("");
					Счетчик = 1;
		...
		//Вывод столбца нумерации 
		СтрокаДляВывода = ПостроительОтчета.МакетЗаголовкаОтчета.ВысотаТаблицы + ПостроительОтчета.МакетШапкиТаблицы.ВысотаТаблицы;
		ДокументРезультат.Область(СтрокаДляВывода, 1).Текст = "№";
		ДокументРезультат.Область(СтрокаДляВывода, 1).Шрифт = Новый Шрифт("Arial", 8, Истина);
		ДокументРезультат.Область(СтрокаДляВывода, 1).ГраницаСверху = ЛинияГраницы;
		ДокументРезультат.Область(СтрокаДляВывода, 1).ГраницаСлева  = ЛинияГраницы;
		ДокументРезультат.Область(СтрокаДляВывода, 1).ГраницаСнизу  = ЛинияГраницы;
		ДокументРезультат.Область(СтрокаДляВывода, 1).ЦветРамки 	  = ЦветаСтиля.ЦветРамки;
		
		Для Каждого СтрокаНумерации Из МассивНумерации Цикл
			
			СтрокаДляВывода = СтрокаДляВывода + 1;
			ДокументРезультат.Область(СтрокаДляВывода, 1).Текст = СтрокаНумерации;
			ДокументРезультат.Область(СтрокаДляВывода, 1).ГраницаСверху = ЛинияГраницы;
			ДокументРезультат.Область(СтрокаДляВывода, 1).ГраницаСлева 	= ЛинияГраницы;
			ДокументРезультат.Область(СтрокаДляВывода, 1).ГраницаСнизу 	= ЛинияГраницы;
			ДокументРезультат.Область(СтрокаДляВывода, 1).ЦветРамки 	= ЦветаСтиля.ЦветРамки;
		...
		Если ДокументРезультат.Области.Найти("ОбщиеИтоги") <> Неопределено Тогда
    ...
    УправлениеОтчетами.УстановитьКолонтитулыПоУмолчанию(ДокументРезультат, мНазваниеОтчета, Строка(глТекущийПользователь));	
...
мВыбиратьИмяРегистра = Истина; // По умолчанию выбирается
Если Метаданные.РегистрыСведений.Найти("ЗначенияСвойствОбъектов") <> Неопределено Тогда
	мВыбиратьИспользованиеСвойств = Истина; // по умолчанию выбирается
Иначе
	мВыбиратьИспользованиеСвойств = Ложь; // по умолчанию не выбирается
КонецЕсли;
