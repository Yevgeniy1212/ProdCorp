...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если Не ДанныеОтчета.Свойство("ДополнительныеПоля") Тогда
		...
		НоваяСтрока.Использование  = НЕ ВидОтчета = "НаличиеДокументовРеализации";			
	...
	Если Не ДанныеОтчета.Свойство("Группировка") Тогда
		...
		Если ВидОтчета = "НаличиеДокументовРеализации" Тогда
			НоваяСтрока = ДанныеОтчета.Группировка.Добавить();
			НоваяСтрока.Поле           = "СчетФактураРеализации";
			НоваяСтрока.Использование  = Истина;
			НоваяСтрока.Представление  = "Счет-фактура";
	...
	ТиповыеОтчеты.ДобавитьОтбор(КомпоновщикНастроек, "СчетФактураВыписан",Ложь,, Ложь); // 		
	ТиповыеОтчеты.ДобавитьОтбор(КомпоновщикНастроек, "СчетФактураПроведен",Ложь,, Ложь); // 
	ТиповыеОтчеты.ДобавитьОтбор(КомпоновщикНастроек, "СчетФактураРеализации.ВалютаДокумента", Справочники.Валюты.ПустаяСсылка(),, Ложь); // 	
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	Для Каждого ВыбранныйПоказатель Из ДанныеОтчета.ПоказателиОтчета Цикл 
		Если ВыбранныйПоказатель.Значение.Значение Тогда
			Если ВыбранныйПоказатель.Ключ = "СФСуммаНДСРегл" ИЛИ ВыбранныйПоказатель.Ключ = "СФОборотПоРеализацииРегл" Тогда
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Отчет по наличию выданных счетов-фактур" + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
...
Процедура ПолучитьСтруктуруПоказателейОтчета() Экспорт
	...
	ПоказателиОтчета = Новый Структура("СуммаБезНДСОборот, СуммаНДСОборот, СуммаВсего, СФСуммаНДСРегл, СФОборотПоРеализацииРегл", 
				...
				Новый Структура("Значение, Использование, Представление", Истина, Истина, "Сумма НДС регл. (счет-фактура)"),
				Новый Структура("Значение, Использование, Представление", Истина, Истина, "Оборот по реализации регл. (счет-фактура)"));
