...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Информация о полученных займах и начисленных процентах " + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
...
Функция ПолучитьВыборку()
	...
	|   ВЫБОР КОГДА Выборка.Договор.ВалютаВзаиморасчетов = &мВалютаРегламентированногоУчета
	|		ТОГДА 0
	|   	ИНАЧЕ Выборка.ВалСуммаПогашения
	|	КОНЕЦ КАК ВалСуммаПогашения,
	|   Выборка.СуммаПогашения-Выборка.СуммаКурсПриПог КАК СуммаПогашения,
	|   ВЫБОР КОГДА Выборка.Договор.ВалютаВзаиморасчетов = &мВалютаРегламентированногоУчета
	|		ТОГДА 0
