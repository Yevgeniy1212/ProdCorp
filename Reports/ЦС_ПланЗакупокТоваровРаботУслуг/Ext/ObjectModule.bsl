...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
			...
			ТиповыеОтчеты.УстановитьПараметр(КомпоновщикНастроек, "ПериодПланирования", Началогода(НачалоПериода));
		    ТиповыеОтчеты.УстановитьПараметр(КомпоновщикНастроек, "ОтбираемыеГруппыСтатейБюджета", ВключаемыеГруппыСтатейБюджета);

	ВнешниеНаборыДанных = Новый Структура;
//	ВыборкаДанных = ПолучитьВыборку();
//	ВнешниеНаборыДанных.Вставить("ТаблицаДанных", ВыборкаДанных);
	
	Поле = СхемаКомпоновкиДанных.НаборыДанных[0].Поля.Найти("ФактПозапрошлогоГода");
	Если Поле <> Неопределено Тогда
		Поле.Заголовок = "Факт "+Формат(Год(НачалоПериода)-2,"ЧГ=0")+" года";
	КонецЕсли;
	
	Поле = СхемаКомпоновкиДанных.НаборыДанных[0].Поля.Найти("ОценкаПредыдущегоГода");
	Если Поле <> Неопределено Тогда
		Поле.Заголовок = "Оценка "+Формат(Год(НачалоПериода)-1,"ЧГ=0")+" года";
	КонецЕсли;
	
	Поле = СхемаКомпоновкиДанных.НаборыДанных[0].Поля.Найти("Год");
	Если Поле <> Неопределено Тогда
		Поле.Заголовок = "План "+Формат(Год(НачалоПериода),"ЧГ=0")+" года";
	КонецЕсли;
    	
	Поле = СхемаКомпоновкиДанных.НаборыДанных[0].Поля.Найти("ОценкаПредыдущегоКПозапрошлому");
	Если Поле <> Неопределено Тогда
		Поле.Заголовок = "Оценка "+Формат(Год(НачалоПериода)-1,"ЧГ=0")+" года";
	КонецЕсли;
	
	Поле = СхемаКомпоновкиДанных.НаборыДанных[0].Поля.Найти("ПланТекущегоКОценкеПредыдущего");
	Если Поле <> Неопределено Тогда
		Поле.Заголовок = "План "+Формат(Год(НачалоПериода),"ЧГ=0")+" года к факту "+Формат(Год(НачалоПериода)-1,"ЧГ=0")+" года";
	КонецЕсли;
	
	
	//Предыдущий период
	//
	ТиповыеОтчеты.УстановитьПараметр(КомпоновщикНастроек, "АД", АД);
	ТиповыеОтчеты.УстановитьПараметр(КомпоновщикНастроек, "ЦентральныйАппарат",ЦентральныйАппарат);
...
//Заголовок = отчеты.ЦС_ПланЗакупокТоваровРаботУслуг.ПолучитьМакет("Заголовок");
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "План закупок товаров, работ и услуг, планируемых к приобретению " + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
