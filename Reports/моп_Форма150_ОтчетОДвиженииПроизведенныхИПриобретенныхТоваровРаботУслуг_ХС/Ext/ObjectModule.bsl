...
// Функция возвращает массив счетов учета номенклатуры
// используемых отчетом по умолчанию
//
// Возвращаемое значение:
//  Массив
//
Функция ПолучитьСчетаУчетаНоменклатурыПоУмолчанию() Экспорт
	
	ПС = ПланыСчетов.Типовой;
	Счета = Новый Массив;
	Счета.Добавить(ПС.СырьеИМатериалы);
	Счета.Добавить(ПС.ГотоваяПродукция);
	Счета.Добавить(ПС.Товары);
	Счета.Добавить(ПС.ПрочиеЗапасы);
	
	Возврат Счета;
	
КонецФункции

// Функция возвращает массив исключаемых счетов учета номенклатуры
// используемых отчетом по умолчанию
//
// Возвращаемое значение:
//  Массив
//
Функция ПолучитьИсключаемыеКорСчетаПоУмолчанию() Экспорт
	
	//ПС = ПланыСчетов.Типовой;
	//Счета = Новый Массив;
	//Счета.Добавить(ПС.КраткосрочнаяДебиторскаяЗадолженностьФилиаловИСтруктурныхПодразделений);
	//Счета.Добавить(ПС.КраткосрочнаяКредиторскаяЗадолженностьФилиаламИСтруктурнымПодразделениям);	
	
	ПС = ПланыСчетов.Типовой;
	Счета = Новый Массив;
	Счета.Добавить(ПС.КраткосрочнаяДебиторскаяЗадолженностьПредставительств);
	Счета.Добавить(ПС.КраткосрочнаяКредиторскаяЗадолженностьпредставительств);	
	Счета.Добавить(ПС.ПереоформлениеПереводМеждуРесурсами);	
	
	Возврат Счета;
	
КонецФункции

// Заполняет структуру форм отчета
//
...
Процедура ВыгрузитьДанныеВXML(ИмяФайла)  Экспорт
	...
	мСписокТегов = моп_РаботаСДиалогами.ПолучитьСписокТегов(150, ИмяФормы);	
...
Настройки.Вставить("СчетаУчетаНоменклатуры", ПолучитьСчетаУчетаНоменклатурыПоУмолчанию());
Настройки.Вставить("ИсключаемыеКорСчета", ПолучитьИсключаемыеКорСчетаПоУмолчанию());
Настройки.Вставить("ИсключитьТоварыБезОборотов", Ложь);
