...
Функция СформироватьЗаголовок() Экспорт
	...
	ОписаниеПериода = Отчеты_БК.СформироватьСтрокуВыводаПараметровПоДатам(ДатаНач, ДатаКон);
	...
	НазваниеОрганизации = РаботаСДиалогами.ВыгрузитьСписокВСтроку(мСписокСтруктурныхЕдиниц,,,Истина);
...
Функция ПолучитьТекстОбщегоЗапроса()
	...
	Отчеты_БК.ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграниченийПоРеквизитам, "СписокОрганизаций", мСписокСтруктурныхЕдиниц, "Организация");
	...
	|	СУММА(СуммаНачКт)
	|ПО
	|	Счет" + ?(ПоГруппамСчетов, " ИЕРАРХИЯ,", ",")
	+ ?(ПоПериодам,"Период"+?(ВсеПериоды, " ПЕРИОДАМИ("+СокрЛП(Период)+",&ДатаНач,&ДатаКон)", "") + ",","")+"
	|	КорСчет" + ?(ПоГруппамКорСчетов, " ИЕРАРХИЯ", "");
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	ОграничениеПоДатамКорректно = Отчеты_БК.ПроверитьКорректностьОграниченийПоДатам(ДатаНач, ДатаКон);
	...
	Отчеты_БК.СформироватьИВывестиЗаголовокОтчета(ЭтотОбъект, ДокументРезультат, ВысотаЗаголовка, ПоказыватьЗаголовок);
	
	// получим форматную строку для формирования наименований периодов
	ФорматПериода = Отчеты_БК.ПолучитьСтрокуФорматаПериода(Период);
	...
	СтруктураНастроекОтчета.Вставить("СписокОрганизаций", мСписокСтруктурныхЕдиниц);
	...
	СтандартныеОтчеты.ВыводПодписейОтчета(ЭтотОбъект, ДокументРезультат);
...
Процедура ВывестиШапкуТаблицы(СтруктураПараметров, Выборка, ШиринаВыводимогоРаздела)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ПоСубсчетамКорСчетов И ВерхнийУровень < Выборка.Уровень() Тогда		
			Продолжить;
		Иначе
		...
		Если ЗначениеЗаполнено(Период) Тогда
			...
			Пока ВыборкаПериод.Следующий() Цикл
				Если Отчеты_БК.ПривестиКЧислу(ВыборкаПериод.СуммаОборотДт)<>0 Тогда
		...
		Иначе
			...
			Если Отчеты_БК.ПривестиКЧислу(Выборка.СуммаОборотДт)<>0 Тогда				
				СтруктураПараметров.СписокДт.Добавить(Выборка.КорСчет);
...
Процедура ВывестиПодвалТаблицы(ОбщийИтог, СтруктураПараметров, Выборка)
	...
	Если Не СтруктураПараметров.РазвернутоеСальдо Тогда
		...
		СуммаНачДт = Отчеты_БК.ПривестиКЧислу(ВыборкаИтогПоСчету.СуммаНачДт);
		СуммаНачКт = Отчеты_БК.ПривестиКЧислу(ВыборкаИтогПоСчету.СуммаНачКт);
	...
	Если Не СтруктураПараметров.РазвернутоеСальдо Тогда
		...
		СуммаКонДт = СуммаНачДт + Отчеты_БК.ПривестиКЧислу(ОбщийИтог.СуммаОборотДт) - Отчеты_БК.ПривестиКЧислу(ОбщийИтог.СуммаОборотКт);
...
// заполняет таблицу развернутых остатков
//
Процедура ЗаполнитьТаблицуРазвернутыхОстатков(СтруктураПараметров)
	...
	Если СтруктураПараметров.РазвернутоеСальдо Тогда
		...
		+ТекстПолей+"
		|ИЗ
		|	РегистрБухгалтерии.Типовой.ОстаткиИОбороты(&ДатаНач, &ДатаКон, "+Период+", , Счет В ИЕРАРХИИ (&Счет), , " + СтрокаОграниченийПоРеквизитам + ") КАК ОстаткиИОбороты
		|
...
Процедура ВывестиПериоды(Выборка, СтруктураПараметров, ОтборДляРасшифровки, БазовыйУровеньВыборки=0)
	...
	Пока ВыборкаПоПериодам.Следующий() Цикл
		...
		Если ВыборкаПоПериодам.Период = NULL Тогда
			...
			СуммаНачДт = Отчеты_БК.ПривестиКЧислу(ВыборкаПоПериодам.СуммаНачДт);
			СуммаНачКт = Отчеты_БК.ПривестиКЧислу(ВыборкаПоПериодам.СуммаНачКт);
		...
		РасшифровкаДт = Отчеты_БК.СоздатьКопиюСоответствияСтруктуры(Расшифровка);
		...
		РасшифровкаКт = Отчеты_БК.СоздатьКопиюСоответствияСтруктуры(Расшифровка);
		...
		СуммаОборотДт = Отчеты_БК.ПривестиКЧислу(ВыборкаПоПериодам.СуммаОборотДт);
		СуммаОборотКт = Отчеты_БК.ПривестиКЧислу(ВыборкаПоПериодам.СуммаОборотКт);
		...
		Если СтруктураПараметров.РазвернутоеСальдо Тогда
			...
			Иначе
				Если ВыборкаПоПериодам.Период=NULL Тогда
					СуммаКонДт = Отчеты_БК.ПривестиКЧислу(СтрокаОстатков.СуммаРазвернутоНачДт);
					СуммаКонКт = Отчеты_БК.ПривестиКЧислу(СтрокаОстатков.СуммаРазвернутоНачКт);
				Иначе
					СуммаКонДт = Отчеты_БК.ПривестиКЧислу(СтрокаОстатков.СуммаРазвернутоКонДт);
					СуммаКонКт = Отчеты_БК.ПривестиКЧислу(СтрокаОстатков.СуммаРазвернутоКонКт);
				КонецЕсли;
			КонецЕсли;
...
Процедура ВывестиКорСчета(ВыборкаОборотыПоКорСчетам, СтруктураПараметров, ВидОборота, Ресурсы, ВыводимаяОбласть, РодительскаяРасшифровка=0)
	...
	Пока ВыборкаОборотыПоКорСчетам.Следующий() Цикл
		...
		// Выводим только верхний уровень
		Если НЕ ПоСубсчетамКорСчетов И ВерхнийУровень < ВыборкаОборотыПоКорСчетам.Уровень() Тогда		
			Продолжить;
		Иначе
		...
		Для каждого Элемент Из СтРесурсы Цикл
			...
			Если НЕ Отчеты_БК.ПривестиКЧислу(ВыборкаОборотыПоКорСчетам[Элемент.Ключ+ТипПоля])=0 и ВыборкаОборотыПоКорСчетам[Элемент.Ключ+ТипПоля]<>NULL Тогда
	...
	Для каждого ЭлементСчет Из СписокСчетов Цикл
		...
		Если ТипЗнч(РодительскаяРасшифровка) = Тип("Соответствие") Тогда
			Расшифровка = Отчеты_БК.СоздатьКопиюСоответствияСтруктуры(РодительскаяРасшифровка);
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ПоГруппамСчетов    = Истина;
	ПоГруппамКорСчетов = Истина;
...
Процедура Настроить(СтруктураПараметров) Экспорт
	...
	Параметры = Отчеты_БК.СоздатьПоСтруктуреСоответствие(СтруктураПараметров); 
...
//	ФормаЖурнала.Отбор.Организация.Установить(Расшифровка.Организация, ЗначениеЗаполнено(Организация));
	ФормаЖурнала.Отбор.Организация.Использование = Ложь;
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка, ПоказыватьЗаголовок=Истина) Экспорт
	...
	Если ИмяОтчетаРасшифровки = "типовой" Тогда
	...
	Иначе
		...
		Если Расшифровка.ИмяОбъекта = "ОборотыСчетаТиповой" Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		ФормаРасшифровка.мСписокСтруктурныхЕдиниц = мСписокСтруктурныхЕдиниц;
...
Отчеты_БК.СоздатьКолонкиУПравилВыводаИтоговИПравилаРазвернутогоСальдо(Неопределено, ПравилаРазвернутогоСальдо, ИмяРегистраБухгалтерии);
