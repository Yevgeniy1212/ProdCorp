...
Функция ЗаполнитьСведенияОНалогоплательщике()Экспорт
	...
	Если (Налогоплательщик  = Неопределено) Или (Налогоплательщик = ОбщегоНазначения.ПустоеЗначениеТипа("СправочникСсылка.Организации")) 
			Или мСписокСтруктурныхЕдиниц.Количество() <> 1 Тогда
		Возврат 0;
	КонецЕсли;
	
	///////////////////////////////////////////////////////
	// сведения о налогоплательщике - головной организациии
	...
	// налоговый комитет налогоплательщика
	НалоговыйКомитетНалогоплательщика = ПроцедурыНалоговогоУчета.ПолучитьНалоговыйКомитетСтруктурнойЕдиницы(Налогоплательщик, 
																					Налогоплательщик,
	...
	НКСведения = ОбщегоНазначения.СведенияОЮрФизЛице(НалоговыйКомитетНалогоплательщика, ДатаПодписи);
	
	ОргСведения.Вставить("КНОНалогоплательщика", СокрЛП(Лев(НКСведения.РНН, 4)));
				
																					
	///////////////////////////////////////////////////////
	// сведения о структурной единице
	
	СтруктурнаяЕдиница = мСписокСтруктурныхЕдиниц[0].Значение;
	
	Если ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.Организации") Тогда
	
		// организация
		ФилСведения = ОбщегоНазначения.ПолучитьСведенияОбОрганизации(СтруктурнаяЕдиница, ТекущаяДата(), Сведения);
		
		// добавляем в общую структуру результата
		ОргСведения.Вставить("НаимФил", ФилСведения.НаимЮЛПол);
		ОргСведения.Вставить("РННФил", ФилСведения.РННЮЛ);
		ОргСведения.Вставить("ФИОРук", ФилСведения.ФИОРук);
		ОргСведения.Вставить("ФИОБух", ФилСведения.ФИОБух);
		
	ИначеЕсли ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
	
		// подразделение организации
		ФилСведения = ОбщегоНазначения.ПолучитьСведенияОбОрганизации(СтруктурнаяЕдиница.Владелец, ТекущаяДата(), Сведения);

		// добавляем в общую структуру результата
		ОргСведения.Вставить("НаимФил", СтруктурнаяЕдиница.Наименование);
		ОргСведения.Вставить("РННФил", ФилСведения.РННЮЛ);
		ОргСведения.Вставить("ФИОРук", ФилСведения.ФИОРук);
		ОргСведения.Вставить("ФИОБух", ФилСведения.ФИОБух);
	...
	// налоговый комитет структурной единицы
	
	// Добавим сведения о кодах налоговых органов
	НКСведения = ОбщегоНазначения.СведенияОЮрФизЛице(НалоговыйКомитет, ДатаПодписи);
	
	ОргСведения.Вставить("КНОФилиала", СокрЛП(Лев(НКСведения.РНН, 4)));
...
ИмяПакета = "http://www.fno60101.rating.kz";
ИмяФайлаВыгрузки = "60101.efn";
