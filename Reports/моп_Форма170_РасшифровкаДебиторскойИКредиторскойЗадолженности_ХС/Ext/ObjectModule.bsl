...
// Хранит метод расчета
Перем мМетодРасчета Экспорт;

// Хранит признак многострочности формы
...
// печати указанных листов формы
// Параметры:
//  ВидПечати - строка, задающая способ печати:
//              "ПоказатьБланк" - с предварительным просмотром;
...
Процедура Печать(ВидПечати) Экспорт 
	...
	Иначе
	    ...
	    Пока Стр < мФорма.ВсегоСтраниц + 1 Цикл		 
		 	Для Каждого Страница Из мФорма.ЭлементыФормы["Панель" + мФорма.ИмяФормы].Страницы Цикл
				...
				мМногострочнаяСтруктура = РегламентированнаяОтчетность.ПолучитьМногострочнуюСтруктуру(мФорма.ИмяФормы,мСписокФормБезИерархии);
			 	...
			 	моп_Общий.ВывестиРазделВТабличныйДокументФормы(мФорма, Стр,ИмяГруппы, СтруктураГруппы,мФорма.ИмяФормы);
		...
		моп_Общий.ВывестиСтраницуМногострочнойФормы(мФорма);
...
// Устанавливает значение номера квартала
//
