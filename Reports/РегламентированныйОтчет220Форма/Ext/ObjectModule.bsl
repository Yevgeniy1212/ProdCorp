...
Функция ЗаполнитьСведенияОНалогоплательщике()Экспорт
	...
	Сведения.Добавить("", "ФИО"); // ФИО налогоплательщика
	...
	Если ЗначениеЗаполнено(Налогоплательщик.ИндивидуальныйПредприниматель) Тогда
		ФизЛицо = Налогоплательщик.ИндивидуальныйПредприниматель;
		...
		Запрос.Текст = 
			...
			|	КОНЕЦ КАК ФИО
			|ИЗ
			|	РегистрСведений.ФИОФизЛиц.СрезПоследних(&Период, ФизЛицо = &ФизЛицо) КАК ФИОФизЛиц";
		...
		Если НЕ Результат.Пустой() Тогда
			ВыборкаФИО = Результат.Выбрать();
			ВыборкаФИО.Следующий();
			ФИО = ВыборкаФИО.ФИО;
			ОргСведения.Вставить("ФИО", ФИО);
		КонецЕсли;
...
Процедура СформироватьРасшифровку(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина, ВнешниеНаборыДанных = Неопределено) Экспорт
	...
	НачалоПериода 	= ПолучитьПараметр(ЭтотОбъект.КомпоновщикНастроек, "НачалоПериода");
	КонецПериода	= ПолучитьПараметр(ЭтотОбъект.КомпоновщикНастроек, "КонецПериода"); 
...
ИмяПакета = "http://www.fno220.rating.kz";
ИмяФайлаВыгрузки = "220.efn";
