...
// Заполняет соответствие структуры групп
//
Функция ЗаполнитьСоответствиеСтруктурыГрупп(СтруктураСоответствияГрупп, ИмяФормы, Страницы) Экспорт
	
	Для Каждого Страница Из Страницы Цикл 	
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("ИмяФормыТаблДокумента", ИмяФормы); 
		СтруктураПоиска.Вставить("ИмяЛиста", Страница.Имя); 
		ТаблицаФормы = мТаблицаПоказателей.НайтиСтроки(СтруктураПоиска);
		
		Если ТаблицаФормы.Количество() = 0 Тогда
			Сообщить("Нет постраничных данных для данной формы. Форма не может быть открыта");
			Возврат Неопределено;
			Прервать;
		ИначеЕсли ТаблицаФормы[1].ИмяСтраницыФормы = Неопределено Тогда
			Сообщить("В показателях отчета не заданы имена страниц форм XML. Форма не может быть открыта");
			Возврат Неопределено;
			Прервать;
		Иначе
			мКлюч     = ТаблицаФормы[1].ИмяСтраницыФормы;
		КонецЕсли;
		СтруктураСоответствияГрупп.Вставить(мКлюч,Страница.Имя);
	КонецЦикла;
	Возврат  СтруктураСоответствияГрупп;
	...
	Если Область <> Неопределено Тогда
		...
		Если Лев(Область,5) = "Месяц" Тогда
			Если Значение = 0 Тогда
				...
				Возврат Значение;
			Иначе
				Значение = Формат(Значение, "ЧЦ=2; ЧВН=; ЧГ=");
				Возврат Значение;
			КонецЕсли;
		     	...
		     	//записываем данные из шапки документа					
					...
					Для Каждого Элемент Из таблицаТегов Цикл
						...
						Если Значение = Неопределено Тогда
							Значение = Формат(Стр,"ЧЦ=3; ЧВН=; ЧГ=");
...
Процедура ВыгрузитьДанныеВXML(ИмяФайла)  Экспорт
	...
	мСписокТегов = моп_РаботаСДиалогами.ПолучитьСписокТегов(160, ИмяФормы);	
...
мОсновнаяКонфигурация = Константы.моп_ОсновнаяКонфигурация.Получить();
