...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Для Каждого Поле Из Настройки.Выбор.Элементы Цикл
		...
		//Если Поле <> Неопределено Тогда
		//	Поле.Заголовок = "Оценка ПКК " + Формат(ТекущГод-1,"ЧГ=0")+" г";
		//КонецЕсли;
		//
		//Поле = СхемаКомпоновкиДанных.НаборыДанных[0].Поля.Найти("ФактПредыдущийГодДЗО");
		//Если Поле <> Неопределено Тогда
		//	Поле.Заголовок = "Оценка дочек " + Формат(ТекущГод-1,"ЧГ=0")+" г";
		//КонецЕсли;
		
		
		Для Каждого ПоляСх Из СхемаКомпоновкиДанных.НаборыДанных[0].Поля Цикл
			
			Если Прав(ПоляСх.ПутьКДанным,3) = "Ком" Тогда
				
				ПоляСх.Заголовок = "АО ""Ак Бидай - Терминал""";

			ИначеЕсли Прав(ПоляСх.ПутьКДанным,3) = "ДЗО" Тогда
				
				ПоляСх.Заголовок = "ТОО ""Астык коймалары""";  
			
			ИначеЕсли Прав(ПоляСх.ПутьКДанным,4) = "Элюм" Тогда
				
				ПоляСх.Заголовок = "ТОО ""Экспертная аграрная компания""";
