...
Функция ПолучитьРазмерКорректировки(ВидТранспортногоСредства, ХарактеристикиОбъекта, СтруктураСтавокНалога)	Экспорт
	...
	Если НЕ ТипЗнч(ВидТранспортногоСредства) = Тип("ПеречислениеСсылка.ВидыТранспортныхСредств") ТОгда
		Возврат ПоправочныйКоэффициент;
	КонецЕсли;
	// 01 июля отчетного года
	ОтчетнаяДата = Дата(Год(мДатаКонцаПериодаОтчета), 07, 01) ;
	
	// - Превышения по V легковых
	Если ВидТранспортногоСредства = Перечисления.ВидыТранспортныхСредств.ЛегковойАвтомобиль Тогда				
		// - Коэффициенты (*К) - легковые, если срок эксплуатации до окончания срока уплаты (01.07)
		//	превышает шесть лет - 72 месяца		
		Если НЕ (ХарактеристикиОбъекта.ДатаВыпуска = Дата(1,1,1)) и ДобавитьМесяц(ХарактеристикиОбъекта.ДатаВыпуска, 72) < ОтчетнаяДата Тогда
				ТаблицаСтавокНалога = Неопределено;				 
				Если ХарактеристикиОбъекта.ПроизводствоСтранСНГ = 1 Тогда															
					 Если СтруктураСтавокНалога.Свойство("КМашиныСтранСНГ", ТаблицаСтавокНалога) Тогда				 	
					...
					КонецЕсли;					 
				Иначе										
					 Если СтруктураСтавокНалога.Свойство("КМашиныПрочие", ТаблицаСтавокНалога) Тогда				 	
						ПоправочныйКоэффициент = ПолучитьСтавкуНалогаНаТранспортДляОбъекта(ТаблицаСтавокНалога, ХарактеристикиОбъекта.ОбъемДвигателя) ;						
					 КонецЕсли;					 					
				КонецЕсли;                                                							
		КонецЕсли;
	КонецЕсли;
	
	// - Коэффициенты (*К) - грузовые, стран СНГ, превышает 7 лет - 84 месяца	
	Если ВидТранспортногоСредства = Перечисления.ВидыТранспортныхСредств.ГрузовойАвтомобиль Тогда
		Если ХарактеристикиОбъекта.ДатаВыпуска <> Дата(1,1,1) 
			и ХарактеристикиОбъекта.ПроизводствоСтранСНГ
			и ДобавитьМесяц(ХарактеристикиОбъекта.ДатаВыпуска, 84) < ОтчетнаяДата Тогда
				ТаблицаСтавокНалога = Неопределено;				 
				 Если СтруктураСтавокНалога.Свойство("КГрузовыеМашиныСтранСНГ", ТаблицаСтавокНалога) Тогда				 	
					ПоправочныйКоэффициент = ПолучитьСтавкуНалогаНаТранспортДляОбъекта(ТаблицаСтавокНалога, РазностьДатВМесяцах(ХарактеристикиОбъекта.ДатаВыпуска, ОтчетнаяДата)/12) ;						
			 КонецЕсли;					 							
		КонецЕсли;
	КонецЕсли;		   
	...
	Если ВидТранспортногоСредства = Перечисления.ВидыТранспортныхСредств.ЛетательныйАппарат 
		 ...
		 ТаблицаСтавокНалога = Неопределено;				  
		 Если ХарактеристикиОбъекта.ДатаВыпуска > Дата(1999, 04, 01) Тогда
	...
	// - Коэффициенты (*К) - судна (катера)	
	Если ВидТранспортногоСредства = Перечисления.ВидыТранспортныхСредств.Катер Тогда
		Если ХарактеристикиОбъекта.ДатаВыпуска <> Дата(1,1,1) Тогда
			 ТаблицаСтавокНалога = Неопределено;				  
			 Если СтруктураСтавокНалога.Свойство("КСудна", ТаблицаСтавокНалога) Тогда				 	
				ПоправочныйКоэффициент = ПолучитьСтавкуНалогаНаТранспортДляОбъекта(ТаблицаСтавокНалога, РазностьДатВМесяцах(ХарактеристикиОбъекта.ДатаВыпуска, ОтчетнаяДата)/12) ;						
			 КонецЕсли;					 													
		КонецЕсли;
	КонецЕсли;		
