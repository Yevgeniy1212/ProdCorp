...
Процедура ЗаполнитьНачальныеНастройки(ВосстанавливаемНастройку = Ложь) Экспорт
	...
	ОбщийОтчет.мНазваниеОтчета = "Список запланированных отпусков";
	...
	ТекстЗапроса = 
		...
		|	НАЧАЛОПЕРИОДА(ВложенныйЗапрос.ДатаОкончания, ДЕНЬ) КАК ВремяОкончания,
		|	СУММА(Календарь.КалендарныеДни) КАК ДнейОтпуска,
		...
		|		СобытийныйПланЗанятостиФизлицСрезПоследних.Регистратор КАК Регистратор,
		...
		|		СобытийныйПланЗанятостиФизлицСрезПоследних.Состояние = &Отпуск
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|		СобытийныйПланЗанятостиФизлиц.Состояние = &Отпуск) КАК ВложенныйЗапрос
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.РегламентированныйПроизводственныйКалендарь КАК Календарь
		|ПО НАЧАЛОПЕРИОДА(ВложенныйЗапрос.Период,ДЕНЬ) <= Календарь.ДатаКалендаря И ВложенныйЗапрос.ДатаОкончания >= Календарь.ДатаКалендаря
		|{
		...
		|	НАЧАЛОПЕРИОДА(ВложенныйЗапрос.ДатаОкончания, ДЕНЬ),
	...
	Если ОбщийОтчет.ИспользоватьСвойстваИКатегории Тогда
		...
		НоваяСтрока.Представление = "Документ планирования отпуска";
	...
	ОбщийОтчет.ЗаполнитьПоказатели("ДнейОтпуска", "Дней отпуска", Истина, "ЧЦ=5; ЧДЦ=0");
	
 	// представление полей запроса
	СтруктураПредставлениеПолей.Вставить("ФизЛицо", "Сотрудник");
	СтруктураПредставлениеПолей.Вставить("ВремяОкончания", "Окончание отпуска");
	СтруктураПредставлениеПолей.Вставить("ВремяНачала", "Начало отпуска");
	СтруктураПредставлениеПолей.Вставить("Регистратор", "Документ планирования отпуска");
	УправлениеОтчетами.ЗаполнитьПредставленияПолей(СтруктураПредставлениеПолей, ПостроительОтчета);
	
	// формат полей
	ОбщийОтчет.СтруктураФорматаПолей.Вставить("ВремяНачала", "ДЛФ=DD");
	ОбщийОтчет.СтруктураФорматаПолей.Вставить("ВремяОкончания", "ДЛФ=DD");
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	ОбщийОтчет.ПостроительОтчета.Параметры.Вставить("Отпуск", Перечисления.ТипыПериодическихЗадачРаботника.ОтпускЕжегодный);
