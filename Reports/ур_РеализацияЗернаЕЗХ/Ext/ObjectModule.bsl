...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	если ОсновнаяСтавкаНДС = Неопределено Тогда
		СтавкаНДС = 1.12;
	Иначе
		СтавкаНДС = 1+ОсновнаяСтавкаНДС.Ставка/100;
	КонецЕсли;
	
	ТиповыеОтчеты.УстановитьПараметр(КомпоновщикНастроек, "СтавкаНДС", СтавкаНДС);
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Реализация зерна " + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
