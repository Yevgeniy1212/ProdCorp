...
Функция ПолучитьТаблицуДанных(мПериодичность,Отбор,мГруппировки)
	...
	Для Каждого СтрокаОтбор Из Отбор Цикл
		Если СтрокаОтбор.Путь="" ИЛИ СтрокаОтбор.ВидСравнения.Пустая() Тогда
			Продолжить;
		КонецЕсли;
		Запрос.УстановитьПараметр("ПараметрОтбора"+Строка(Отбор.Индекс(СтрокаОтбор)),?(ЗначениеЗаполнено(СтрокаОтбор.Значение),СтрокаОтбор.Значение,фин_ОбщегоНазначенияВызовСервераПовтИсп.ТипЗначенияГруппировки(СтрокаОтбор.ПолеОтбора).ПривестиЗначение(Неопределено)));
	КонецЦикла;
	
	Запрос.Текст = "ВЫБРАТЬ
	               |	СтатьиБюджета.Счет КАК Счет"+ТекстИзмерения+ТекстПоказатели+"
	               |ИЗ
	               |	(ВЫБРАТЬ
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.фин_Бюджетирование.Обороты(
	               |				&НачалоПланирования,
	               |				&ОкончаниеПланирования,
	               |				Регистратор,
				   ...
				   |		
	               |ГДЕ
	               |	(Обороты.Регистратор.Дата ЕСТЬ NULL 
	               |			ИЛИ Обороты.Регистратор.Дата < &ГраницаАктуальности"+?(ТекстОтбораОстатки="",""," И "+ТекстОтбораОстатки)+")"+?(ТекстОтбораИтоги="",""," И "+ТекстОтбораИтоги);
...
Функция ПолучитьСКД(мГруппировки)
	...
	Для Каждого СтрокаПоказатель Из Показатели Цикл
		...
		КонецЕсли;
			...
			НовоеПоле=НаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
			НовоеПоле.Заголовок		= фин_УправлениеОтчетамиБюджетирование.ПредставлениеКолонкиОтчета(СтрокаПоказатель.Показатель,ЯзыкОтчета,СтруктураПеревода,КЭШ,Сценарий.Валюта);
			...
			НовоеПолеИтога.Выражение="СУММА("+Имя+")";
...
Функция ПолучитьНастройкиСКД()
		...
		ИначеЕсли Имя<>"Счет" Тогда
				Если Показатели.Количество()>0 Тогда
					...
					Для Каждого Показатель Из Показатели Цикл
						...
						Отбор.Использование = Истина;
						Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(фин_ОбщегоНазначенияСервер.ПолучитьИмяЭлементаПеречисленияПоЗначению(Показатель.Показатель));
					КонецЦикла;
					ГруппировкаПоОсновномуПолю.ПараметрыВывода.Элементы.Найти("ВыводитьОтбор").Использование = Истина;
					ГруппировкаПоОсновномуПолю.ПараметрыВывода.Элементы.Найти("ВыводитьОтбор").Значение = ТипВыводаТекстаКомпоновкиДанных.НеВыводить;
				КонецЕсли;
	...
	Для Каждого СтрокаПоказатель Из Показатели Цикл
			...
			ВыбранноеПоле.Использование	= Истина;
			ВыбранноеПоле.Заголовок		= фин_УправлениеОтчетамиБюджетирование.ПредставлениеКолонкиОтчета(СтрокаПоказатель.Показатель,ЯзыкОтчета,СтруктураПеревода,КЭШ,Сценарий.Валюта);
			ВыбранноеПоле.Поле			= Новый ПолеКомпоновкиДанных(Имя);
			ПолеОформления = Настройки.УсловноеОформление.Элементы.Добавить();
			ПолеОформления.Использование = Истина;
			ДопПоле = ПолеОформления.Поля.Элементы.Добавить();
	...
	Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("Периодичность",?(ВидДанных=Перечисления.фин_ВидыДанныхОтчета.Фактические,Периодичность,фин_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьПериодичностьСценария(Сценарий)));
...
Процедура Скомпоновать(ДокументРезультат,Отбор) Экспорт
	...
	//				//Если Найти(Параметр.Выражение,"СчетРеквизиты.Счет") <>0 Тогда
	//				//	Параметр.Выражение =СтрЗаменить(Параметр.Выражение,"СчетРеквизиты.Счет","СчетИерархия.Счет");
	//				//КонецЕсли;
	//			КонецЦикла;
	//		КонецЦикла;
	...
	Если ЗначениеЗаполнено(ЯзыкОтчета) Тогда
		ОбластьИтого = ДокументРезультат.НайтиТекст("Итого");
		Если ОбластьИтого<>Неопределено Тогда
			ОбластьИтого.Текст = фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина("Итого",ЯзыкОтчета);
		КонецЕсли;
	КонецЕсли;
	МакетЗаголовок = ПолучитьОбщийМакет("фин_МакетЗаголовк");
	Область = МакетЗаголовок.ПолучитьОбласть("Заголовок");
	Заголовок=фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина(СтатьяБаланса,ЯзыкОтчета,,КЭШ)+Символы.ПС+фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина("Сценарий",ЯзыкОтчета,,КЭШ)+": "+фин_УправлениеМногоязычнымиОтчетами.ПолучитьПереводТермина(Сценарий,ЯзыкОтчета,,КЭШ)+Символы.ПС+
