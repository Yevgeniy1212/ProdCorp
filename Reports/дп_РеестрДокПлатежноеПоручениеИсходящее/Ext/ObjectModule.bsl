...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Реестр документов платежное поручение исходящее " + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
...
Функция ПолучитьВыборку()
	...
	|	ВЫБОР КОГДА ПлатежноеПоручениеИсходящее.ВалютаДокумента <> &мВалютаРегламентированногоУчета 
	|	ТОГДА
	|		ПлатежноеПоручениеИсходящее.СуммаДокумента*ЕстьNull(ОтборКурсов.Курс,1)
	|	ИНАЧЕ
	|       ПлатежноеПоручениеИсходящее.СуммаДокумента
	|   КОНЕЦ  КАК СуммаДокумента,
	|	ПлатежноеПоручениеИсходящее.ТипОперации,
	...
	|		) КАК ОтборКурсов	
	|	ПО ОтборКурсов.Валюта = ПлатежноеПоручениеИсходящее.ВалютаДокумента		
	|ГДЕ
