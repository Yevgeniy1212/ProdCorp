...
Функция ЗаполнитьСведенияОНалогоплательщике()Экспорт
	...
	ОргСведения.Вставить("КБе", СокрЛП(Налогоплательщик.КБе));	
...
// Вызывает диалог выбора файла для выбора файла данных
//
Функция ВыборФайлаПриложения(ИмяФайлаДляВыгрузки) Экспорт
	
	Режим                 = РежимДиалогаВыбораФайла.Открытие;
	Диалог                = Новый ДиалогВыбораФайла(Режим);
	Диалог.Заголовок      = "Выберите файл";
	Диалог.ПолноеИмяФайла = ИмяФайлаДляВыгрузки;
	Диалог.Фильтр         = "*.xml|*.xml|Все файлы(*.*)|*.*";
	Диалог.Расширение     = "xml";
	Если Диалог.Выбрать() Тогда
		ИмяФайла = Диалог.ПолноеИмяФайла;
...
ИмяПакета = "http://www.fno200.rating.kz";
ИмяФайлаВыгрузки = "200.xml";
