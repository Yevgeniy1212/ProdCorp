...
Процедура ВывестиПодписи(Результат)
	...
	ОтветЛица = ОбщегоНазначения.ОтветственныеЛицаОрганизаций(ИскомаяОрганизация, Период, глЗначениеПеременной("глТекущийПользователь").ФизЛицо);
...
Функция ПолучитьВыборку()
	...
	|	ВЫБОР
	|		КОГДА &ЕдиницаИзмерения = ЗНАЧЕНИЕ(Перечисление.ур_ВариантыВыводаОтчетовПоУчетуЗерна.килограммы)
	|			ТОГДА ЕСТЬNULL(ОстаткиФуражныйФонд.ЗачтенныйвесОстаток, 0) * 1000
	|		ИНАЧЕ ЕСТЬNULL(ОстаткиФуражныйФонд.ЗачтенныйвесОстаток, 0)
	|	КОНЕЦ КАК ФуражныйФонд
	//++Аксиома 27.01.2025
	|	,ВЫБОР КОГДА ур_ЗерноНаСкладахОстатки.ЗерноваяРасписка.ур_Бюджет ТОГДА ВЫБОР
	|		КОГДА &ЕдиницаИзмерения = ЗНАЧЕНИЕ(Перечисление.ур_ВариантыВыводаОтчетовПоУчетуЗерна.килограммы)
	|			ТОГДА ЕСТЬNULL(ур_ЗерноНаСкладахОстатки.ЗачтенныйвесОстаток, 0) * 1000
	|		ИНАЧЕ ЕСТЬNULL(ур_ЗерноНаСкладахОстатки.ЗачтенныйвесОстаток, 0)
	|	КОНЕЦ ИНАЧЕ 0 КОНЕЦ КАК Бюджет
	//--Аксиома 27.01.2025
	|ИЗ
	...
	|			И ур_ЗерноНаСкладахОстатки.Культура = ОстаткиЗалога.Культура
	|			И ур_ЗерноНаСкладахОстатки.Класс = ОстаткиЗалога.Класс
	|			И ур_ЗерноНаСкладахОстатки.ГодУрожая = ОстаткиЗалога.ГодУрожая
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			_ФуражныйФондОстатки.Организация КАК Организация,
	|			_ФуражныйФондОстатки.ВидРесурса КАК ВидРесурса,
	|			_ФуражныйФондОстатки.Склад КАК Склад,
	//|			_ФуражныйФондОстатки.ТипОперации КАК ТипОперации,
	|			_ФуражныйФондОстатки.ГодУрожая КАК ГодУрожая,
	|			_ФуражныйФондОстатки.Культура КАК Культура,
	|			_ФуражныйФондОстатки.Класс КАК Класс,
	|			_ФуражныйФондОстатки.Зерноваярасписка КАК Зерноваярасписка,
	|			_ФуражныйФондОстатки.Получатель КАК Получатель,
	|			_ФуражныйФондОстатки.ЗачтенныйвесОстаток КАК ЗачтенныйвесОстаток
	|		ИЗ
	|			РегистрНакопления._ФуражныйФонд.Остатки(&Период, ) КАК _ФуражныйФондОстатки) КАК ОстаткиФуражныйФонд
	|		ПО ур_ЗерноНаСкладахОстатки.Организация = ОстаткиФуражныйФонд.Организация
	|			И ур_ЗерноНаСкладахОстатки.ВидРесурса = ОстаткиФуражныйФонд.ВидРесурса
	|			И ур_ЗерноНаСкладахОстатки.Склад = ОстаткиФуражныйФонд.Склад
	|			И ур_ЗерноНаСкладахОстатки.ЗерноваяРасписка = ОстаткиФуражныйФонд.Зерноваярасписка
	|			И ур_ЗерноНаСкладахОстатки.Культура = ОстаткиФуражныйФонд.Культура
	|			И ур_ЗерноНаСкладахОстатки.Класс = ОстаткиФуражныйФонд.Класс";
