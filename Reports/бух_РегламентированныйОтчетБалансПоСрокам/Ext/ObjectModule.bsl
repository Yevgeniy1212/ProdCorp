...
Процедура СформироватьРасшифровку(КодСтрокиБаланса) Экспорт
	...
	тзКопия = тз.Скопировать(Новый Структура("КодСтрокиБаланса",КодСтрокиБаланса));
	
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасшифровкаБаланса";
	...
	Для Каждого СтрокаТЗ из тзКопия Цикл
		...
		ОбластьМакета.Параметры.Наименование = СтрокаТЗ.Счет.Наименование;
		ТабДокумент.Вывести(ОбластьМакета);
	КонецЦикла;
	
	ОбластьМакета = Макет.ПолучитьОбласть("Итог");
	ОбластьМакета.Параметры.ОстатокНаКонец = тзКопия.Итог("ОстатокНаКонец");
	ОбластьМакета.Параметры.ОстатокНаНачало = тзКопия.Итог("ОстатокНаНачало");
	ТабДокумент.Вывести(ОбластьМакета);
