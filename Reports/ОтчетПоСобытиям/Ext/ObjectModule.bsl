...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мСтруктураСоответствияИмен = Новый Структура("КонтактноеЛицо, Событие", "Кандидат", "Событие");
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	УправлениеОтчетами.УстановитьКолонтитулыПоУмолчанию(Таб, "Отчет по событиям", Строка(глТекущийПользователь));
КонецПроцедуры
...
Процедура СформироватьСобытие(Событие, Макет, Индекс, Таб)
	...
	Для каждого Элементы Из мСписокЭлементовСобытия Цикл
		...
		ИначеЕсли Элементы.Значение = "Вложения" Тогда
			Если УниверсальныеМеханизмы.ЕстьДополнительнаяИнформация(Событие) Тогда
