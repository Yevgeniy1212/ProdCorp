	...
	Для Каждого Поле Из Настройки.Выбор.Элементы Цикл
		...
		Если Поле <> Неопределено Тогда
			...
			Если Год(НачалоДня(Дата(ТекущГод-1,1,1))) = 2016
					...
					ИЛИ Год(НачалоДня(Дата(ТекущГод-1,1,1))) = 2018
					ИЛИ Год(НачалоДня(Дата(ТекущГод-1,1,1))) = 2019 Тогда
		...
		Если Поле <> Неопределено Тогда
			...
			Если Год(НачалоДня(Дата(ТекущГод-1,1,1))) = 2016
				...
				Поле.Заголовок = "Факт ПКК "+Формат(Год(НачалоДня(Дата(ТекущГод-1,1,1))),"ЧГ=0")+" года";
			Иначе
				Поле.Заголовок = "Факт ПКК "+Формат(Год(НачалоДня(Дата(ТекущГод-1,1,1))),"ЧГ=0")+" года";
			КонецЕсли;
		...
		Если Поле <> Неопределено Тогда
			...
			Если Год(НачалоДня(Дата(ТекущГод-1,1,1))) = 2016
					ИЛИ Год(НачалоДня(Дата(ТекущГод-1,1,1))) = 2017 
					ИЛИ Год(НачалоДня(Дата(ТекущГод-1,1,1))) = 2018 Тогда
				Поле.Заголовок = "Факт дочек "+Формат(Год(НачалоДня(Дата(ТекущГод-1,1,1))),"ЧГ=0")+" года";
			Иначе
				Поле.Заголовок = "Факт дочек "+Формат(Год(НачалоДня(Дата(ТекущГод-1,1,1))),"ЧГ=0")+" года";
			КонецЕсли;
