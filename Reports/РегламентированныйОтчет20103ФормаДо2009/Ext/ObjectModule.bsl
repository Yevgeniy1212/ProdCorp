...
Функция ЗаполнитьСведенияОНалогоплательщике()Экспорт
	...
	ОргСведения.Вставить("КодНалоговогоОрганаИПН", 		СокрЛП(Лев(НКСведения.РНН, 4)));
	ОргСведения.Вставить("КодНалоговогоОрганаОПВиСО", 	СокрЛП(Лев(НКСведения.РНН, 4)));
...
Функция ВыборФайла() Экспорт
	...
	Диалог.Фильтр         = "*.isd|*.isd|Все файлы(*.*)|*.*";
	Диалог.ПолноеИмяФайла = "20103";
	Диалог.Расширение     = "isd";
...
Функция ПоказатьОсновнойПериод(ТекДатаНачала,ТекДатаОкончания) Экспорт
	...
	ОпределитьНомерМесяца(ТекДатаОкончания);
	Возврат СтрПериодОтчета;
		
КонецФункции // ПоказатьОсновнойПериод()

Процедура ОпределитьНомерМесяца(КонецПериодаОтчета)
	
	Если ЗначениеЗаполнено(КонецПериодаОтчета) И Месяц(КонецПериодаОтчета) = 9 Тогда 
		мМесяц = Месяц(КонецПериодаОтчета);
		мГод   = Год(КонецПериодаОтчета);
	Иначе
		мМесяц = 3;
		мГод   = Год(КонецПериодаОтчета);		
	КонецЕсли;
...
ИмяПакета = "http://www.fno20103.rating.kz";
ИмяФайлаВыгрузки = "20103.isd";
