...
Процедура ВосстановитьНастройкиИзСтруктуры(СтруктураСНастройками) Экспорт
	...
	Если СтруктураСНастройками.Свойство("ВидРесурса") Тогда
		...
		ВидРесурса = ?(ВидРесурса = "", "НачальныйОстаток", ВидРесурса);
	Иначе
...
Процедура НастроитьПараметрыДиаграммы(Диаграмма)
	...
	ВидПодписей = ?(ВидПодписей = Неопределено, ВидПодписейКДиаграмме.Нет, ВидПодписей);
	Диаграмма.ВидПодписей = ВидПодписей;
	
	МаксСерий = ?(МаксСерий = Неопределено,МаксимумСерий.НеЗадано, МаксСерий);
	...
	РаздвижениеСерий = ?(РаздвижениеСерий=Неопределено,АвтоРаздвижениеСерий.Все,РаздвижениеСерий) ;
...
Процедура СформироватьОтчет(Диаграмма) Экспорт
	...
	ПостроительОтчета.Параметры.Вставить("СписокОрганизаций", мСписокСтруктурныхЕдиниц);
...
// Процедура - обработчик расшифровки
//
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка) Экспорт
	...
	Параметры.Вставить("СписокОрганизаций", мСписокСтруктурныхЕдиниц);
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|ИЗ
	|	РегистрБухгалтерии." + ИмяРегистраБухгалтерии + ".ОстаткиИОбороты(&ДатаНач, &ДатаКон, " + Период + ", , Счет В ИЕРАРХИИ (&Счет), , Организация В ИЕРАРХИИ(&СписокОрганизаций) " + ?(Не ПустаяСтрока(ТекстОтбор), "{"+ ТекстОтбор + "}", "") + ") КАК ОстаткиИОбороты
...
Процедура НачалоВыбораЗначенияСубконто(Элемент, СтандартнаяОбработка, ТипЗначенияПоля=Неопределено) Экспорт
	...
	Попытка
		Если Элемент.Значение.Метаданные().Владельцы.Количество() > 0 Тогда
			ВыбиратьОрганизацию = Истина;
		КонецЕсли;
	Исключение
	КонецПопытки;
	Если ВыбиратьОрганизацию Тогда
		ВыбОрганизация = мСписокСтруктурныхЕдиниц.ВыбратьЭлемент();
	...
	Для Инд=0 По ПостроительОтчета.Отбор.Количество()-1 Цикл
		...
		Если СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Номенклатура.ТипЗначения Тогда
			СписокПараметров.Вставить("Номенклатура", ЗначениеОтбора);
		ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Склады.ТипЗначения Тогда
			СписокПараметров.Вставить("Склад", ЗначениеОтбора);
		ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Контрагенты.ТипЗначения Тогда
			СписокПараметров.Вставить("Контрагент", ЗначениеОтбора);
		ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Договоры.ТипЗначения Тогда
			СписокПараметров.Вставить("ДоговорКонтрагента", ЗначениеОтбора);
		КонецЕсли;
		
	КонецЦикла;
	
	ПроцедурыБухгалтерскогоУчета.ОбработатьВыборСубконто(Элемент, СтандартнаяОбработка, Организация, СписокПараметров, ТипЗначенияПоля);
