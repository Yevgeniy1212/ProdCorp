...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("Бронирование","Залоговое обеспечение");
...
Процедура ПроверитьЗаполнениеСтроки(ЭтотОбъект,СтруктураШапкиДокумента,Отказ,Заголовок)
	...
	Для каждого СтрокаЗР ИЗ Товары Цикл
		...
		ДоступныйВес = УправлениеРесурсами.КонтрольОстатковЗалогаНаЗерновыхРасписках(ЭтотОбъект,СтруктураШапкиДокумента,СтрокаЗР,Отказ);
		
		Если (ДоступныйВес-СтрокаЗР.Количество) < 0 Тогда
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента,
	...
	Если ВидОперации = перечисления.зм_ВидыОперацийЗалоговогоОбеспечения.Закрытие Тогда
														
		// Выполнить движения по регистру накопления "Зерно на складах"
		
		НаборДвижений = Движения.ур_ЗерноВЗалоге;
		...
		Если Не Отказ Тогда
			Движения.ур_ЗерноВЗалоге.ВыполнитьРасход();
		КонецЕсли;
	ИначеЕсли ВидОперации = перечисления.зм_ВидыОперацийЗалоговогоОбеспечения.Регистрация Тогда	
		...
		Если Не Отказ Тогда
			Движения.ур_ЗерноВЗалоге.ВыполнитьПриход();
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ВидОперации = Перечисления.зм_ВидыОперацийЗалоговогоОбеспечения.Закрытие
		ИЛИ ВидОперации = Перечисления.зм_ВидыОперацийЗалоговогоОбеспечения.Уменьшение Тогда
