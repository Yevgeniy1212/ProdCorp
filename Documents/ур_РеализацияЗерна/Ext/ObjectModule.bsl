...
Процедура ЗаполнитьСчетаНУ(СтрокаТЧ, ИмяТабЧасти, СчетаУчета, ЗаполнятьНУ, МетаданныеДокумента)
	...
	Если ЗаполнятьНУ  = Истина Тогда
		...
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СчетСписанияСебестоимостиНУ", МетаданныеДокумента, ИмяТабЧасти) Тогда	
			УправлениеПроизводством.ЗаполнитьСчетНалоговогоУчетаВСтрокеТабличногоПоля(СтрокаТЧ, "СчетСписанияСебестоимостиБУ","СчетСписанияСебестоимостиНУ" , Дата);	
			ИмяАналитикиСчетовУчета = "СубконтоСписанияСебестоимостиБУ";
			ИмяАналитикиТабличнойЧасти = "СубконтоСписанияСебестоимостиНУ";
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 1]) Тогда
				СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 1] = СчетаУчета[ИмяАналитикиСчетовУчета + 4];
			КонецЕсли;	
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 2]) Тогда
				СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 2] = СтрокаТЧ.Номенклатура.НоменклатурнаяГруппа;
			КонецЕсли;	
		КонецЕсли; 		
		...
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СчетДоходовНУ", МетаданныеДокумента, ИмяТабЧасти) Тогда
			...
			ИмяАналитикиСчетовУчета = "СубконтоДоходовБУ";
			ИмяАналитикиТабличнойЧасти = "СубконтоДоходовНУ";
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 1]) Тогда
				СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 1] = СчетаУчета[ИмяАналитикиСчетовУчета + 1];
			КонецЕсли;	
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 2]) Тогда
				СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 2] = СтрокаТЧ.Номенклатура;
			КонецЕсли;	
		КонецЕсли; 	
		
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СчетДоходовКомиссионногоСбораНУ", МетаданныеДокумента, ИмяТабЧасти) Тогда	
			УправлениеПроизводством.ЗаполнитьСчетНалоговогоУчетаВСтрокеТабличногоПоля(СтрокаТЧ, "СчетДоходовКомиссионногоСбораБУ","СчетДоходовКомиссионногоСбораНУ" , Дата);	
		КонецЕсли; 	
...
Функция ПечатьЗ8()
	...
	|	Доверенность,
	|	ДоверенностьЛицо КАК ЧерезКого
	|ИЗ
	|	Документ.ур_РеализацияЗерна КАК РеализацияЗерна
	...
	|	реализация.ЕдиницаИзмеренияНаименование,
	|	реализация.ТоварКод,
	|	реализация.Количество,
	|	реализация.Цена,
	|	реализация.Сумма,
	|	реализация.СуммаНДС,
	|	реализация.СуммаСНДС
	|ИЗ (ВЫБРАТЬ
	|		1 КАК ID,
	...
	|				КОНЕЦ КАК СуммаСНДС
	|	ИЗ
	|		Документ.ур_РеализацияЗерна.Товары КАК РеализацияТоваровУслугТовары
	|	ГДЕ
	...
	|		2 КАК ID,
	|		РеализацияТоваровУслугТовары.НомерСтроки,
	|		""Вознагрождение"" КАК Номенклатура,
	...
	|		КОНЕЦ КАК СуммаСНДС
	|	ИЗ
	|		Документ.ур_РеализацияЗерна.ПроцентыВозврата КАК РеализацияТоваровУслугТовары
	|	ГДЕ
	|		РеализацияТоваровУслугТовары.Ссылка = &ТекДокумент
	|   ) КАК реализация
	|
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТоварНаименование = ВыборкаСтрокТовары.Номенклатура;
	...
	Если ЗначениеЗаполнено(Шапка.Склад) Тогда
		ДанныеМОЛ = ПроцедурыУправленияПерсоналом.ДанныеФизЛица(Шапка.Организация, Шапка.МОЛ, Дата);
...
Функция ПечатьНакладной()   
	...
	|ИЗ
	|	Документ.ур_РеализацияЗерна КАК РеализацияТоваровУслуг
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтветственныеЛица.СрезПоследних(&Дата, ) КАК ОтветственныеЛицаСклад
	...
	|		ЗерноваяРасписка     КАК ЗерноваяРасписка,
	|		ЕдиницаИзмерения     КАК ЕдиницаИзмерения,
	|		Цена                 КАК Цена,
	...
	|	ИЗ
	|		Документ.ур_РеализацияЗерна.Товары КАК РеализацияТоваровУслуг
	|	ГДЕ
	|		РеализацияТоваровУслуг.Ссылка = &ТекущийДокумент
	|	СГРУППИРОВАТЬ ПО
	|		Номенклатура,
	...
	|	
	|ИЗ
	|	Документ.ур_РеализацияЗерна.Услуги КАК РеализацияТоваровУслуг
	|
	|ГДЕ
	|	РеализацияТоваровУслуг.Ссылка = &ТекущийДокумент
	|
	...
	|	""Вознагрождение"",
	|	"""",
	|	ЗНАЧЕНИЕ(Документ.ур_ЗерноваяРасписка.Пустаяссылка),
	|	0,
	|	"""",
	|	"""",
	|	0,
	...
	|	
	|ИЗ
	|	Документ.ур_РеализацияЗерна.ПроцентыВозврата КАК РеализацияПроцентыВозврата
	|
	|ГДЕ
	|	РеализацияПроцентыВозврата.Ссылка = &ТекущийДокумент
	|	
	|УПОРЯДОЧИТЬ ПО
	|	ID,
	|	НомерСтроки";
	...
	Для каждого ВыборкаСтрокТовары из ЗапросТовары Цикл 
		...
		ЗерноваяРасписка = СокрЛП(ВыборкаСтрокТовары.ЗерноваяРасписка.КодМСХ)+" "+СокрЛП(ВыборкаСтрокТовары.ЗерноваяРасписка.СерияНомерМСХ)+" от "+ Формат(ВыборкаСтрокТовары.ЗерноваяРасписка.Дата,"ДФ = dd.MM.yyyy");
		...
		ИтКоличество = ИтКоличество+ВыборкаСтрокТовары.Количество;
	КонецЦикла;
	...
	ОбластьИтого.Параметры.ИтКоличество = ИтКоличество;
	...
	Руководители = ОбщегоНазначения.ОтветственныеЛицаОрганизаций(Организация, КонецДня(Дата),);

	
	ОбластьМакета = Макет.ПолучитьОбласть("Подписи");
	ОбластьМакета.Параметры.ФИОРуководителя 		= Руководители.Руководитель;
	ОбластьМакета.Параметры.ФИОГлавногоБухгалтера 	= ?(НЕ ЗначениеЗаполнено(Руководители.ГлавныйБухгалтер), "Не предусмотрен", Руководители.ГлавныйБухгалтер);
...
Функция ПечатьАктаОбОказанииУслуг()
	...
	|			И РеализацияТоваровУслуг.Ответственный.ФизЛицо = РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо
//	|			И РеализацияТоваровУслуг.СтруктурноеПодразделение = РаботникиОрганизацийСрезПоследних.СтруктурноеПодразделение
	...
	ОбластьМакета.Параметры.РННОрганизации           = СведенияОбОрганизации.РНН;
	...
	ОбластьМакета.Параметры.РННКонтрагента           = СведенияОПокупателе.РНН;
...
Функция ПечатьЗ2()
	...
	ЗапросПоТоварам = Новый Запрос();
	ЗапросПоТоварам.УстановитьПараметр("ТекДокумент", Ссылка);
	ЗапросПоТоварам.Текст = 
	"ВЫБРАТЬ
	|	РеализацияТоваровУслугТовары.НомерСтроки,
	|	РеализацияТоваровУслугТовары.Номенклатура,
	|	РеализацияТоваровУслугТовары.ЗерноваяРасписка,
	|	РеализацияТоваровУслугТовары.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмеренияНаименование,
	|	РеализацияТоваровУслугТовары.Номенклатура.Код КАК ТоварКод,
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
		ОбластьМакета.Параметры.НомерПП = ВыборкаСтрокТовары.НомерСтроки;
		ОбластьМакета.Параметры.ТоварНаименование = ВыборкаСтрокТовары.Номенклатура.НаименованиеПолное+" зерн.расписка "+ЗерноваяРасписка;
...
//
Процедура ЗаполнитьИРассчитатьПроцентыВозврата() Экспорт
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТекДокумент",ЭтотОбъект.Ссылка);
	Запрос.УстановитьПараметр("Процент",ПроцентВозврата);
	...
	|	Товары.СчетУчетаБУ,
	|	Товары.СтавкаНДС,
	|	Товары.НДСВидОперацииРеализации,
	|	Товары.СчетУчетаНДСПоРеализации,
	|	Товары.СчетДоходовБУ,
	|	Товары.СубконтоДоходовБУ1,
	|	Товары.СубконтоДоходовБУ2,
	|	Товары.СубконтоДоходовБУ3,
	|	Товары.СубконтоДоходовБУ4,
	|	Товары.СчетДоходовНУ,
	|	Товары.СубконтоДоходовНУ1,
	|	Товары.СубконтоДоходовНУ2,
	|	Товары.СубконтоДоходовНУ3,
	|	Товары.СубконтоДоходовНУ4,
	|	Товары.СчетДоходовКомиссионногоСбораБУ,
	|	Товары.СчетДоходовКомиссионногоСбораНУ,
	|	ВЫБОР КОГДА &Процент=0
	|	ТОГДА 0
	|	ИНАЧЕ Товары.Сумма*&процент/100
	|	КОНЕЦ КАК Сумма
	|ИЗ
	...
	ПроцентыВозврата.Загрузить(тз);
	
	Для Каждого СтрокаТЗ ИЗ ПроцентыВозврата Цикл
		// Рассчитать реквизиты табличной части.
		ОбработкаТабличныхЧастей.РассчитатьСуммуНДСТабЧасти(СтрокаТЗ, ЭтотОбъект);
		
		УправлениеРесурсами.РассчитатьСуммуКомиссионногоСбора(СтрокаТЗ, ЭтотОбъект);
	КонецЦикла;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("Накладная,Акт, З2","Расходная накладная","Акт об оказании услуг", "З-2 (накладная на отпуск запасов) (Приказ №562)");
...
Процедура ЗаполнитьСчетаБУ(СтрокаТЧ, ИмяТабЧасти, СчетаУчета, ЗаполнятьБУ, МетаданныеДокумента) Экспорт
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СчетДоходовБУ", МетаданныеДокумента, ИмяТабЧасти) Тогда
		ИмяАналитикиСчетовУчета = "СубконтоДоходовБУ";
		ИмяАналитикиТабличнойЧасти = "СубконтоДоходовБУ";
		// Счет доходов и его аналитика
		СтрокаТЧ.СчетДоходовБУ  = СчетаУчета.СчетДоходовБУ;
		// Прочую аналитику заполняем по данным регистра сведений 
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 1]) Тогда
			СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 1] = СтрокаТЧ.Номенклатура.ВидДохода;
		КонецЕсли;	
		
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 2]) Тогда
			СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 2] = ТипОперации;
		КонецЕсли;	
		
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 3]) Тогда
			СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 3] = ДоговорКонтрагента;
		КонецЕсли;	
	...
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СчетСписанияСебестоимостиБУ", МетаданныеДокумента, ИмяТабЧасти) Тогда
		ИмяАналитикиСчетовУчета = "СубконтоСписанияСебестоимостиБУ";
		ИмяАналитикиТабличнойЧасти = "СубконтоСписанияСебестоимостиБУ";
		// Счет себестоимости и его аналитика
		СтрокаТЧ.СчетСписанияСебестоимостиБУ  = СчетаУчета.СчетСписанияСебестоимостиБУ;
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 4]) Тогда
			СтрокаТЧ[ИмяАналитикиТабличнойЧасти + 4] = СчетаУчета[ИмяАналитикиСчетовУчета + 4];
		КонецЕсли;	
		
	КонецЕсли;
	
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СчетДоходовКомиссионногоСбораБУ", МетаданныеДокумента, ИмяТабЧасти) Тогда
		СтрокаТЧ.СчетДоходовКомиссионногоСбораБУ  = СчетаУчета.СчетДоходовКомиссионногоСбораБУ;
	КонецЕсли;
...
Процедура ЗаполнитьСчетаУчетаВСтрокеТабЧасти(СтрокаТЧ, ИмяТабЧасти, ЗаполнятьБУ, ЗаполнятьНУ) Экспорт
	...
	СчетаУчета     = УправлениеРесурсами.ПолучитьСчетаУчетаРесурсовПоРеализации(ИсточникФинансирования, СтрокаТЧ.СчетУчетаБУ);
...
Процедура ЗаполнитьПоДокументуОснования(Основание) Экспорт
	...
	// -Управление договорами- начало
	Попытка
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РегистрацияДоговоров") Тогда
		дог_УправлениеДоговорами.ОбработкаЗаполненияПоДоговоруОбработкаЗаполнения(ЭтотОбъект, Основание, Истина);
	КонецЕсли;
	// -управление договорами- конец
...
Функция ПодготовитьТаблицуТоваров(РезультатЗапросаПоТоварам, СтруктураШапкиДокумента) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		КонецЕсли;
		СтрокаТаблицы.Сумма = СтрокаТаблицы.Сумма - СтрокаТаблицы.СуммаКомиссионногоСбора;
		СтрокаТаблицы.НДС = СтрокаТаблицы.НДС - СтрокаТаблицы.НДСКомиссионногоСбора;
...
Функция ПодготовитьТаблицуТоваровКомСбор(РезультатЗапросаПоТоварамКомСбор, СтруктураШапкиДокумента)
	
	ТаблицаТоваров = РезультатЗапросаПоТоварамКомСбор.Выгрузить();
...
//Проверить заполненость обязательных реквизитов зерновой расписки
Процедура ПроверитьЗаполнениеЗерновыхРасписокПоПриходу(ТаблицаПоЗерновымРаспискам, Отказ, Заголовок)
	СтрокаНачалаСообщенияОбОшибке = "";
	Для каждого СтрокаЗР ИЗ ТаблицаПоЗерновымРаспискам Цикл
		СтрокаНачалаСообщенияОбОшибке = "В строке номер """+ СокрЛП(СтрокаЗР.НомерСтроки) +
		""" Зерновая расписка """;
		...
		Если СтрокаЗР.ЗачтенныйВес <> СтрокаЗР.ЗачтенныйВесПоЗР Тогда
			
			СтрокаСообщения = "Вес по ЗР не равен весу остатка !";
			ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + СтрокаСообщения, Отказ, Заголовок);
		КонецЕсли;
...
Процедура ПроверитьТабЧатьТоварыПоСтрочно(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для каждого СтрокаТЧ Из ТаблицаПоТоварам Цикл
		...
		Если НЕ СтрокаТЧ.СчетУчетаБУ.Забалансовый Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетСписанияСебестоимостиБУ) Тогда
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам,
								ТаблицаПоУслугам, ТаблицаПоТоварамКомСбор, ТаблицаПоЗерновымРаспискам,
	...
	//Ответ = Вопрос("Перепровести документ ?",Режим);
	//Если Ответ = КодВозвратаДиалога.Нет Тогда
	//	Возврат;
	//КонецЕсли;	
    ...
    //	Бух	учет
	// Проводки по списанию зерна
	Проводка = ПроводкиБУ.Добавить();
	Проводка.Период = Дата;
	Проводка.Организация = Организация;
	Проводка.СчетДт      = Стр.СчетСписанияСебестоимостиБУ;
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Доходы",Стр.СубконтоСписанияСебестоимостиБУ1);
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"ТипыОпераций",Стр.СубконтоСписанияСебестоимостиБУ2);
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Договоры",Стр.СубконтоСписанияСебестоимостиБУ3);
	Проводка.СчетКт      = Стр.СчетУчетаБУ;
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Номенклатура",Стр.Номенклатура);
	Проводка.КоличествоКт = Стр.Количество;
	Проводка.Сумма        = Стр.СуммаПроводки;
	...
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"ТипыОпераций",ТипОперации);
	Проводка.ВалютаДт =    ВалютаДокумента;
	Проводка.ВалютнаяСуммаДт = Стр.ВалютнаяСуммаВыручкиПроводки;
	Проводка.СчетКт = Стр.СчетДоходовБУ;
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Доходы",Стр.СубконтоСписанияСебестоимостиБУ1);
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"ТипыОпераций",Стр.СубконтоСписанияСебестоимостиБУ2);
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Договоры",ДоговорКонтрАгента);
	Проводка.Сумма = Стр.СуммаВыручкиПроводки;
	...
	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"ТипыОпераций",ТипОперации);
	Проводка.ВалютаДт =    ВалютаДокумента;
	Проводка.ВалютнаяСуммаДт = Стр.СуммаНДСПроводки;
	Проводка.СчетКт = Стр.СчетУчетаНДСПоРеализации;
	...
	Проводка.Сумма = Стр.СуммаНДСПроводки;
	...
	Для Каждого Стр из Товары Цикл
		...
		Проводка.Период = Дата;
		Проводка.СчетДт = Стр.СчетСписанияСебестоимостиНУ;
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"НоменклатурныеГруппы",Стр.СубконтоСписанияСебестоимостиНУ2);
		Проводка.СчетКт = Стр.СчетУчетаНУ;
		Проводка.КоличествоКт = Стр.Количество;
	...
	Для Каждого Стр из Товары Цикл
		...
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Контрагенты",КонтрАгент);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Договоры",ДоговорКонтрАгента);
		Проводка.СчетКт = Стр.СчетДоходовНУ;
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Доходы",Стр.СубконтоДоходовНУ1);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Номенклатура",Стр.СубконтоДоходовНУ2);
		Проводка.Сумма = Стр.СуммаВыручкиПроводки;
		Проводка.Содержание = "Выручка от реализации";
		Если стр.СуммаНДС <> 0 Тогда
		Проводка = ПроводкиНУ.Добавить();
		...
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Контрагенты",КонтрАгент);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Договоры",ДоговорКонтрАгента);
		Проводка.СчетКт = ПланыСчетов.Налоговый.НалогНаДобавленнуюСтоимость;
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"НалогиСборыОтчисления",Справочники.НалогиСборыОтчисления.НалогНаДобавленнуюСтоимость);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"ВидыПлатежейВБюджетИФонды",Перечисления.ВидыПлатежейВБюджетИФонды.Налог);
		Проводка.Сумма = Стр.СуммаНДСПроводки;
		Проводка.Содержание = "НДС";
	...
	ДвиженияРегистровПодсистемыУчетаРесурсов(СтруктураШапкиДокумента, ТаблицаПоЗерновымРаспискам, 
											ТаблицаРасходаПоЗерновымРаспискам, ТаблицаПриходаПоЗерновымРаспискам, Отказ, Заголовок);
// Движение по регестру РеализацияЗерна											
		//сформируем движения для регистра "Реализация Зерна"
//		ТаблицаВыручкиРеализацияТМЗ = ТаблицаВыручки.Скопировать();
//		ТаблицаВыручкиРеализацияТМЗ.Свернуть("СчетУчетаБУ, ЗерноваяРасписка, ВидДвиженияЗерна, Партия ","Сумма, СуммаСписания, СуммаКомиссионногоСбора, Количество, НДС, НДСКомиссионногоСбора, Акциз");
...
//			ДвижениеРеализации.СтруктурноеПодразделение = СтруктураШапкиДокумента.СтруктурноеПодразделение;
			...
			ДвижениеРеализации.ВидОперации  			= ВидОперации;
...
//			ДвижениеРеализации.Партия 	  				= СтрокаРеализации.Партия;
			...
			ДвижениеРеализации.Стоимость  				= СтрокаРеализации.СуммаПроводки;
			ДвижениеРеализации.Количество 				= СтрокаРеализации.Количество;
			ДвижениеРеализации.Счет 					= СтрокаРеализации.СчетУчетаБУ;
		...
		Для Каждого СтрокаРеализации ИЗ Товары Цикл
			...
			ДвижениеРеализации.ГодУрожая				= СтрокаРеализации.Зерноваярасписка.ГодУрожая;
			ДвижениеРеализации.Зерноваярасписка			= СтрокаРеализации.Зерноваярасписка;
			ДвижениеРеализации.Сумма 			  		= СтрокаРеализации.СуммаВыручкиПроводки;
			ДвижениеРеализации.НДС 				  		= СтрокаРеализации.СуммаНДСПроводки;                
			ДвижениеРеализации.Счет 					= СтрокаРеализации.СчетУчетаБУ;

		КонецЦикла;
        Движения.ур_РеализацияЗерна.Записать();
		Для Каждого СтрокаНДС ИЗ Товары Цикл
			
			ДвижениеНДС = Движения.НДС.Добавить();
			ДвижениеНДС.Активность 			= 		Истина;
			ДвижениеНДС.Период 			    = 		Дата;
			ДвижениеНДС.Организация 		= 		Организация;
			ДвижениеНДС.Налогоплательщик 	= 		Организация.ГоловнаяОрганизация;
			ДвижениеНДС.ВидОперацииРеализации= 		СтрокаНДС.НДСВидОперацииРеализации;
			ДвижениеНДС.ВидНалогаНДС 		= 		Перечисления.ВидыНДС.НДС;
			ДвижениеНДС.СтавкаНДС  			=      	СтрокаНДС.СтавкаНДС;
			ДвижениеНДС.Контрагент			= 		Контрагент;
			ДвижениеНДС.ТМЗ					= 		СтрокаНДС.Номенклатура;
			ДвижениеНДС.СтруктурнаяЕдиница	=		Организация;
			ДвижениеНДС.СуммаБезНДС  		=		СтрокаНДС.СуммаВыручкиПроводки;
			ДвижениеНДС.СуммаНДС            =       СтрокаНДС.СуммаНДС;
			ДвижениеНДС.ОборотПоРеализации	=       СтрокаНДС.СуммаВыручкиПроводки;
		КонецЦикла;
...
//			ДвижениеРеализации.СтруктурноеПодразделение = СтруктураШапкиДокумента.СтруктурноеПодразделение;
		...
		КонецЦикла;	
			...
			ДвижениеРеализации.Склад 	  = Склад;
			ДвижениеРеализации.ДоговорКонтрагента = ДоговорКонтрагента;
			ДвижениеРеализации.Сумма 			  = Товары.Итог("СуммаПроводки");
			ДвижениеРеализации.НДС 				  = Товары.Итог("СуммаНДСПроводки");
			Движения.РеализацияТМЗ.Записать();
	Иначе								
	...
	Для Каждого СтрокаСписания ИЗ ТаблицаПоТоварам Цикл
		...
		СтрокаСписания.СписанныеПартииБУ = УправлениеЗапасамиПартионныйУчет.ПустаяТаблицаСписанныхПартий("Бух");
		
		СтрокаСписания.Сумма = 0;
		СтрокаСписания.НДС = 0;
		...
		Для каждого СтрокаТаблицыСписания Из ТаблицаСписания Цикл 
			...
			ТаблицаПартийБУ = СтрокаТаблицыСписания.СписанныеПартииБУ;
			//Для каждого текПартия из ТаблицаПартийБУ Цикл
			НоваяСтрока = СтрокаСписания.СписанныеПартииБУ.Добавить();
			НоваяСтрока.Партия = СтрокаСписания.ЗерноваяРасписка;
			НоваяСтрока.Количество = ТаблицаПартийБУ.Итог("Количество");
			НоваяСтрока.СуммаСписания = ТаблицаПартийБУ.Итог("СуммаСписания");
			НоваяСтрока.СуммаСписанияПартии = ТаблицаПартийБУ.Итог("СуммаСписанияПартии");
			НоваяСтрока.СчетУчетаБУ = СтрокаСписания.СчетУчетаБУ;
		КонецЦикла;
		//КонецЦикла;
	КонецЦикла;
	...
	КонецЕсли;
//	КонецЕсли;
	//УправлениеРесурсами.ДвижениеПартийТоваров(ЭтотОбъект,ТаблицаПоТоварам, СтруктураШапкиДокумента , Отказ);
	...
	ТаблицыДокумента.Вставить("ТаблицаПоТоварамКомСбор",	ТаблицаПоТоварамКомСбор);    
	...
	//					|СтавкаНДС",
	//					"Сумма, СуммаВал,СуммаБезНДС, СуммаБезНДСВал, НДС, НДСВал,
	//					|СуммаКомиссионногоСбора, НДСКомиссионногоСбора, Акциз, АкцизВал, Количество");
	...
	Для каждого СтрокаТаблицы из Товары Цикл
		...
		//	Продолжить;
		//КонецЕсли;
		СуммаВзаиморасчетов = СтрокаТаблицы.Сумма*КурсВзаиморасчетов;
		СуммаВзаиморасчетовВал = СтрокаТаблицы.Сумма;
		
		СуммаНДСВзаиморасчетов = СтрокаТаблицы.СуммаНДС*КурсВзаиморасчетов;
		СуммаНДСВзаиморасчетовВал = СтрокаТаблицы.СуммаНДС;
		...
		СуммаПроводки 	 = ?(СуммаВключаетНДС,(СуммаВзаиморасчетов-?(СуммаНДСВзаиморасчетов=0,0,(СуммаНДСВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов)))-(СуммаКомиссионногоСбораВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов)-СуммаНДСКомиссионногоСбораВзаиморасчетов,СуммаВзаиморасчетов-(СуммаКомиссионногоСбораВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов)-СуммаНДСКомиссионногоСбораВзаиморасчетов);
		СуммаПроводкиВал = ?(СуммаВключаетНДС,(СуммаВзаиморасчетовВал-?(СуммаНДСВзаиморасчетовВал=0,0,(СуммаНДСВзаиморасчетовВал-СуммаНДСКомиссионногоСбораВзаиморасчетовВал)))-(СуммаКомиссионногоСбораВзаиморасчетовВал-СуммаНДСКомиссионногоСбораВзаиморасчетовВал)-СуммаНДСКомиссионногоСбораВзаиморасчетовВал,СуммаВзаиморасчетовВал-(СуммаКомиссионногоСбораВзаиморасчетовВал-СуммаНДСКомиссионногоСбораВзаиморасчетовВал)-СуммаНДСКомиссионногоСбораВзаиморасчетовВал);
		...
		///////////////////////////////////////////////////
		НоваяСтрокаВыручки = ТаблицаВыручки.Добавить();
		
		НоваяСтрокаВыручки.СчетУчетаБУ = СтрокаТаблицы.СчетУчетаБУ;
		НоваяСтрокаВыручки.Номенклатура = СтрокаТаблицы.Номенклатура;
		НоваяСтрокаВыручки.ЗерноваяРасписка = СтрокаТаблицы.ЗерноваяРасписка;
		НоваяСтрокаВыручки.Сумма = СуммаПроводки;
		...
		//
		Если СуммаНДСВзаиморасчетов>0 Тогда
			Проводка = ПроводкиБУ.Добавить();
			
			Проводка.Период      = ДатаДока;
			Проводка.Организация = СтруктураШапкиДокумента.Организация;
			Проводка.Сумма       = СуммаНДСВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
			Проводка.Содержание  = "НДС";
			...
			Если СтруктураШапкиДокумента.СчетУчетаРасчетовСКонтрагентом.Валютный Тогда
				...
				//Если документ в иностранной валюте, он может быть выписан только в валюте взаиморасчетов
				Проводка.ВалютаДт        = СтруктураШапкиДокумента.ВалютаВзаиморасчетов;
				Проводка.ВалютнаяСуммаДт = СуммаНДСВзаиморасчетовВал-СуммаНДСКомиссионногоСбораВзаиморасчетовВал;
			...
			НоваяСтрокаВыручки.Номенклатура = СтрокаТаблицы.Номенклатура;
			НоваяСтрокаВыручки.ЗерноваяРасписка = СтрокаТаблицы.ЗерноваяРасписка;
			НоваяСтрокаВыручки.НДС = СуммаНДСВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
			
			ПроводкаНУ = ПроводкиНУ.Добавить();
			
			ПроводкаНУ.Период      = ДатаДока;
			ПроводкаНУ.Организация = СтруктураШапкиДокумента.Организация;
			ПроводкаНУ.Сумма       = СуммаНДСВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
		...
		СтрокаДвижения.ВидОперацииРеализации	= СтрокаТаблицы.НДСВидОперацииРеализации;		
		...
		СтрокаДвижения.СуммаНДС    				= ?(СуммаНДСВзаиморасчетов>0,СуммаНДСВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов,0);			
		СтрокаДвижения.ОборотПоРеализации		= СуммаОборотаПоРеализации;
		
		Если СуммаКомиссионногоСбораВзаиморасчетов > 0 Тогда
			Проводка = ПроводкиБУ.Добавить();
			
			Проводка.Период      = ДатаДока;
			Проводка.Организация = СтруктураШапкиДокумента.Организация;
			Проводка.Сумма   	 = СуммаКомиссионногоСбораВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
			...
			Если СтруктураШапкиДокумента.СчетУчетаРасчетовСКонтрагентом.Валютный Тогда
				...
				//Если документ в иностранной валюте, он может быть выписан только в валюте взаиморасчетов
				Проводка.ВалютаДт        = СтруктураШапкиДокумента.ВалютаВзаиморасчетов;
				Проводка.ВалютнаяСуммаДт = СуммаКомиссионногоСбораВзаиморасчетовВал-СуммаНДСКомиссионногоСбораВзаиморасчетовВал;
			...
			Проводка.Содержание = "Коммиссионный сбор";
			
			Если Проводка.СчетКт.Валютный Тогда
				//Если документ в иностранной валюте, он может быть выписан только в валюте взаиморасчетов
				Проводка.ВалютаКт        = СтруктураШапкиДокумента.ВалютаВзаиморасчетов;
				Проводка.ВалютнаяСуммаКт = СуммаКомиссионногоСбораВзаиморасчетовВал-СуммаНДСКомиссионногоСбораВзаиморасчетовВал;
			КонецЕсли;
			...
			НоваяСтрокаВыручки.Номенклатура = СтрокаТаблицы.Номенклатура;
			НоваяСтрокаВыручки.ЗерноваяРасписка = СтрокаТаблицы.ЗерноваяРасписка;
			НоваяСтрокаВыручки.Сумма = СуммаКомиссионногоСбораВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
			ОсновнаяСтавкаНДС = Справочники.СтавкиНДС.ПустаяСсылка();
			Если товары.Количество()>0 тогда
				ОсновнаяСтавкаНДС = Товары[0].СтавкаНДС;	
			КонецЕсли;
			#Если Клиент тогда
				ОсновнаяСтавкаНДС = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глТекущийПользователь, "ОсновнаяСтавкаНДС");
			#Конецесли
			СтрокаДвижения							= ТаблицаДвижений_НДС.Добавить();		     		
			...
			СтрокаДвижения.ТМЗ		 			= СтрокаТаблицы.Номенклатура;
			СуммаОборотаПоРеализации 			= СуммаКомиссионногоСбораВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
			
			СтрокаДвижения.СуммаБезНДС 			= СуммаКомиссионногоСбораВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
			СтрокаДвижения.СуммаНДС    			= СуммаНДСКомиссионногоСбораВзаиморасчетов;			
			СтрокаДвижения.ОборотПоРеализации	= СуммаОборотаПоРеализации;
			...
			ПроводкаНУ.Период      = ДатаДока;
			ПроводкаНУ.Организация = СтруктураШапкиДокумента.Организация;
			ПроводкаНУ.Сумма   	 = СуммаКомиссионногоСбораВзаиморасчетов-СуммаНДСКомиссионногоСбораВзаиморасчетов;
			...
			Проводка.Период      = ДатаДока;
			Проводка.Организация = СтруктураШапкиДокумента.Организация;
			Проводка.Сумма       = СуммаНДСКомиссионногоСбораВзаиморасчетов;
			Проводка.Содержание  = "НДС комиссионного сбора";
			...
			Если СтруктураШапкиДокумента.СчетУчетаРасчетовСКонтрагентом.Валютный Тогда
				...
				//Если документ в иностранной валюте, он может быть выписан только в валюте взаиморасчетов
				Проводка.ВалютаДт        = СтруктураШапкиДокумента.ВалютаВзаиморасчетов;
				Проводка.ВалютнаяСуммаДт = СуммаНДСКомиссионногоСбораВзаиморасчетовВал;
				
			КонецЕсли;
			
			Проводка.СчетКт = Планысчетов.Типовой.НалогНаДобавленнуюСтоимость;
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"НалогиСборыОтчисления", Справочники.НалогиСборыОтчисления.НалогНаДобавленнуюСтоимость);
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"ВидыПлатежейВБюджетИФонды", Перечисления.ВидыПлатежейВБюджетИФонды.Налог);
			...
			НоваяСтрокаВыручки.Номенклатура = СтрокаТаблицы.Номенклатура;
			НоваяСтрокаВыручки.ЗерноваяРасписка = СтрокаТаблицы.ЗерноваяРасписка;
			НоваяСтрокаВыручки.НДС = СуммаНДСКомиссионногоСбораВзаиморасчетов;
			...
			ПроводкаНУ.Период      = ДатаДока;
			ПроводкаНУ.Организация = СтруктураШапкиДокумента.Организация;
			ПроводкаНУ.Сумма       = СуммаНДСКомиссионногоСбораВзаиморасчетов;
			ПроводкаНУ.Содержание  = "НДС";
	...
	КонецЦикла;
//	//По табличной части процены возврата
	
	Для Каждого СтрокаТаблицы ИЗ ПроцентыВозврата Цикл
		

		
		СуммаПроводки = ?(СуммаВключаетНДС,(СтрокаТаблицы.Сумма-СтрокаТаблицы.СуммаНДС-(СтрокаТаблицы.СуммаКомиссионногоСбора-СтрокаТаблицы.СуммаНДСКомиссионногоСбора)),(СтрокаТаблицы.Сумма-(СтрокаТаблицы.СуммаКомиссионногоСбора-СтрокаТаблицы.СуммаНДСКомиссионногоСбора)));
		СуммаНДСПроводки = СтрокаТаблицы.СуммаНДС-СтрокаТаблицы.СуммаНДСКомиссионногоСбора;
		...
		Проводка.Содержание = "коэффициент возврата";
		...
		ПроводкаНУ.Содержание = "коэффициент возврата";
		ПроцедурыНалоговогоУчета.ВидУчетаНУ(ПроводкаНУ,  ВидУчетаНУ);
	...
	////фиксация ком.сбора по БУ
	//
	//Для каждого СтрокаКомСбора ИЗ ТаблицаПоТоварамКомСбор Цикл
	//	Если СтрокаКомСбора.Сумма = 0 Тогда
	//		Продолжить;
	//	КонецЕсли;
	//	
	//	СуммаПроводки 	 = СтрокаКомСбора.СуммаБезНДС;						
	//	СуммаПроводкиВал = СтрокаКомСбора.СуммаБезНДСВал;
	//	
	...
	//		Проводка.Период      = ДатаДока;
	//		Проводка.Организация = СтруктураШапкиДокумента.Организация;
	//		Проводка.Сумма   	 = СуммаПроводки-СтрокаКомСбора.НДС;
	//		
	//		ПроцедурыБухгалтерскогоУчета.УстановитьПодразделенияПроводки(
	//		Проводка, СтруктураШапкиДокумента.СтруктурноеПодразделение, СтруктураШапкиДокумента.СтруктурноеПодразделение);
	//		
	...
	//			//Если документ в иностранной валюте, он может быть выписан только в валюте взаиморасчетов
	//			Проводка.ВалютаДт        = СтруктураШапкиДокумента.ВалютаВзаиморасчетов;
	//			Проводка.ВалютнаяСуммаДт = СуммаПроводкиВал-СтрокаКомСбора.НДСВал;
	//			
	//		КонецЕсли;
	//		
	//		Проводка.СчетКт 	 = СтрокаКомСбора.СчетДоходовКомиссионногоСбораБУ;
	//		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1,           СтрокаКомСбора.СубконтоДоходовБУ1);
	//		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 2,           СтрокаКомСбора.СубконтоДоходовБУ2);
	//		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 3,           СтрокаКомСбора.СубконтоДоходовБУ3);			
	//		
	//		Проводка.Содержание = "Ком.сбор от реализации";
	//		
	//		Если Проводка.СчетКт.Валютный Тогда
	//			//Если документ в иностранной валюте, он может быть выписан только в валюте взаиморасчетов
	//			Проводка.ВалютаКт        = СтруктураШапкиДокумента.ВалютаВзаиморасчетов;
	...
	//	КонецЕсли;		
	//	// НДС
	//	Если СтруктураШапкиДокумента.УчитыватьНДС И СтрокаКомСбора.НДС <> 0 Тогда
	//		
	//		Проводка = ПроводкиБУ.Добавить();
	//		
	//		Проводка.Период      = ДатаДока;
	//		Проводка.Организация = СтруктураШапкиДокумента.Организация;
	//		Проводка.Сумма       = СтрокаКомСбора.НДС;
	//		Проводка.Содержание  = "НДС ком.сбора";
	//		
	//		ПроцедурыБухгалтерскогоУчета.УстановитьПодразделенияПроводки(
	//		Проводка, СтруктураШапкиДокумента.СтруктурноеПодразделение, СтруктураШапкиДокумента.СтруктурноеПодразделение);
	//		
	...
	//			//Если документ в иностранной валюте, он может быть выписан только в валюте взаиморасчетов
	//			Проводка.ВалютаДт        = СтруктураШапкиДокумента.ВалютаВзаиморасчетов;
	//			Проводка.ВалютнаяСуммаДт = СтрокаКомСбора.НДСВал;
	//			
	//		КонецЕсли;
	//		
	//		Проводка.СчетКт = СтрокаКомСбора.СчетУчетаНДСПоРеализации;
	//		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"НалогиСборыОтчисления", Справочники.НалогиСборыОтчисления.НалогНаДобавленнуюСтоимость);
	//		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"ВидыПлатежейВБюджетИФонды", Перечисления.ВидыПлатежейВБюджетИФонды.Налог);
	//		
	//	КонецЕсли;
	//КонецЦикла; //фиксация ком.сбора по БУ
	//
	...
	//		
	//		//фиксация ком.сбора по НУ
	//		ВыручкаКомСбораПоНУ = ТаблицаПоТоварамКомСбор.Скопировать();
	//		ВыручкаКомСбораПоНУ.Свернуть("СчетУчетаНДСПоРеализации, СчетУчетаАкцизаПоРеализации, СчетДоходовКомиссионногоСбораНУ, СубконтоДоходовНУ1,СубконтоДоходовНУ2,СубконтоДоходовНУ3,
	//									|СтавкаНДС",
	//									"Сумма, СуммаБезНДС, НДС, Акциз,  Количество");
	//		Для каждого СтрокаТаблицы из ВыручкаКомСбораПоНУ Цикл			
	//			// Выручка
	//			Если СтрокаТаблицы.Сумма = 0 Тогда
	//				Продолжить;
	//			КонецЕсли;
	...
	//				
	//				
	//				ПроводкаНУ.СчетКт 	 = СтрокаТаблицы.СчетДоходовКомиссионногоСбораНУ;
	//				ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(ПроводкаНУ.СчетКт, ПроводкаНУ.СубконтоКт, 1,           СтрокаТаблицы.СубконтоДоходовНУ1);
	//				ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(ПроводкаНУ.СчетКт, ПроводкаНУ.СубконтоКт, 2,           СтрокаТаблицы.СубконтоДоходовНУ2);
	//				ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(ПроводкаНУ.СчетКт, ПроводкаНУ.СубконтоКт, 3,           СтрокаТаблицы.СубконтоДоходовНУ3);			
	//				
	//				ПроводкаНУ.Содержание = "Ком.сбор от реализации";
	//				ПроцедурыНалоговогоУчета.ВидУчетаНУ(ПроводкаНУ,  ВидУчетаНУ);
	//				
	//			КонецЕсли; 
	//			
	...
	//	КонецЕсли; 
		...
		ТаблицаВыручкиРеализацияТМЗ = ТаблицаВыручки.Скопировать();
		ТаблицаВыручкиРеализацияТМЗ.Свернуть("СчетУчетаБУ, Номенклатура, Партия","Сумма, СуммаСписания, СуммаКомиссионногоСбора, Количество, НДС, НДСКомиссионногоСбора, Акциз");
		...
		Для Каждого СтрокаРеализации ИЗ ТаблицаВыручкиРеализацияТМЗ Цикл
			...
			ДвижениеРеализации.Склад 	  				= СтруктураШапкиДокумента.Склад;
			ДвижениеРеализации.ДоговорКонтрагента 		= СтруктураШапкиДокумента.ДоговорКонтрагента;
			ДвижениеРеализации.ВидОперации  			= СтруктураШапкиДокумента.ВидОперации;
			ДвижениеРеализации.ТипОперации				= СтруктураШапкиДокумента.ТипОперации;
			ДвижениеРеализации.Партия 	  				= СтрокаРеализации.Партия;
			ДвижениеРеализации.Видресурса				= УправлениеРесурсами.ПолучитьВидРесурса(СтруктураШапкиДокумента.источникФинансирования, СтрокаРеализации.СчетУчетаБУ);
			ДвижениеРеализации.ВидДвиженияЗерна			= СтрокаРеализации.ВидДвиженияЗерна;
			ДвижениеРеализации.ВидКультуры				= СтрокаРеализации.Зерноваярасписка.ВидКультуры;
			ДвижениеРеализации.КлассКультуры			= СтрокаРеализации.Зерноваярасписка.Классностькультуры;
			ДвижениеРеализации.ГодУрожая				= СтрокаРеализации.Зерноваярасписка.ГодУрожая;
		...
		Движение.Событие 			= Справочники.дог_СобытияПоДоговору.Реализация;
...
Процедура ДвиженияРегистровСведенийПоСтроке(СтрокаТаблицы,СтруктураШапкиДокумента, КонтрагентОрганизации)
		...
		// Движения по регистру сведений "Индоссаменты зерновой расписки"
		//
		...
		// Ресурсы
		Движение.Индоссат         = КонтрагентОрганизации;
		Движение.Индоссант        = СтруктураШапкиДокумента.Контрагент;
		...
		если СтатусПогашения = Неопределено Тогда
			...
			// Ресурсы
			Движение.ВидПогашения         = Перечисления.ур_ВидыПогашенияЗерновыхРасписок.Выдача;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаДокумента = УчетНДСИАкциза.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Товары") + УчетНДСИАкциза.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Услуги") + УчетНДСИАкциза.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "ПроцентыВозврата");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураШапкиДокумента.Вставить("Получатель", Контрагент);
	...
	СтруктураПолей.Вставить("ВидДвиженияЗерна", "ВидДвиженияЗерна");
	СтруктураПолей.Вставить("ЗерноваяРасписка", "ЗерноваяРасписка");
	СтруктураПолей.Вставить("Количество"  , "Количество");
	СтруктураПолей.Вставить("Коэффициент"  , "Коэффициент");
	СтруктураПолей.Вставить("Сумма"       , "Сумма");
	СтруктураПолей.Вставить("СуммаКомиссионногоСбора"       , "СуммаКомиссионногоСбора");
	
	СтруктураПолей.Вставить("СтавкаНДС"   , "СтавкаНДС");
	СтруктураПолей.Вставить("НДС"         , "СуммаНДС");
	СтруктураПолей.Вставить("НДСКомиссионногоСбора"         , "СуммаНДСКомиссионногоСбора");
	...
	СтруктураПолей.Вставить("СубконтоСписанияСебестоимостиБУ4",	"СубконтоСписанияСебестоимостиБУ4");
	...
	// Получим необходимые данные по ком.сборам для проведения и проверки заполенения данные 
	...
	СтруктураПолей.Вставить("Количество"  , "Количество");
	СтруктураПолей.Вставить("Коэффициент"  , "Коэффициент");
	СтруктураПолей.Вставить("Сумма"       , "СуммаКомиссионногоСбора");
	
	СтруктураПолей.Вставить("СтавкаНДС"   , "СтавкаНДС");
	СтруктураПолей.Вставить("НДС"         , "СуммаНДСКомиссионногоСбора");
	СтруктураПолей.Вставить("НДСВидОперацииРеализации", "НДСВидОперацииРеализации");	
	СтруктураПолей.Вставить("СчетУчетаНДСПоРеализации",  "СчетУчетаНДСПоРеализации");
	...
	//СтруктураПолей.Вставить("СчетДоходовКомиссионногоСбораБУ","СчетДоходовКомиссионногоСбораБУ");
	
	СтруктураПолей.Вставить("СубконтоДоходовБУ1", "СубконтоДоходовБУ1");	
	СтруктураПолей.Вставить("СубконтоДоходовБУ2", "СубконтоДоходовБУ2");
	СтруктураПолей.Вставить("СубконтоДоходовБУ3", "СубконтоДоходовБУ3");
	
	СтруктураПолей.Вставить("СчетДоходовКомиссионногоСбораНУ",	  "СчетДоходовКомиссионногоСбораНУ");
	СтруктураПолей.Вставить("СубконтоДоходовНУ1", "СубконтоДоходовНУ1");
	СтруктураПолей.Вставить("СубконтоДоходовНУ2", "СубконтоДоходовНУ2");
	СтруктураПолей.Вставить("СубконтоДоходовНУ3", "СубконтоДоходовНУ3");

	РезультатЗапросаПоТоварамКомСбор = УправлениеЗапасами.СформироватьЗапросПоТабличнойЧасти(ЭтотОбъект, "Товары", СтруктураПолей);
	
	ТаблицаПоТоварамКомСбор = ПодготовитьТаблицуТоваровКомСбор(РезультатЗапросаПоТоварамКомСбор, СтруктураШапкиДокумента);
	...
	ТаблицаРасходаПоЗерновымРаспискам = УправлениеРесурсами.ПодготовитьТаблицуРасходаЗерновыхРасписокПриРеализации(ЭтотОбъект);
	ТаблицаПриходаПоЗерновымРаспискам = УправлениеРесурсами.ПодготовитьТаблицуПриходаЗерновыхРасписокПриРеализации(ЭтотОбъект);
	ПроверитьЗаполнениеЗерновыхРасписокПоПриходу(ТаблицаПриходаПоЗерновымРаспискам, Отказ, Заголовок);
	...
	УправлениеВзаиморасчетами.ПодготовкаТаблицыЗначенийДляЦелейПриобретенияИРеализации(ТаблицаПоТоварамКомСбор, СтруктураШапкиДокумента, Истина);
	...
	//УправлениеРесурсами.ПроверитьОстатки(ЭтотОбъект,СтруктураШапкиДокумента,Отказ);
