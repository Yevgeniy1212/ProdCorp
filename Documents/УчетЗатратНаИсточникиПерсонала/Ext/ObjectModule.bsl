...
Функция СформироватьЗапросПоЗатратам(Режим)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗатратыНаИсточникиПерсонала КАК Регистр
	               |		ПО Регистр.Период = Док.Дата И Регистр.ИсточникПерсонала = Док.ИсточникПерсонала
	               |
	               |ГДЕ
...
Процедура ПроверитьЗаполнениеСтрокиЗатрат(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если Не ЕстьИсточникПерсонала Тогда
		ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не выбран источник!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамНакоплений(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, 
	...
	Если СтруктураПроведенияПоРегистрамНакоплений.Свойство(ИмяРегистра) Тогда
		...
		// Ресурсы
		Движение.СуммаЗатрат	= ОбщегоНазначения.ПересчитатьИзВалютыВВалюту(ВыборкаПоСтрокамДокумента.Сумма,ВыборкаПоСтрокамДокумента.Валюта,ВалютаУправленческогоУчета,
																		ВыборкаПоСтрокамДокумента.КурсДокумента ,СтруктураКурса.Курс,
																		ВыборкаПоСтрокамДокумента.КратностьДокумента,СтруктураКурса.Кратность);
