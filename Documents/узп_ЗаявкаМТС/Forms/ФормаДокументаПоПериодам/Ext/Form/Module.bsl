...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если Не ЗначениеЗаполнено(Объект.Сценарий) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Перед записью необходимо заполнить сценарий планирования заявки!'");
...
Процедура НоменклатураПриИзмененииНаСервере(Идентификатор,ТЧ = "ТаблицаТовары")
	...
	Если ТипЗнч(ТекущиеДанные.Номенклатура) = Тип("СправочникСсылка.фин_ПлановаяНоменклатура")
		ИЛИ ВремСтрока.Цена<>0 Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеСтрокиТаблицы 					= Новый Структура(СтруктураПериодов + "НомерСтроки,Номенклатура,Характеристика,ЕдиницаИзмерения,Коэффициент,Подразделение,Проект,Приоритет,Описание");
...
Процедура ТоварыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ДобавитьВСписок Тогда
		...
		//НС.Количество = 1;
		Для НомерПериода = 1 По ПродолжительностьЦикла Цикл
			НС["ТоварыКоличество_" + НомерПериода]	= 0;
		КонецЦикла;
...
Процедура ТоварыПериодПриИзменении(Элемент)
	...
	Если Найти(ИмяЭлемента,"ТоварыСумма")>0 Тогда
		Если ТекущиеДанные["ТоварыКоличество"+"_"+НомерКолонки]=0 Тогда
			ТекущиеДанные["ТоварыКоличество"+"_"+НомерКолонки]=1;
		КонецЕсли;
		ТекущиеДанные["ТоварыЦена"+"_"+НомерКолонки]= Окр(ТекущиеДанные["ТоварыСумма"+"_"+НомерКолонки]/ТекущиеДанные["ТоварыКоличество"+"_"+НомерКолонки],2);
	Иначе
		ТекущиеДанные["ТоварыСумма"+"_"+НомерКолонки]= ТекущиеДанные["ТоварыКоличество"+"_"+НомерКолонки]*ТекущиеДанные["ТоварыЦена"+"_"+НомерКолонки]*ТекущиеДанные.коэффициент
	КонецЕсли;
	Если не ЗначениеЗаполнено(ТекущиеДанные["ТоварыПериод"+"_"+НомерКолонки]) Тогда
		ТекущиеДанные["ТоварыПериод"+"_"+НомерКолонки] = ИнкрементПериода(Объект.Сценарий,Объект.ПериодПланирования,НомерКолонки);
	КонецЕсли;
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура ТаблицаУслугиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ДобавитьВСписок Тогда
		...
		НС.Номенклатура = ВыбранноеЗначение;
		Для НомерПериода = 1 По ПродолжительностьЦикла Цикл
			НС["УслугиКоличество_" + НомерПериода]	= 0;
		КонецЦикла;
		НС.НомерСтроки = ТаблицаУслуги.Индекс(НС)+1;
		НоменклатураПриИзмененииНаСервере(НС.ПолучитьИдентификатор(),"ТаблицаУслуги");
...
Процедура УслугиПериодПриИзменении(Элемент)
	...
	ТекущиеДанные = Элементы.ТаблицаУслуги.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если не ЗначениеЗаполнено(Элементы.ТаблицаУслуги.ТекущиеДанные["УслугиПериод"+"_"+НомерКолонки]) Тогда
		Элементы.ТаблицаУслуги.ТекущиеДанные["УслугиПериод"+"_"+НомерКолонки] = ДобавитьМесяц(Объект.ПериодПланирования, НомерКолонки-1);
	КонецЕсли;	
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ТаблицаТоварыПодразделение.Видимость = НЕ (Объект.ИсточникЗаявки=ПредопределенноеЗначение("Перечисление.узп_ИсточникиЗаявокМТС.Подразделение"));
	Элементы.ТаблицаУслугиПодразделение.Видимость = НЕ (Объект.ИсточникЗаявки=ПредопределенноеЗначение("Перечисление.узп_ИсточникиЗаявокМТС.Подразделение"));
	ПродолжительностьЦикла = ПродолжительностьЦикла(Объект.Сценарий, Объект.ПериодПланирования);
...
Процедура ЗагрузкаСпецификацииВТЗ()
	...
	Для НомерПериода = 1 По ПродолжительностьЦикла Цикл
		...
		ТекущийПериод = фин_ПроцедурыМеханизмовБюджетированияКлиентСервер.ПолучитьПериодСтрокой(ИнкрементПериода(Объект.Сценарий,Объект.ПериодПланирования,НомерПериода),Объект.Сценарий);
	...
	Для каждого СтрокаТаблица из Таблицы Цикл
		Если НЕ ИспользоватьПроекты Тогда
			СтрокаТаблица.ПолеПроекта.Видимость = Ложь;
		КонецЕсли;
		СтрокаТаблица.Таблица.Очистить();
		ТекущийПериодДата = Объект.ПериодПланирования;
		Для НомерПериода = 1 По ПродолжительностьЦикла Цикл
			// создаем реквизиты
			ДобавляемыеРеквизиты = Новый Массив;
			//ТекущийПериод = фин_ПроцедурыМеханизмовБюджетированияКлиентСервер.ПолучитьПериодСтрокой(ИнкрементПериода(Объект.Сценарий,Объект.ПериодПланирования,НомерПериода),Объект.Сценарий);
			ТекущийПериод = фин_ПроцедурыМеханизмовБюджетированияКлиентСервер.ПолучитьПериодСтрокой(ТекущийПериодДата,Объект.Сценарий);
			ТекущийПериодДата = фин_ПроцедурыМеханизмовБюджетированияКлиентСервер.ДобавитьИнтервал(Объект.ПериодПланирования,Объект.Сценарий,НомерПериода);
			//создаем элементы
			...
			Для Каждого СтрокаТЧ Из СтрокаТаблица.Объект Цикл
				...
				Если СтрокаТаблица.УчетКоэфЕдИзм Тогда
					СтрокаУчетКоэфЕдИзм = ", ЕдиницаИзмерения, Коэффициент";
				Иначе
					СтрокаУчетКоэфЕдИзм = "";
				КонецЕсли;
				ПараметрыПоиска = Новый Структура("Номенклатура,Подразделение,Описание,ОбъектРемонта" + СтрокаПроект + СтрокаХарактеристика + СтрокаУчетКоэфЕдИзм,);
				ПараметрыПоиска.Номенклатура 			= СтрокаТЧ.Номенклатура;
				ПараметрыПоиска.Подразделение 			= мПодразделение;
				ПараметрыПоиска.Описание 				= СтрокаТЧ.Описание;
				ПараметрыПоиска.ОбъектРемонта 			= СтрокаТЧ.ОбъектРемонта;
				Если ИспользоватьПроекты Тогда 
					ПараметрыПоиска.Проект 				= СтрокаТЧ.Проект;
				КонецЕсли;
				Если ИспользоватьХарактеристики Тогда
				...
				Если СтрокаТаблица.УчетКоэфЕдИзм Тогда
					...
					ПараметрыПоиска.Коэффициент 		= СтрокаТЧ.Коэффициент;
				КонецЕсли;
...
Процедура РассчетОбщейСуммыТоварногоДоговора()
	...
	Для НомерПериода = 1 По ПродолжительностьЦикла(Объект.Сценарий, Объект.ПериодПланирования) Цикл
		...
		//Элементы["КолонкаТоварыСумма"+"_"+НомерПериода].ТекстПодвала = ТаблицаТовары.Итог("ТоварыСумма"+"_"+НомерПериода);
	КонецЦикла;
