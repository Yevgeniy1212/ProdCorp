...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	СтруктураОбязательныхПолей = Новый Структура("Организация, ИсточникФинансирования, ИсточникФинансированияПолучателя, СкладОтправитель, СкладПолучатель, ТипОперации, ТипОперацииПолучателя");
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, 
	...
	//	УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров(ТаблицаСписания, Истина, СтруктураШапкиДокумента.НеобходимостьОтраженияВНУ, Отказ,СтрокаСписания.ЗерноваяРасписка,"Ухудшение качества ресурсов зерна");
	//КонецЦикла;
	...
	Для Каждого СтрокаТЗ ИЗ Товары Цикл
		...
		//Получим источник финансирования ухудшаемого ресурса
		...
		Запрос.УстановитьПараметр("СчетУчетаБУ",СтрокаТЗ.СчетУчетаБУ);
		...
		Запрос.УстановитьПараметр("ЗерноваяРасписка",СтрокаТЗ.ЗерноваяРасписка);
		...
		ИсточникФинансированияУР = ИсточникФинансирования;
		
		Если Результат.Следующий() Тогда
			ИсточникФинансированияУР = Результат.ИсточникФинансирования;
		Иначе
			Сообщить("Количество ресурса "+ СтрокаТЗ.Номенклатура+" на складе "+СтруктураШапкиДокумента.Склад+" равно 0");
			Отказ = Истина;
		КонецЕсли;
		
		//Если ИсточникФинансированияУР = ИсточникФинансирования Тогда
			//Списание номенклатуры при ухудшении качества
			Проводка = ПроводкиБУ.Добавить();
			
			Проводка.Период       = ДатаДока;
			Проводка.Организация  = СтруктураШапкиДокумента.Организация;
			//Проводка.ИсточникФинансирования  = СтруктураШапкиДокумента.ИсточникФинансированияПолучателя;
			Проводка.Содержание   = "Ухудшение качества ресурсов";
			
			Проводка.СчетДт       = СтрокаТЗ.НовыйСчетУчетаБУ;
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Номенклатура", СтрокаТЗ.НоменклатураЗамена,Истина, Заголовок);
			...
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Номенклатура", СтрокаТЗ.Номенклатура,Истина, Заголовок);
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Склады",       СтруктураШапкиДокумента.Склад);
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Партии",       СтрокаТЗ.ЗерноваяРасписка);	
			...
			Проводка.СчетДт       = СтрокаТЗ.НовыйСчетУчетаНУ;
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Номенклатура", СтрокаТЗ.НоменклатураЗамена,Истина, Заголовок);
			Проводка.КоличествоДт = СтрокаТЗ.Количество;
			
			Проводка.СчетКт       = СтрокаТЗ.СчетУчетаНУ;
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Номенклатура", СтрокаТЗ.Номенклатура,Истина, Заголовок);
		...
		//Иначе
		//	//Списание номенклатуры при ухудшении качества при разных источниках финансирования
		//	Проводка = ПроводкиБУ.Добавить();
		//	
		//	Проводка.Период       = ДатаДока;
		//	Проводка.Организация  = СтруктураШапкиДокумента.Организация;
		//	Проводка.ИсточникФинансирования  = ИсточникФинансированияУР;
		//	Проводка.Содержание   = "Списание ресурсов при ухудшении качества ресурсов";
		//	
		//	Проводка.СчетДт       = ПланыСчетов.Типовой.ПереоформлениеПереводМеждуРесурсами;
		//	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"ТипыОпераций", СтруктураШапкиДокумента.ТипОперации,Истина, Заголовок);
		//							
		//	Проводка.СчетКт       = СтрокаТЗ.НовыйСчетУчетаБУ;
		//	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Номенклатура", СтрокаТЗ.Номенклатура,Истина, Заголовок);
		//	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Склады",       СтруктураШапкиДокумента.Склад);
		//	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Партии",       СтрокаТЗ.ЗерноваяРасписка);	
		//	
		...
		//	Проводка.Организация  = СтруктураШапкиДокумента.Организация;
		//	Проводка.ИсточникФинансирования  = СтруктураШапкиДокумента.ИсточникФинансированияПолучателя;
		//	Проводка.Содержание   = "Поступление при ухудшении качества ресурсов";
		//	
		//	Проводка.СчетДт       = СтрокаТЗ.НовыйСчетУчетаБУ;
		//	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Номенклатура", СтрокаТЗ.НоменклатураЗамена,Истина, Заголовок);
		//	ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Склады",       СтруктураШапкиДокумента.Склад);
...
Процедура ДвиженияРегистровПодсистемыУчетаРесурсов(СтруктураШапкиДокумента, 
		...
		Если Не Отказ Тогда
			...
			Для Каждого СтрокаТЗ Из Товары Цикл
				...
				Движение.Видзерна = СтрокаТЗ.ЗерноваяРасписка.ВидЗерна;
				Если ИсточникФинансирования = Справочники.ИсточникиФинансирования.ГосударственныеСредства Тогда
					Движение.Видресурса				 = Справочники.ур_ВидыРесурсов.ГосударственныйРесурсРеализационногоЗерна;
				Иначе
					Движение.Видресурса				 = Справочники.ур_ВидыРесурсов.КоммерческиеРесурсыЗерна;
				КонецЕсли;
				Движение.Культура = СтрокаТЗ.Номенклатура.ВидКультуры;
				Движение.Класс = СтрокаТЗ.Номенклатура.Класс;
				Движение.ГодУрожая = СтрокаТЗ.ЗерноваяРасписка.ГодУрожая;
				Движение.ЗерноваяРасписка = СтрокаТЗ.ЗерноваяРасписка;
				Движение.ТипОперации = ТипОперации;
				Движение.ЗачтенныйВес = СтрокаТЗ.Количество;
				
				Движение = Движения.ур_ЗерноНаСкладах.ДобавитьПриход();
				Движение.Период = СтруктураШапкиДокумента.Дата;
				Движение.Организация = СтруктураШапкиДокумента.Организация;
				Движение.Склад = СтруктураШапкиДокумента.Склад;
				Движение.Видзерна = СтрокаТЗ.ЗерноваяРаспискаНовая.ВидЗерна;
				Если ИсточникФинансирования = Справочники.ИсточникиФинансирования.ГосударственныеСредства Тогда
					Движение.Видресурса				 = Справочники.ур_ВидыРесурсов.ГосударственныйРесурсРеализационногоЗерна;
				Иначе
					Движение.Видресурса				 = Справочники.ур_ВидыРесурсов.КоммерческиеРесурсыЗерна;
				КонецЕсли;
				Движение.Культура = СтрокаТЗ.НоменклатураЗамена.ВидКультуры;
				Движение.Класс = СтрокаТЗ.НоменклатураЗамена.Класс;
				Движение.ГодУрожая = СтрокаТЗ.ЗерноваяРаспискаНовая.ГодУрожая;
				Движение.ЗерноваяРасписка = СтрокаТЗ.ЗерноваяРаспискаНовая;
				Движение.ТипОперации = ТипОперации;
				Движение.ЗачтенныйВес = СтрокаТЗ.Количество;
				...
				Движение.Зерноваярасписка		 = СтрокаТЗ.ЗерноваяРаспискаНовая;
				Движение.Номенклатура 			 = СтрокаТЗ.НоменклатураЗамена;
				Движение.Класс					 = СтрокаТЗ.НоменклатураЗамена.Класс;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураПолей.Вставить("НоменклатураЗамена"    , "НоменклатураЗамена");
