...
Функция ПечатьИНВ8() Экспорт
	...
	|		ПО ИнвентаризацияОСОС.ОсновноеСредство = СчетаБухгалтерскогоУчетаОсновныхСредствСрезПоследних.ОсновноеСредство
	|
	|ГДЕ
	...
	Шапка.Параметры.ОбъектыСчета      		 = СчетУчета.Наименование;
	...
	Для Каждого СтрокаСчет Из ТаблицаСчетаУчета Цикл
		...
		Иначе
			БалансовыеСчета = строка(БалансовыеСчета) + ", " + СтрокаСчет.СчетУчетаБУ;
		КонецЕсли;	
	КонецЦикла;
...
Функция ПечатьИНВ16() Экспорт
	...
	Шапка.Параметры.ПредставлениеОрганизации = ОбщегоНазначения.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,");
	Шапка.Параметры.ОрганизацияРНН_БИН       = ОбщегоНазначения.ОписаниеОрганизации(СведенияОбОрганизации, "БИН_ИИН,", Ложь);
	Шапка.Параметры.ДокументОснованиеДата    = СтрЗаменить(Формат(Док.ДокументОснованиеДата,"ДЛФ=DD"), "г.", "года");
	
	Шапка.Параметры.НомерДок = ОбщегоНазначения.ПолучитьНомерНаПечать(ЭтотОбъект, глСписокПрефиксовУзлов);
	Шапка.Параметры.ДатаДок  = Формат(Док.Дата, "ДФ=dd.MM.yyyy") + " года";
	...
	СтрокаНадТаблицей.Параметры.ДатаНачалаИнвентаризации    = СтрЗаменить(Формат(Док.ДатаНачалаИнвентаризации,"ДЛФ=DD"), "г.", "года");
	СтрокаНадТаблицей.Параметры.ДатаОкончанияИнвентаризации = СтрЗаменить(Формат(Док.ДатаОкончанияИнвентаризации,"ДЛФ=DD"), "г.", "года");
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "ИНВ8" тогда
		ТабДокумент = ПечатьИНВ8();
	ИначеЕсли ИмяМакета = "ИНВ16" тогда
		ТабДокумент = ПечатьИНВ16();
	КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("ИНВ8,ИНВ16","Инв-8 (Инвентаризационная опись)","ИНВ-16 (сличительная ведомость)");
