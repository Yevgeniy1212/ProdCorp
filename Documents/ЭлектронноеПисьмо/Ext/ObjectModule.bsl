...
Процедура ЗахватитьТекст() Экспорт
	...
	ИмяСохраненияФайла = УниверсальныеМеханизмы.ПолучитьИмяФайла(УниверсальныеМеханизмы.ПолучитьИмяКаталога(глТекущийПользователь), (ИмяФайла + ".HTM"));
	...
	УправлениеЭлектроннойПочтой.ОткрытьФайлДляРедактированияВнешнимХТМЛРедактором(ИмяФайлаРедактированияХТМЛТекста);
...
Процедура ОтправитьПисьмо(ФормаПисьма = Неопределено) Экспорт
	...
	Если УправлениеЭлектроннойПочтой.ПолучитьДоступныеУчетныеЗаписи(глТекущийПользователь).Транспорт.НайтиПоЗначению(УчетнаяЗапись) <> Неопределено Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Если ЗначениеЗаполнено(Основание) И ТипЗнч(Основание) = Тип("ДокументСсылка.ЭлектронноеПисьмо") Тогда
