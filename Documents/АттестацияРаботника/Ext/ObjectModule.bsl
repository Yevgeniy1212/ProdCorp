...
Функция СформироватьЗапросДляПечати(Режим)
	...
	Если Режим = "ПоТабличнойЧастиДокумента" Тогда
		...
		Запрос.УстановитьПараметр("ПустаяКомпетенция", Справочники.КомпетенцииРаботников.ПустаяСсылка());
		Запрос.Текст = 
			"ВЫБРАТЬ
			...
			|
			|ГДЕ
			|	АттестацияРаботникаСписокКомпетенций.Ссылка = &ДокументСсылка И
			|	АттестацияРаботникаСписокКомпетенций.Компетенция <> &ПустаяКомпетенция
			|
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки,
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если      ИмяМакета = "ОценочныйЛист" Тогда
		...
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, РаботаСДиалогами.СформироватьЗаголовокДокумента(ЭтотОбъект,"Оценочный лист"));
...
Процедура ПроверитьЗаполнениеСтрокиОценки(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
		...
		Если Не ЕстьКомпетенция Тогда
			ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не выбрана компетенция!", Отказ, Заголовок);
		КонецЕсли;

		// ДатаНачала
        ЕстьОценка = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Оценка);
		Если Не ЕстьОценка Тогда
			ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не выбрана оценка!", Отказ, Заголовок);
		КонецЕсли;

		// ДатаОкончания
        ЕстьДатаОценки = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОценки);
		Если Не ЕстьДатаОценки Тогда
			ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не указана дата оценки!", Отказ, Заголовок);
		КонецЕсли;
