...
Функция ПечатьГрафика()
	...
	|	ГрафикВознагрождений.Контрагент КАК Контрагент,
	|	ГрафикВознагрождений.Дата КАК ДатаНачала,
	|	ГрафикВознагрождений.Дата КАК ДатаОкончания
	|ИЗ
	|	Документ.НачислениеВознагрожденийПоЗаймамПолученным КАК ГрафикВознагрождений
	|ГДЕ
	...
	|	ГрафикВознагрождений.ДатаПоступления,
	|	ГрафикВознагрождений.ДатаНачалаПериода,
	|	ГрафикВознагрождений.ДатаОкончанияПериода,
	...
	|	ГрафикВознагрождений.СуммаПоступления,
	|	ГрафикВознагрождений.СуммаПогашения,
	|	ГрафикВознагрождений.ВалСуммаВознагрождения,
	|	ГрафикВознагрождений.СуммаВознагрождения
	|ИЗ
	|	Документ.НачислениеВознагрожденийПоЗаймамПолученным.График КАК ГрафикВознагрождений
	|ГДЕ
	|	 ГрафикВознагрождений.ссылка = &ТекДокумент
	|Итоги 
	|	Сумма(ВалСумма),
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ЭтотОбъект, "Начисление вознагрождения по полученным займам", глСписокПрефиксовУзлов);
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоНачислению, Отказ, Заголовок)
	...
	Для Каждого СтрокаТЗ ИЗ ТаблицаПоНачислению Цикл
		...
		Проводка.СчетДт      = СтрокаТЗ.СчетУчетаРасходов;
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,1,СтрокаТЗ.СубконтоРасходов1);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,СтрокаТЗ.СубконтоРасходов2);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,3,СтрокаТЗ.СубконтоРасходов3);
		
		Проводка.СчетКт 	 = СтрокаТЗ.СчетУчетаВознагрождений;
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1,           СтрокаТЗ.Контрагент);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 2,           СтрокаТЗ.ДоговорКонтрагента);
		ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 3,           СтрокаТЗ.ТипОперации);			
		
		Проводка.Содержание = "Начислены % по займам полученным";
		
		Проводка.ВалютаКт        = СтрокаТЗ.Валюта;
		Проводка.ВалютнаяСуммаКт = СтрокаТЗ.ВалСуммаВознагрождения;
		...
		Если СтруктураШапкиДокумента.НеобходимостьОтраженияВНУ Тогда				
			...
			Проводка.СчетДт      = СтрокаТЗ.СчетУчетаРасходовНУ;
			ПроцедурыБухгалтерскогоУчета.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,1,СтрокаТЗ.СубконтоРасходов1);
			...
			Проводка.Содержание = "Начислены % по кредитам и займам";
	...
	СписокВалютПоДокументу = УправлениеЗаймами.ПолучитьСписокВалют(ЭтотОбъект, мВалютаРегламентированногоУчета);
	
	Для каждого СтрокаТЗ ИЗ СписокВалютПоДокументу Цикл
		СтруктураШапкиДокумента.Вставить("ВалютаДокумента",СтрокаТЗ.ВалютаДокумента);
		СтруктураШапкиДокумента.Вставить("КурсДокумента",СтрокаТЗ.КурсДокумента);
	...
	КонецЦикла; // Учет курсовых разниц
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураПолей.Вставить("СчетУчетаРасходов"  , "СчетУчетаРасходов");
	СтруктураПолей.Вставить("СубконтоРасходов1"  , "СубконтоРасходов1");
	СтруктураПолей.Вставить("Субконторасходов2"  , "Субконторасходов2");
	СтруктураПолей.Вставить("СубконтоРасходов3"  , "СубконтоРасходов3");
	СтруктураПолей.Вставить("СчетУчетаВознагрожденийНУ"  , "СчетУчетаВознагрожденийНУ");
	СтруктураПолей.Вставить("СчетУчетаРасходовНУ"  , "СчетУчетаРасходовНУ");
	СтруктураПолей.Вставить("СубконтоРасходовНУ1"  , "СубконтоРасходовНУ1");
	СтруктураПолей.Вставить("СубконторасходовНУ2"  , "СубконторасходовНУ2");
	СтруктураПолей.Вставить("СубконтоРасходовНУ3"  , "СубконтоРасходовНУ3");
