...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если      ИмяМакета = "Диаграмма" Тогда
		...
		Отчет = Отчеты.ЗанятостьПомещений.Создать();
		...
		ТиповыеОтчеты.УстановитьПараметр(Отчет.КомпоновщикНастроек, "НачалоПериода", ?(НЕ ЗначениеЗаполнено(ДатаНачала),НачалоГода(РабочаяДата),НачалоМесяца(ДатаНачала)));
		ТиповыеОтчеты.УстановитьПараметр(Отчет.КомпоновщикНастроек, "КонецПериода", ?(НЕ ЗначениеЗаполнено(ДатаОкончания),КонецГода(РабочаяДата),КонецМесяца(ДатаОкончания)));
...
Функция СформироватьЗапросПоСписокПомещений(Режим)
	...
	ТекстЗапроса = 
		...
		|	ВЫБОР КОГДА ПланируемаяЗанятостьПомещений.Занятость <> &Свободно И НЕ((ПланируемаяЗанятостьПомещений.Занятость) ЕСТЬ NULL ) ИЛИ ПланируемаяЗанятостьПомещений.Период >= &ДатаНачала ТОГДА ""Нельзя"" ИНАЧЕ ""Можно"" КОНЕЦ КАК ПроверяемоеЗначение,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПланируемаяЗанятостьПомещений.СрезПоследних(&ДатаОкончания, Помещение в (ВЫБРАТЬ РАЗЛИЧНЫЕ	РезервированиеПомещенийСписокПомещений.Помещение ИЗ	Документ.РезервированиеПомещений.СписокПомещений КАК РезервированиеПомещенийСписокПомещений ГДЕ	РезервированиеПомещенийСписокПомещений.Ссылка = &ДокументСсылка)) КАК ПланируемаяЗанятостьПомещений
		...
		|		ПО ПерваяТаблица.Ссылка = ВтораяТаблица.Ссылка И ПерваяТаблица.Помещение = ВтораяТаблица.Помещение И ПерваяТаблица.НомерСтроки < ВтораяТаблица.НомерСтроки
		|
		|ГДЕ
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Занятость) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении("Не указана занятость помещений!", Отказ, Заголовок);
	КонецЕсли;

	// Дата
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаНачала) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении("Не задана дата и время резервирования помещений!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.ДатаНачала = ВыборкаПоШапкеДокумента.ДатаОкончания ИЛИ НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаОкончания) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении("Не указана продолжительноть занятости помещений!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиСписокПомещений(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Помещение) Тогда
			ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не выбрано помещение!", Отказ, Заголовок);
		КонецЕсли;
