...
&НаКлиенте
Процедура РаботаСЭкземплярамиБюджетов(Команда)
	ОткрытьФорму("Обработка.фин_РаботаСЭкземплярамиБюджетов.Форма",,,Новый УникальныйИдентификатор,?(фин_ОбщегоНазначенияКлиентПовтИсп.РежимОтдельногоОткрытияОкон(),ВариантОткрытияОкна.ОтдельноеОкно,Окно));
КонецПроцедуры
