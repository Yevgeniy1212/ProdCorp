...
Процедура ЗаполнитьПоДокументуОснования(Основание) Экспорт
	...
	Если ТипОснования = Тип("ДокументСсылка.РегистрацияКандидатов") Тогда
		
		// Заполним реквизиты из стандартного набора.
		ЗаполнениеДокументов.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание);
		
		Если Основание.Проведен Тогда
			ВременнаяТаблица = Основание.Кандидаты.Выгрузить();
			ВременнаяТаблица.Колонки.ДатаПоступленияРезюме.Имя = "ДатаПринятияРешения";
			ВременнаяТаблица.ЗаполнитьЗначения(?(НЕ ЗначениеЗаполнено(Дата),ОбщегоНазначения.ПолучитьРабочуюДату(),Дата),"ДатаПринятияРешения");
			// Заполнение табличной части. 
			Кандидаты.Загрузить(ВременнаяТаблица);
		КонецЕсли;
...
Функция СформироватьЗапросПоКандидаты(Режим)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |					ПО КандидатыВнутри.ФизЛицо = Док.ФизЛицо И КандидатыВнутри.Период <= Док.ДатаПринятияРешения
	               |			
	               |			ГДЕ
	...
	//Если ВыборкаПоСтрокамДокумента.Следующий() Тогда
		...
		Если ЕстьФизЛицо и ЕстьДатаПринятияРешения Тогда
			...
			Если ВыборкаПоСтрокамДокумента.ДатаЗначения = ВыборкаПоСтрокамДокумента.ДатаПринятияРешения Тогда
				ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "по кандидату на дату оценки уже присутствует решение!", Отказ, Заголовок);
			КонецЕсли;
			
			Если ВыборкаПоСтрокамДокумента.ПроверяемоеЗначение = Перечисления.СостоянияКандидатаНаРаботу.Отклонен Тогда
				ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "указан кандидат,по которому принято решение ""Отказать""!", Отказ, Заголовок);
			КонецЕсли;
