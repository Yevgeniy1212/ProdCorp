...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Диаграмма" Тогда
		Если НЕ ПроверитьЗаполнение() Тогда
			Возврат;
		КонецЕсли;
		Мероприятие.ПолучитьОбъект().Печать(Ссылка);
	КонецЕсли;
...
Функция СформироватьЗапросПоРаботники(Режим)
	...
	ТекстЗапроса = 
		...
		|	ВЫБОР КОГДА ПланируемаяЗанятостьФизлиц.Состояние <> &Свободно И НЕ((ПланируемаяЗанятостьФизлиц.Состояние) ЕСТЬ NULL ) ИЛИ ПланируемаяЗанятостьФизлиц.Период >= ПерваяТаблица.ЧастьМероприятияДатаНачала ТОГДА ""Нельзя"" ИНАЧЕ ""Можно"" КОНЕЦ КАК ПроверяемоеЗначение,
		...
		|		ВЫБОР КОГДА УчастиеВМероприятияхРаботники.ЧастьМероприятия.Владелец = УчастиеВМероприятияхРаботники.Ссылка.Мероприятие ТОГДА ЛОЖЬ ИНАЧЕ ИСТИНА КОНЕЦ КАК НевернаяЧастьМероприятия
		|	ИЗ
		...
		|			ПО УчастиеВМероприятияхРаботники.ФизЛицо = ПланируемаяЗанятостьФизлиц.ФизЛицо И УчастиеВМероприятияхРаботники.ЧастьМероприятия.ДатаОкончания > ПланируемаяЗанятостьФизлиц.Период
		|	
		|	ГДЕ
		...
		|		УчастиеВМероприятияхРаботники.ЧастьМероприятия) КАК ПерваяТаблица
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СобытийныйПланЗанятостиФизлиц КАК ПланируемаяЗанятостьФизлиц
		|		ПО ПерваяТаблица.ДатаЗначения = ПланируемаяЗанятостьФизлиц.Период И ПерваяТаблица.ФизЛицо = ПланируемаяЗанятостьФизлиц.ФизЛицо
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УчастиеВМероприятиях.Работники КАК ВтораяТаблица
		|		ПО ПерваяТаблица.Ссылка = ВтораяТаблица.Ссылка И ПерваяТаблица.ФизЛицо = ВтораяТаблица.ФизЛицо И (ПерваяТаблица.ЧастьМероприятия.ДатаНачала <= ВтораяТаблица.ЧастьМероприятия.ДатаНачала И ВтораяТаблица.ЧастьМероприятия.ДатаНачала < ПерваяТаблица.ЧастьМероприятия.ДатаОкончания ИЛИ ПерваяТаблица.ЧастьМероприятия.ДатаНачала < ВтораяТаблица.ЧастьМероприятия.ДатаОкончания И ВтораяТаблица.ЧастьМероприятия.ДатаОкончания <= ПерваяТаблица.ЧастьМероприятия.ДатаОкончания ИЛИ ВтораяТаблица.ЧастьМероприятия.ДатаНачала <= ПерваяТаблица.ЧастьМероприятия.ДатаНачала И ПерваяТаблица.ЧастьМероприятия.ДатаОкончания <= ВтораяТаблица.ЧастьМероприятия.ДатаОкончания) И ПерваяТаблица.НомерСтроки < ВтораяТаблица.НомерСтроки
		|
		|СГРУППИРОВАТЬ ПО
...
Процедура ПроверитьЗаполнениеСтрокиРаботники(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ЧастьМероприятия) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не указана составная часть мероприятия, в которой участвует сотрудник!", Отказ, Заголовок);
	...
	Если ВыборкаПоСтрокамДокумента.ПроверяемоеЗначение <> "Можно" Тогда
		ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "cотрудник в указанный период времени уже занят!", Отказ, Заголовок);
	КонецЕсли;
