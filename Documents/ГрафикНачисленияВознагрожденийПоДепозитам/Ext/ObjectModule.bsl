...
Функция ПечатьГрафика()
	...
	|	ГрафикВознагрождений.БанковскийСчет,
	|	ГрафикВознагрождений.БанковскийСчет.ВалютаДенежныхСредств КАК валюта,
	|	ГрафикВознагрождений.БанковскийСчет.НомерДоговора КАК НомерДоговора,
	|	ГрафикВознагрождений.БанковскийСчет.ДатаНачалаДоговора КАК ДатаНачала,
	|	ГрафикВознагрождений.БанковскийСчет.ДатаОкончанияДоговора КАК ДатаОкончания,
	|	ГрафикВознагрождений.БанковскийСчет.СуммаДоговора КАК Сумма,
	|	ГрафикВознагрождений.БанковскийСчет.ПроцентВознагрождения КАК ПроцентВознагрождения,
	|	ГрафикВознагрождений.БанковскийСчет.ВИдКалендарныхДней КАК КоличествоДней
	|ИЗ
	|	Документ.ГрафикНачисленияВознагрожденийПоДепозитам КАК ГрафикВознагрождений
	|ГДЕ
	...
	|ИЗ
	|	Документ.ГрафикНачисленияВознагрожденийПоДепозитам.График КАК ГрафикВознагрождений
	|ГДЕ
	|	 ГрафикВознагрождений.ссылка = &ТекДокумент
	|Итоги 
	|	Сумма(СуммаКПН),
	|	Сумма(СуммаВознагрожденияКПолучению)
	|ПО Общие
	|";
	...
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ГрафикВознагрожденияПоДепозиту";
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ЭтотОбъект, "График вознагрождения по депозиту", глСписокПрефиксовУзлов);
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьСтрока.Параметры.Период = ПредставлениеПериода(НачалоМесяца(Выборка.Период), КонецМесяца(Выборка.Период), "ФП = Истина");
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	СтруктураОбязательныхПолей = Новый Структура("Организация,БанковскийСчет");
...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамСведений(СтрокаТЗ,СтруктураШапкиДокумента)
	...
	Движение.Объект                     = СтруктураШапкиДокумента.БанковскийСчет;
	// Ресурсы
	...
	// Реквизиты
	Движение.ДатаОкончания                  = СтруктураШапкиДокумента.БанковскийСчет.ДатаОкончанияДоговора;
