...
Процедура ЗаполнитьПоДокументуОснования(Основание) Экспорт
	...
	Если (ТипЗнч(Основание) = Тип("ДокументСсылка.ур_ПоступлениеЗерна")) Тогда
		
		Склад = Основание.Склад;
		ТипОперации = Основание.ТипОперации;
		
		ДокументОснование = Основание;
		
		ЭтотОбъект.Товары.Загрузить(Основание.Товары.Выгрузить());
...
Функция ПодготовитьТаблицуПоЗерновымРаспискам(РезультатЗапросаПоЗерновымРаспискам) Экспорт
	Для Каждого СтрокаТЗ ИЗ РезультатЗапросаПоЗерновымРаспискам Цикл
		СтрокаТЗ.Зачтенныйвес = - СтрокаТЗ.ЗачтенныйВес;
	Конеццикла;
	
	Возврат РезультатЗапросаПоЗерновымРаспискам;
КонецФункции
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
		...
		Проводка.Содержание   = "Возврат зерна";
...
Процедура ДвиженияРегистровПодсистемыНДС(СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ, Заголовок)
	...
	//	Если ТабЧасть.Количество()> 0 Тогда              			
			УчетНДСИАкциза.СформироватьДвиженияПоРегиструКорректировкиНДСКВозмещению(СтруктураШапкиДокумента, ТаблицаПоТоварам, "Товары", Движения, Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаПоУслугам    = ПодготовитьТаблицуТоваров(РезультатЗапросаПоУслугам, СтруктураШапкиДокумента);
 	...
 	РезультатЗапросаПоЗерновымРаспискам    = УправлениеРесурсами.ПодготовитьТаблицуЗерновыхРасписок(ЭтотОбъект);

	ТаблицаПоЗерновымРаспискам = ПодготовитьТаблицуПоЗерновымРаспискам(РезультатЗапросаПоЗерновымРаспискам);
