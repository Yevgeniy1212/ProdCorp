...
Функция ПечатьОписи() Экспорт
    ...
    Возврат ТабДок;
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДок, 1, , "Данные табличной части документа " + ЭтотОбъект, Ссылка);
КонецФункции 
...
Процедура ЗаполнитьСчетаУчета() Экспорт
	...
	МассивОтбора.Добавить(ПланыСчетов.Типовой.АрендованноеИмущество);	
	...
	МассивОтбора.Добавить(ПланыСчетов.Типовой.АмортизацияАрендованногоИмущества);
...
Функция ПроверкаПоСтроке(СтрокаОС, БезусловныеРеквизиты, БухУчет, НалУчет, Отказ, Заголовок, МетаданныеРеквизиты)
	...
	Если СтрокаОС.НачислятьАмортизациюБУ Тогда
		...
		Если СтрокаОС.СпособНачисленияАмортизацииБУ = Перечисления.СпособыНачисленияАмортизацииОС.Производственный Тогда
			...
			РеквизитыДляПроверки.Вставить("ВыработкаКоличествоБУ" ,СтатусСообщения.Важное);
	...
	Для Каждого СтрокаОС Из ТабОС Цикл
		...
		Если СтрокаОС.ОбъектИмущественногоНалога Тогда
			...
			Если мОтображатьСтруктурныеПодразделения Тогда
				...
				Движение.СтруктурнаяЕдиница = ?(СтруктураШапкиДокумента.ПодразделениеОрганизации.ЯвляетсяСтруктурнымПодразделением,СтруктураШапкиДокумента.ПодразделениеОрганизации, СПДокумента);
			Иначе
...
Процедура ФормированиеПроводокНал(ТабОС, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаОС Из ТабОС Цикл
		...
		Иначе	
			...
			Стоимость = СтрокаОС.ТекущаяСтоимостьБУ - СтрокаОС.НакопленнаяАмортизацияБУ - СтрокаОС.ТекущаяСтоимостьПР;
			// не ФА моглут являться объекты по которым не предусмотрена амортизация в БУ, при этом они используются для целей СГД. Поэтому считать, что вся стоимость не ФА является ПР нельзя.
			// Таким образом по умолчанию стоимость не ФА отражается как НУ, но если это объект не используемый в СГД, его можно провести с видом учета ПР.
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, Отказ,Заголовок)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		ДвиженияПоРегистрамИмущественногоЗемельногоТранспортногоНалогов ( ТабОС, СтруктураШапкиДокумента);
