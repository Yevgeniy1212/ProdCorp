...
Функция СформироватьТаблицуВлияющихОборотов() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	б_ОборотыКЭлиминированиюРасчетнаяТаблица.СтатьяБюджета,
	|	СУММА(б_ОборотыКЭлиминированиюРасчетнаяТаблица.Сумма) КАК Сумма,
	|	б_ОборотыКЭлиминированиюРасчетнаяТаблица.Ссылка.ПериодПланирования КАК Период
	|ИЗ
	|	Документ.б_ОборотыКЭлиминированию.РасчетнаяТаблица КАК б_ОборотыКЭлиминированиюРасчетнаяТаблица
	|ГДЕ
	|	б_ОборотыКЭлиминированиюРасчетнаяТаблица.Ссылка = &ТекДокумент
	|
	|СГРУППИРОВАТЬ ПО
	|	б_ОборотыКЭлиминированиюРасчетнаяТаблица.СтатьяБюджета,
	|	б_ОборотыКЭлиминированиюРасчетнаяТаблица.Ссылка.ПериодПланирования";
...
Процедура ЗаполнитьЗависимымиОборотами(ТекБюджет,
	...
	|ГДЕ
	|	ЗависимостиСтатейБюджета.ВлияющийБюджет В(&Бюджеты)
	|	И ЗависимостиСтатейБюджета.ВлияющаяСтатьяБюджета = &ТекСтатьяБюджета";
	
	Бюджеты = Новый СписокЗначений;
	Бюджеты.Добавить(ТекБюджет);
	Бюджеты.Добавить(Справочники.фин_Бюджеты.НайтиПоКоду("ПР0000060"));
	
	//Запрос.УстановитьПараметр("ТекБюджет",ТекБюджет);
	Запрос.УстановитьПараметр("Бюджеты", Бюджеты);
...
Процедура РассчитатьЗависимыеОбороты() Экспорт
	...
	Для Каждого СтрокаТЗ из тз Цикл
		ЗаполнитьЗависимымиОборотами(СтрокаТЗ.СтатьяБюджета.Владелец,
...
Функция СформироватьТаблицуДляДоходовИРасходов(СтруктураШапкиДокумента)
		...
		|ИЗ
		|	Документ.б_ОборотыКЭлиминированию.РасчетнаяТаблица КАК РасчетнаяТаблица
		|ГДЕ
	...
	тз.ЗаполнитьЗначения(СтруктураШапкиДокумента.Сценарий, "Сценарий");
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента,
	...
	НаборДвижений = Движения.б_БюджетОборотыКЭлиминированию;
	...
	Если Не Отказ Тогда
		Движения.б_БюджетОборотыКЭлиминированию.ВыполнитьДвижения();
	КонецЕсли;

	Для Каждого ТекСтрокаЗависимыеОбороты Из ЗависимыеОбороты Цикл
			
		Движение = Движения.б_БюджетОборотыКЭлиминированию.Добавить();
