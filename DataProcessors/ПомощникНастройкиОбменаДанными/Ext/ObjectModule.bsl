...
Функция ОбработчикКаталогНачалоВыбора(Элемент, СтандартнаяОбработка) Экспорт
	...
	РезультатВыбора = УниверсальныеМеханизмы.ВыбратьКаталог(Элемент.Значение);
					...
					Если НЕ РезультатЗапроса.Пустой() Тогда
						...
						Пока КоличествоНайденныхОбъектов >= 0 цикл
							...
							Для каждого НайденнаяСтрока из МассивНайденныхСтрок цикл
								...
								ДанныеОбъекта = ДанныеОбъектовМетаданных.Найти(НайденнаяСтрока.ВидОбъекта,"ИмяМетаданныхОбъекта");
					...
					Если НЕ РезультатЗапроса.Пустой() Тогда
						...
						Пока КоличествоНайденныхОбъектов >= 0 цикл
							...
							Для каждого НайденнаяСтрока из МассивНайденныхСтрок цикл
								...
								//значение реквизитов объекта из второй ИБ
								СтрокаЭлементаДерева.ЗначенияРеквизитов = Новый Соответствие;
								ДанныеОбъекта = ДанныеОбъектовМетаданныхДокументов.Найти(НайденнаяСтрока.ВидОбъекта,"ИмяМетаданныхОбъекта");
...
Функция СформироватьТекстЗапросаДляДанныхОбъектовМетаданных(ДанныеОбъектовМетаданных) Экспорт
	...
	Для  ИндексДанныхОбъектов = 0 По ДанныеОбъектовМетаданных.Количество()-1 цикл
		...
		ИерархияПодключения = ?(ТипНастройки = Перечисления.ТипыАвтоматическогоОбменаДанными.ОбменЧерезComСоединение, НЕ МетаданныеОбъекта.ВидИерархии = ПодключениеКПриемнику.MetaData().ObjectProperties.HierarchyType.HierarchyOfItems, Истина);
		
		Если МетаданныеОбъекта.Иерархический И (НЕ СокрЛП(МетаданныеОбъекта.ВидИерархии) = "ИерархияЭлементов")
			И ИерархияПодключения Тогда
			ТекстЗапроса = ТекстЗапроса + "
...
Процедура ОтобразитьНовоеСообщениеВФормеХодаОбработки(Знач ТекстСообщения, Знач ПроцентПрибавления = 0)
	...
	мФормаХодаВыполненияОбмена.НадписьСостоянияПрогрессора = ТекстСообщения;	
	мФормаХодаВыполненияОбмена.ЗначениеИндикатора = мФормаХодаВыполненияОбмена.ЗначениеИндикатора + ПроцентПрибавления;		
...
Процедура ИнициализироватьФормуХодаОбработки(Знач ТекстСообщения = "", Знач ПроцентПрибавления = 0)
	...
	Если мФормаХодаВыполненияОбмена = Неопределено Тогда
		...
		мФормаХодаВыполненияОбмена = ПолучитьОбщуюФорму("ФормаИндикатора");
		
	КонецЕсли;
	
	мФормаХодаВыполненияОбмена.НаименованиеОбработкиДанных = "Сохранение настроек для обмена данными ...";
	мФормаХодаВыполненияОбмена.НадписьСостоянияПрогрессора = ТекстСообщения;	
	мФормаХодаВыполненияОбмена.ЗначениеИндикатора = ПроцентПрибавления;
...
Функция СохранитьНастройкиПервогоЭтапа(НастройкаОбменаОбъект, ИнформацияОТипеОбмена, НужноРегистрироватьИзмененияДляОбмена = Истина) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ПолныеПрава.ОбработатьУстановкуВозможногоПрефиксаИнформационнойБазы(ПрефиксДляИБПриемника);
		...
		Если НужноВнестиИзмененияВИнформационнуюБазуПриемника Тогда 
			...
			ПодключениеКПриемнику.ПолныеПрава.ОпределитьФактИспользованияРИБ();
...
ВерсияПлатформыИнформационнойБазыДляПодключения = ОбщегоНазначения.ВерсияПлатформыИнформационнойБазы();
