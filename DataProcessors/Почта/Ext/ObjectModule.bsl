...
Процедура ОбновитьДоступныеУчетныеЗаписи() Экспорт
	...
	мСтруктураДоступныхУчетныхЗаписей = УправлениеЭлектроннойПочтой.ПолучитьДоступныеУчетныеЗаписи(глТекущийПользователь);
...
Процедура ПоместитьИсходныеДанныеВОбъект(XMLИсточник, ПотокВывода, ФормаОбъекта, ДатаЗаписи)
	...
	Для каждого СтрокаОтвет из ОтветыТаблицы Цикл
		...
		ИначеЕсли Вопрос = ПланыВидовХарактеристик.ВопросыДляАнкетирования.РНН  и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.ФизическиеЛица") тогда
			ПотокВывода.РНН = Ответ;
		ИначеЕсли Вопрос = ПланыВидовХарактеристик.ВопросыДляАнкетирования.Национальность  и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.ФизическиеЛица") тогда
			ПотокВывода.Национальность = Ответ;
		//ИначеЕсли Вопрос = ПланыВидовХарактеристик.ВопросыДляАнкетирования.РННГоловногоКонтрагента и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.Контрагенты") тогда
		...
		//	КонецЕсли;
		ИначеЕсли Вопрос = ПланыВидовХарактеристик.ВопросыДляАнкетирования.СИК  и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.ФизическиеЛица") тогда
...
Процедура СоздатьФизлицо(ТекущийПредмет) Экспорт
	...
	ЗапросПоПредмету.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                         ...
	                         |	КОНЕЦ КАК Хранилище,
	                         |	ВЫРАЗИТЬ(ПредметыЭлектронныхПисем.Регистратор КАК Документ.ЭлектронноеПисьмо).КомуТЧ.(ВЫБОР
	                         |			КОГДА ВЫРАЗИТЬ(ПредметыЭлектронныхПисем.Регистратор КАК Документ.ЭлектронноеПисьмо).СтатусПисьма = &Отправленное
	                         |				ТОГДА ВЫРАЗИТЬ(ПредметыЭлектронныхПисем.Регистратор КАК Документ.ЭлектронноеПисьмо).КомуТЧ.АдресЭлектроннойПочты
	                         |			ИНАЧЕ """"
	                         |		КОНЕЦ) КАК Кому,
	                         |	ВЫБОР
	                         |		КОГДА ВЫРАЗИТЬ(ПредметыЭлектронныхПисем.Регистратор КАК Документ.ЭлектронноеПисьмо).СтатусПисьма = &Полученное
	                         |			ТОГДА ВЫРАЗИТЬ(ПредметыЭлектронныхПисем.Регистратор КАК Документ.ЭлектронноеПисьмо).ОтправительАдресЭлектроннойПочты
	                         |		ИНАЧЕ """"
	...
	Пока ВыборкаЗапроса.Следующий() Цикл
		...
		Пока ВыборкаПолучателей.Следующий() Цикл
			Если ЗначениеЗаполнено(ВыборкаПолучателей.АдресКому) тогда
				СтрокаИД 				= ИсходныеДанные.Добавить();
			...
			КонецЕсли;
		КонецЦикла;
		Если ЗначениеЗаполнено(ВыборкаЗапроса.АдресОтКого) тогда
			СтрокаИД 				= ИсходныеДанные.Добавить();
			СтрокаИД.ВидДанных 		= "КИ_адресЭП";
