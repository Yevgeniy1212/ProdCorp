...
Процедура ЗаполнитьТаблицуПолей()
	...
	мФормаПрогрессора.СтрокаСостоянияПрогрессора = "Выполняется заполнение таблицы полей ...";
	...
	Если НаименованиеПрограммы = "OutlookExpress" Тогда
		...
		Для каждого ЭлементСписка Из СписокПолей Цикл
			...
			мФормаПрогрессора.ЗначениеИндикатора = Число(СписокПолей.Индекс(ЭлементСписка) + 1);
	...
	ИначеЕсли НаименованиеПрограммы = "MSOutlook" Тогда
		...
		Для каждого ЭлементСписка Из СписокПолей Цикл
			...
			мФормаПрогрессора.ЗначениеИндикатора = СписокПолей.Индекс(ЭлементСписка) + 1;
...
Процедура ЗаполнитьТаблицуПолейВыгрузки()
	...
	мФормаПрогрессора.СтрокаСостоянияПрогрессора = "Заполняется таблица полей выгрузки ...";
	...
	Для каждого Поле Из мСписокРеквизитовОбъекта Цикл
		мФормаПрогрессора.ЗначениеИндикатора = мСписокРеквизитовОбъекта.Индекс(Поле) + 1;
...
Функция ЗаполнитьТаблицуДанныхФайла()
	...
	мФормаПрогрессора.СтрокаСостоянияПрогрессора = "Выполняется анализ данных файла ...";
	...
	Для а = 2 По ТекстФайла.КоличествоСтрок() Цикл
		...
		мФормаПрогрессора.ЗначениеИндикатора = а;
...
Процедура ПроверитьТаблицуОбъектовВыгрузки() Экспорт
	...
	мФормаПрогрессора.СтрокаСостоянияПрогрессора = "Проверяется таблица объектов выгрузки ...";
...
Процедура ЗаписатьВыгружаемуюИнформацию() Экспорт
	...
	мФормаПрогрессора.СтрокаСостоянияПрогрессора = "Формируется выходной файл ...";
	мФормаПрогрессора.ЗначениеИндикатора = 0;
	мФормаПрогрессора.Открыть();
	...
	НомерПоляКИ = 0;
	мФормаПрогрессора.ЭлементыФормы.Индикатор.МаксимальноеЗначение = НастройкаПолейВыгрузки.Количество();
	Для каждого СтрокаТаблицы Из НастройкаПолейВыгрузки Цикл
	
		мФормаПрогрессора.ЗначениеИндикатора = НастройкаПолейВыгрузки.Индекс(СтрокаТаблицы) + 1;
	...
	Если СписокВидовКИ.Количество() > 0 Тогда
		...
		|		ВЫРАЗИТЬ(КонтактнаяИнформацияВнутр.Представление КАК СТРОКА(1000)) КАК Представление,
...
Процедура ЗаписатьЗагружаемуюИнформацию(КомментироватьПроцессЗагрузки = Истина, РодительДляЗагрузки = Неопределено) Экспорт
	...
	мФормаПрогрессора.СтрокаСостоянияПрогрессора = "Загрузка данных ...";
	...
	Для каждого СтрокаТаблицы Из НастройкаЗагрузкиЗначений Цикл
		...
		мФормаПрогрессора.ЗначениеИндикатора = НастройкаЗагрузкиЗначений.Индекс(СтрокаТаблицы) + 1;
