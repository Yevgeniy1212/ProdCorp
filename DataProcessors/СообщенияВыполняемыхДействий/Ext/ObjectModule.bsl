...
Функция ДобавитьСообщение(ТекстСообщения, ВидСообщения, 
	...
	Если ПорядокСообщения = Неопределено Тогда
		Если ТипЗнч(РодительскаяСтрока) = Тип("СтрокаДереваЗначений") Тогда
			НоваяСтрока.ПорядокСообщения = РодительскаяСтрока.Строки.Количество();
		Иначе
			НоваяСтрока.ПорядокСообщения = Сообщения.Строки.Количество();
		КонецЕсли;
	Иначе
		НоваяСтрока.ПорядокСообщения = ПорядокСообщения;
	КонецЕсли;
...
Функция ЗаменитьТекстСообщения(ТекстСообщенияИсходный, ТекстСообщенияНовый, ВключатьПодчиненные = Ложь) Экспорт
	
	НайденнаяСтрока = Сообщения.Строки.Найти(ТекстСообщенияИсходный, "Текст", ВключатьПодчиненные);
	
	Если НайденнаяСтрока <> Неопределено Тогда
		НайденнаяСтрока.Текст = ТекстСообщенияНовый;
	КонецЕсли;
...
Процедура ПоказатьСообщения() Экспорт
	...
	// отсортируем строки сообщений
	Сообщения.Строки.Сортировать("ПорядокСообщения ВОЗР", Истина);
...
СпецСимволы = "%%";
Сообщения.Колонки.Добавить("ПорядокСообщения"); // порядок сообщения для вывода, используется если сообщения добавляемые первыми в дерево надо отображать в середине
