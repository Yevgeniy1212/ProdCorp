...
Функция СформироватьНачалоЗапроса(Знач ТаблицаПоиска, Знач ИмяПоляВозврата, Знач НужноРазрешенные, Знач КоличествоСтрокДляВыбора, 
	...
	СтрокаИмениПоляПоиска = ?(Не ЗначениеЗаполнено(ИмяПоляПоискаДанныхДляПользователя), "Выразить ( """" Как Строка(1000)) КАК ИМЯПОЛЯПОИСКА", "Выразить(""" + ИмяПоляПоискаДанныхДляПользователя + """ Как Строка(1000)) КАК ИМЯПОЛЯПОИСКА");
		...
		Для Каждого СтрокаПоля Из СтрокаДереваПредка.СписокПолей Цикл
			...
			Если СтрокаПоля.Пометка Тогда
				...
				Если мФормаПрогрессора <> Неопределено Тогда
					мФормаПрогрессора.НадписьСостоянияПрогрессора = "Выборка данных из """ + СтрокаДереваПредка.ОписаниеТаблицыПоиска +
...
Процедура ДополнитьСписокСсылок(ТекущийСписокСсылок, ДополняющаяТаблица, ДополнятьОбъединением = Истина,Знач ИмяОбластиПоиска, 
	...
	Если мФормаПрогрессора <> Неопределено Тогда									
		мФормаПрогрессора.НадписьСостоянияПрогрессора = "Обработка результатов запроса к информационной базе. 0 из " + ОбщееКоличествострокаПереноса;
	КонецЕсли;	
	...
	Если (ТекущийСписокСсылок.Строки.Количество() = 0) Тогда
		...
		Для Каждого ЭлементДерева Из ДополняющаяТаблица.Строки Цикл
			...
			Если НомерСтрокиПереноса%100 = 0 Тогда
				...
				Если мФормаПрогрессора <> Неопределено Тогда
					мФормаПрогрессора.НадписьСостоянияПрогрессора = "Обработка результатов запроса к информационной базе. " + Строка(НомерСтрокиПереноса) + " из " + ОбщееКоличествострокаПереноса;
				КонецЕсли;
...
Функция ВыполнитьПрограммныйПоискДанных(ДеревоПоискаДляОбъектовТекущегоТипа, СтрокаПоиска, Знач МаксимальноеКоличествоОбъектовДляПоиска,
	...
	Если ПоказыватьФормуПрогресса Тогда
		...
		мФормаПрогрессора = ПолучитьОбщуюФорму("ФормаИндикатора");
		...
		мФормаПрогрессора.НадписьСостоянияПрогрессора = "Поиск информации ...";
