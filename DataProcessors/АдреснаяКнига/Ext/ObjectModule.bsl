...
Процедура ЗаполнитьСписокГруппРассылки(ГруппыРассылкиСписок) Экспорт
	...
	Иначе
		СписокУчетныхЗаписей = УправлениеЭлектроннойПочтой.ПолучитьДоступныеУчетныеЗаписи(глТекущийПользователь).Отправка.Скопировать();
	КонецЕсли;
		...
		Иначе
			...
			Если ТипЗнч(СписокЭлементов) = Тип("СписокЗначений") Тогда
				...
				Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
				               ...
				               |	ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК СТРОКА(1000)) КАК АдресЭлектроннойПочты,
				               ...
				               |	И КонтактнаяИнформация.Объект В ИЕРАРХИИ(&СписокОбъектов)";
