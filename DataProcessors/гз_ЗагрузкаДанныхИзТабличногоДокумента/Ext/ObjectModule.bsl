...
Перем Колонки; // локальная переменная

// Структура примитивных типов (Булево, Дата, Строка, Число)
Перем ПримитивныеТипы; // локальная переменная

// Структура Менеджеров объектов метаданных
Перем МенеджерыОбъектовМетаданных; // локальная переменная

// Соответствие, для кэширования информации о соответствии типа менеджеру
Перем КешМенеджеровПоТипу; // локальная переменная
...
КонецПроцедуры // гз_ОбщегоНазначения.мСообщитьОбОшибке()
...
//		игнорируются.
//		Например,
//		ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(",а,,,б", ",") возвратит массив значений из пяти элементов,
//		три из которых - пустые строки, а
//		ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(" а   б", " ") возвратит массив значений из двух элементов
//
//	Параметры:
...
//
Функция мПрочитатьТабличныйДокументИзExcel(ТабличныйДокумент, ИмяФайла, ИмяЛистаExcel) Экспорт
	...
	Попытка
		...
		ИндексЛиста = 1;
		//Для ИндексЛиста = 1 По Excel.Sheets.Count Цикл
		//	ЭтотОбъект.ПолучитьФорму("Форма").Лист.Добавить(Excel.Sheets(ИндексЛиста).Name);
		//КонецЦикла; 
		ExcelЛист = Excel.Sheets(Строка(ИмяЛистаExcel));
	Исключение
...
Функция ПолучитьМетаданныеИсточника() Экспорт
	...
	Если ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_ПланЗакупокПодразделения.ПолучитьСсылку()) 									ИЛИ
	ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_ГодовойПлан.ПолучитьСсылку())									 					ИЛИ
	ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_ЗаказПодразделения.ПолучитьСсылку()) Тогда 
		Возврат СсылкаИсточника.Метаданные().ТабличныеЧасти.Найти("СоставЗаявкиТовары");
	ИначеЕсли ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_ЦеновоеПредложениеПоставщика.ПолучитьСсылку())		 					ИЛИ 
	ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_ОбъявлениеОПроведенииГосударственныхЗакупок.ПолучитьСсылку())						ИЛИ
	ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_ИтогиГосударственныхЗакупок.ПолучитьСсылку())								 		ИЛИ
	ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_ПоступлениеТоваровРаботУслугПоДоговорамГосударственныхЗакупок.ПолучитьСсылку())	ИЛИ
	ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_УстановкаПлановыхЦенНоменклатуры.ПолучитьСсылку())									ИЛИ
	ТипЗнч(СсылкаИсточника.Ссылка) = ТипЗнч(Документы.гз_РегистрацияДоговораПоГосударственнымЗакупкам.ПолучитьСсылку())  	Тогда 
		Возврат СсылкаИсточника.Метаданные().ТабличныеЧасти.Найти("Товары");
	КонецЕсли;
...
Процедура ОбновитьДанныеТабличногоДокумента(ТабличныйДокумент, БезВопросов = Ложь) Экспорт
	...
	Источник = СсылкаИсточника[ТабличнаяЧастьИсточника.Имя];
...
Функция ЗагрузитьДанные(ТабличныйДокумент, Индикатор) Экспорт
	...
	ИначеЕсли РежимЗагрузки = 1 Тогда
		...
		ОбъектИсточника = СсылкаИсточника;
		Источник = ОбъектИсточника[ТабличнаяЧастьИсточника.Имя];
...
Функция ОбработатьОбласть(Область, Колонка, ТекущиеДанные, ТекстыЯчеек)
	...
	Иначе
		...
		Если НайденныеЗначения.Количество() = 0 Тогда
			...
			Если Колонка.ИмяРеквизита = "Номенклатура" И СоздаватьНовыеЭлементы Тогда 
				НовыйОбъект = Справочники["Номенклатура"].СоздатьЭлемент();
				НовыйОбъект.Родитель = ГруппаНоменклатуры.Ссылка;
				НовыйОбъект.Наименование = Представление;
...
Процедура ЗаполнитьНастройкиКолонок(Настройки) Экспорт
	...
	Если ТипЗнч(Настройки) = Тип("ТабличныйДокумент") Тогда
		...
		Попытка
			...
			Пока НЕ ПустаяСтрока(ИмяРеквизита) Цикл
				...
				Для к = 1 По СтрЧислоСтрок(ОписаниеТиповСтрокой)  Цикл
					...
					кс = Неопределено;КвалификаторЧисла = Неопределено;кд = Неопределено;
					...
					ИначеЕсли МассивЧастейТипа[0] = "число" Тогда
						Тип = Тип("Число");
						КвалификаторЧисла = Новый КвалификаторыЧисла(мПривестиКЧислу(МассивЧастейТипа[1]),мПривестиКЧислу(МассивЧастейТипа[2]),?(МассивЧастейТипа.Количество() = 4, ДопустимыйЗнак.Неотрицательный, ДопустимыйЗнак.Любой));
					...
					Типы.Добавить(Тип);
				КонецЦикла;
				ОписаниеТипов = Новый ОписаниеТипов(Типы, КвалификаторЧисла, кс, кд);
		...
		ТабличнаяЧастьИсточника = ?(ВосстановленныйРежимЗагрузки,МетаданныеОбъекта,Неопределено);
