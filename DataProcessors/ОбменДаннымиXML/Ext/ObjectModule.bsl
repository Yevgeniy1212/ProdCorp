	...
	Для каждого ПКС Из КоллекцияПКС Цикл
		...
		Если (ПКОСвойств <> Неопределено) 
			...
			Если ТипЗнч(Параметры) = Тип("Структура") Тогда
				Если Параметры.Свойство("ПроверятьДатуНачалаВыгрузкиДокументов") Тогда
					Если (ПКО.Имя = "АктуальныеЭСФ" ИЛИ ПКО.Имя = "ЭСФ") 
						И ТипЗнч(Значение) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
						
						Параметры.ПроверятьДатуНачалаВыгрузкиДокументов = Ложь;
					Иначе
						Параметры.ПроверятьДатуНачалаВыгрузкиДокументов	= Истина;
					КонецЕсли;
				КонецЕсли;		
			КонецЕсли;	
...
Функция ВыполнитьОтложенныеОперацииСДокументами(СтрокаСообщенияОбОшибкеОтложенногоДвиженияДокументов = "", 
	...
	Попытка
		...
		МакетПриоритетов = Вычислить("ПолучитьОбщийМакет(""ПорядокОтложенногоПроведенияДокументов"")");
...
Процедура ИнициализацияРеквизитовИМодульныхПеременных()
	...
	// Версия платформы для подключения
