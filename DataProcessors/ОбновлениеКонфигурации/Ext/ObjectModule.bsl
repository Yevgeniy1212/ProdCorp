...
Перем ТехническаяИнформацияОбОшибке Экспорт;
...
Процедура ВосстановитьНастройкиОбновления() Экспорт
	...
	РежимОбновления = ?(ПроцедурыОбменаДанными.ОпределитьЭтаИнформационнаяБазаФайловая(), 0, 2);
	...
	Попытка
		...
		Если НестандартныеПортыСервера ИЛИ ПроцедурыОбменаДанными.ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
...
Функция ПолучитьФайлССервера(Знач Адрес, Знач ЛокальныйПуть, Знач ВыдаватьСообщения = Истина, 
	...
	Если НЕ ЗначениеЗаполнено(Адрес) ИЛИ НЕ ЗначениеЗаполнено(ЛокальныйПуть) Тогда
		...
		ТехническаяИнформацияОбОшибке = ТекстОшибки;
		Возврат ТекстОшибки;
	КонецЕсли;
...
#КонецЕсли
	...
	Если Результат И НЕ ПроцедурыОбменаДанными.ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
...
Функция ВставитьПараметрыСкрипта(Знач Текст)
	...
	Результат = СтрЗаменить(Результат, "[БлокироватьСоединенияИБ]", ?(ПроцедурыОбменаДанными.ОпределитьЭтаИнформационнаяБазаФайловая(), "false", "true"));
