...
Процедура НастроитьПредставлениеСроков()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |ИЗ
	               |	Задача.фин_ЗадачиБюджетногоПроцесса.ЗадачиПоИсполнителю(
	               |			&ТекПользователь,
	               |			Выполнена = ЛОЖЬ
	               |				И Оповещение = ИСТИНА
...
Процедура ВыполнитьЗадачи(Команда)
	...
	Если Элементы.Список.ВыделенныеСтроки.Количество() > 1 Тогда
		...
		ОбработчикОповещения = Новый ОписаниеОповещения("ОбработкаВопроса",ЭтотОбъект);
		ПоказатьВопрос(ОбработчикОповещения,"Вы уверены, что следует прекратить выделенные напоминания?", РежимДиалогаВопрос.ДаНет, 0);
...
&НаКлиенте
Процедура ОбработкаВопроса(Ответ,ДополнительныеПараметры) Экспорт
...
Процедура ОбработатьВыделенныеСтроки(Режим="Открыть")
	...
	Пока ШагПрохода <= Элементы.Список.ВыделенныеСтроки.Количество() - 1 Цикл
		...
		Если Режим = "Открыть" Тогда
			ОткрытьФорму("Задача.фин_ЗадачиБюджетногоПроцесса.ФормаОбъекта",Новый Структура("Ключ",ВыделеннаяСтрока.Ссылка));
